...
[ ] Overvej en global Prettier-konfiguration i monorepo-roden.
...
[ ] Firebase Projekt Setup & SDK Integration:
Opret et nyt Firebase-projekt i Firebase Console.
Aktiver Firebase Authentication (vælg login-metoder, f.eks. E-mail/Password, Google).
(Valgfrit for nu) Aktiver Firestore eller Realtime Database.
Hent Firebase config-objektet og gem det sikkert i apps/frontend/ (via miljøvariabler).
Initialiser Firebase SDK i apps/frontend/src/lib/firebase.ts (eller lignende).
