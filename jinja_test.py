...
if not os.path.isdir(template_dir):
    ...
    print(
        f"Please ensure this script is in the project root directory and the 'templates' folder exists there."
    )
...
try:
    ...
    env = jinja2.Environment(
        loader=jinja2.FileSystemLoader(template_dir), enable_async=False
    )
    ...
    try:
        ...
        print(
            f"ERROR: Unexpected error during direct parsing of _macros.html: {type(e).__name__}: {e}"
        )
        print("\nFull Traceback:")
        traceback.print_exc()
    ...
    try:
        ...
        print(
            "SUCCESS: Jinja successfully parsed a template string importing from _macros.html!"
        )
    except jinja2.exceptions.TemplateSyntaxError as e:
        print(f"ERROR: TemplateSyntaxError during import parsing!")
        print(
            f" -> File causing error: {getattr(e, 'filename', 'N/A')}"
        )  # Should point to _macros.html if error occurs
        ...
        print(
            f"ERROR: Unexpected error occurred during import parsing: {type(e).__name__}: {e}"
        )
        print("\nFull Traceback:")
        traceback.print_exc()
