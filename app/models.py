...
from datetime import datetime, timezone, timedelta # Brug timezone-aware datetime, ADD timedelta
import re
...
from sqlalchemy import func, Table, desc # Added Table for association_table, ADDED desc
...
user_badge_association = Table('user_badge_association', db.Model.metadata,
    db.Column('user_id', db.Integer, db.ForeignKey('user.id'), primary_key=True),
    db.Column('badge_id', db.Integer, db.ForeignKey('badge.id'), primary_key=True),
    db.Column('awarded_at', db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
)
...
class User(db.Model, UserMixin):
    ...
    _is_active_db = db.Column(db.Boolean, default=True, nullable=False, name='is_active')
    
    balance = db.Column(db.Float, default=0.0, nullable=False)
    registration_date = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=False)
    last_login = db.Column(db.DateTime(timezone=True), nullable=True)
    last_seen = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=True) # Track user activity
    ...
    backup_codes = db.Column(db.Text, nullable=True) # Store as JSON string or comma-separated
    ...
    xp_next_level = db.Column(db.Integer, default=100, nullable=False) # Example starting value
    ...
    # Relationships
    badges = db.relationship('Badge', secondary=user_badge_association, back_populates='users', lazy='dynamic')
    bet_history_entries = db.relationship('BetHistory', backref='user', lazy='dynamic', cascade="all, delete-orphan")
    ...
    def avatar_url(self):
        ...
        default_avatar_path = current_app.config.get('DEFAULT_AVATAR', 'static/avatars/default_avatar.png') # Ensure full path for fallback
        ...
        if avatar_filename and isinstance(avatar_filename, str):
            ...
            avatar_upload_subdir = current_app.config.get('AVATAR_UPLOAD_SUBDIR', 'avatars')
            # Construct path relative to 'static' folder for url_for
            user_avatar_path_for_url_for = f"{avatar_upload_subdir}/{avatar_filename}".replace("\\", "/")
            
            final_filename_for_url_for = user_avatar_path_for_url_for
        
        try:
            # _external=False is default, but good to be explicit for clarity
            return url_for('static', filename=final_filename_for_url_for, _external=False)
        except RuntimeError:
            # This can happen if url_for is called outside of an active app context
            log.error(f"Could not generate avatar URL for '{final_filename_for_url_for}' (RuntimeError: likely no app context). User: {self.username}")
            # Fallback to a manually constructed path if url_for fails
            return f"/static/{default_avatar_path}"
        except Exception as e:
            log.exception(f"Unexpected error generating avatar URL for user '{self.username}' (file: {final_filename_for_url_for}): {e}")
            return f"/static/{default_avatar_path}"
    ...
    def post_count(self) -> int:
        ...
        try:
            ...
            count = db.session.query(func.count(ForumPost.id)).filter(ForumPost.author_username == self.username).scalar()
            return count or 0
        except Exception as e:
...
class Badge(db.Model):
    ...
    color = db.Column(db.String(50), nullable=True) # e.g., 'danger', 'info', or hex code

    users = db.relationship('User', secondary=user_badge_association, back_populates='badges', lazy='dynamic')
...
class BetHistory(db.Model):
    ...
    session_id = db.Column(db.String(36), nullable=True, index=True) # Link to Redis session ID
    ...
    result = db.Column(db.String(50), nullable=True) # e.g., "won", "lost", "pending", "cancelled"
    ...
    def __init__(self, user_id: int, stake: float, session_id: Optional[str] = None,
                 match_name: Optional[str] = None, outcome_name: Optional[str] = None,
                 payout: Optional[float] = 0.0, result: Optional[str] = "pending",
                 status: Optional[str] = "open", timestamp: Optional[datetime] = None, **kwargs):
...
class InviteCode(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    code = db.Column(db.String(36), unique=True, nullable=False, index=True) # e.g., UUID
    
    created_by_username = db.Column(db.String(80), db.ForeignKey('user.username'), nullable=True) # Optional: link to creator
    creator = db.relationship('User', backref=db.backref('created_invite_codes', lazy='dynamic'), foreign_keys=[created_by_username])
    ...
    is_active = db.Column(db.Boolean, default=True, nullable=False) # Can be manually deactivated
    ...
    used_by_username = db.Column(db.String(80), db.ForeignKey('user.username'), nullable=True)
    user_who_used = db.relationship('User', backref=db.backref('used_invite_code', uselist=False), foreign_keys=[used_by_username])
...
class PasswordResetRequest(db.Model):
    ...
    def is_valid(self) -> bool:
        ...
        if expires_aware and (expires_aware.tzinfo is None or expires_aware.tzinfo.utcoffset(expires_aware) is None):
             log.warning(f"PasswordResetRequest {self.id} expires_at was offset-naive. Assuming UTC.")
...
# ==========================================================
#       Eksisterende User Klasse (Fra JSON) - Omdøbt
# ==========================================================
class JsonUserWrapper(UserMixin): # Renamed from User
    """
    Wrapper-klasse for brugerdata hentet fra JSON (players.json).
    Bruges af Flask-Login via `load_user` callback'en.
    Dette er IKKE en SQLAlchemy database-model.
    """

    def __init__(self, user_id: str, data: dict):
        self.id = user_id # Sæt Flask-Logins 'id'
        self.data = data if data else {} # Gem resten af data

    # --- Flask-Login påkrævede/anbefalede properties/methods ---
    @property
    def is_active(self): return self.data.get('is_active', True)

    # --- Kerneegenskaber ---
    @property
    def username(self): return self.data.get('username', self.id)
    @property
    def password_hash(self): return self.data.get('password_hash')
    @property
    ...
    # --- Profil/Statistik Egenskaber ---
    @property
    def balance(self):
        try: return float(self.data.get('balance', 0.0))
        except (ValueError, TypeError): return 0.0
    @property
    def registration_date(self): return self.data.get('registration_date')
    @property
    def last_login(self): return self.data.get('last_login')
    @property
    def rank(self): return self.data.get('rank', 'Nybegynder')
    @property
    def join_date(self): return self.registration_date
    @property
    def invited_by(self): return self.data.get('invited_by')
    @property
    def uid(self):
        """
        Returnerer brugerens UID som et heltal (int) hvis muligt.
        Forsøger at konvertere fra streng, hvis det ligner et tal.
        Returnerer None, hvis UID mangler, ikke er et tal, eller ikke kan konverteres.
        """
        raw_uid = self.data.get('uid') # Hent værdien (kan være int, str, None, ...)

        # 1. Tjek om det allerede ER et heltal
        if isinstance(raw_uid, int):
            return raw_uid

        # 2. Hvis ikke, prøv at konvertere det til et heltal
        #    Dette håndterer både tal-strenge ("2") og None (giver TypeError)
        if raw_uid is None: # Explicitly handle None case first
             return None
        try:
            # Forsøg konvertering only if not None
            int_uid = int(raw_uid)
            return int_uid
        except (ValueError, TypeError):
            # Konvertering fejlede (f.eks. ved UUID-streng eller andet non-int string)
            if raw_uid is not None: # Log kun en advarsel hvis der *var* en værdi, der fejlede
                 # Brug self.id eller self.username for bedre logning, hvis de er tilgængelige
                 user_identifier = getattr(self, 'id', 'UkendtBruger')
                 log.warning(f"UID for bruger '{user_identifier}' ('{raw_uid}') er ikke et heltal eller kan ikke konverteres. uid property returnerer None.")
            # Returner None for at signalere, at et gyldigt heltal-UID ikke kunne findes
            return None
    ...
    def avatar_url(self):
        # Import locally to avoid circular dependencies
        from flask import url_for, current_app
        avatar_filename = self.data.get('avatar')
        default_avatar_path = current_app.config.get('DEFAULT_AVATAR', 'avatars/default_avatar.png')
        final_filename = default_avatar_path

        if avatar_filename and isinstance(avatar_filename, str):
            avatar_subdir = current_app.config.get('AVATAR_UPLOAD_SUBDIR', 'avatars')
            user_avatar_path = f"{avatar_subdir}/{avatar_filename}".replace("\\", "/")
            final_filename = user_avatar_path

        try:
            return url_for('static', filename=final_filename, _external=False)
        except RuntimeError:
             log.error(f"Could not generate avatar URL for '{final_filename}' (likely no app context).")
             return f'/static/{default_avatar_path}'
        except Exception as e:
             log.exception(f"Unexpected error generating avatar URL for user '{self.id}': {e}")
             return f'/static/{default_avatar_path}'

    # --- 2FA Egenskaber ---
    @property
    def is_2fa_enabled(self): return self.data.get('2fa_enabled', False)
    @property
    def twofa_secret(self): return self.data.get('2fa_secret')
    @property
    def backup_codes(self): return self.data.get('backup_codes', [])

    # --- Spil/Stats Relaterede Egenskaber ---
    @property
    def wins(self): return int(self.data.get('wins', 0))
    @property
    def losses(self): return int(self.data.get('losses', 0))
    @property
    def total_staked(self):
        try: return float(self.data.get('total_staked', 0.0))
        except (ValueError, TypeError): return 0.0
    @property
    def total_won(self):
        try:
            return float(self.data.get('total_won', 0.0))
        except (ValueError, TypeError):
            log.warning(f"Invalid 'total_won' data for user {self.id}: {self.data.get('total_won')}")
            return 0.0
    @property
    def total_lost(self):
        try:
            return float(self.data.get('total_lost', 0.0))
        except (ValueError, TypeError):
            log.warning(f"Invalid 'total_lost' data for user {self.id}: {self.data.get('total_lost')}")
            return 0.0

    # --- Aktie/Portefølje Egenskaber ---
    @property
    def portfolio(self): return self.data.get('portfolio', {})
    @property
    def stock_transactions(self): return self.data.get('stock_transactions', [])
    @property
    def watchlist(self): return self.data.get('watchlist', [])
    @property
    def dividends(self): return self.data.get('dividends', [])

    # --- Forum Relaterede Egenskaber ---
    @property
    def post_count(self):
        if 'ForumPost' in globals(): # Tjek om ForumPost er defineret globalt i modulet
             try:
                 # Vigtigt: Kræver ForumPost model (hvilket den nu er)
                 return ForumPost.query.filter(ForumPost.author_username == self.username).count()
             except NameError:
                  log.error("NameError accessing ForumPost within User.post_count (Should not happen if ForumPost is defined above)")
                  return 0
             except Exception as e:
                 log.error(f"Error counting forum posts for user {self.username}: {e}")
                 return 0
        else:
             log.warning("ForumPost class not found in globals() for post_count calculation.")
             return 0
    ...
    def __eq__(self, other):
        if isinstance(other, JsonUserWrapper): # Updated class name
...
class ForumPost(db.Model):
    ...
    thread = db.relationship('ForumThread', back_populates='posts', foreign_keys=[thread_id]) # Angiv FK eksplicit
    ...
    def _handle_body_set(target, value, oldvalue, initiator):
        if not value or value == oldvalue: return
        log.debug(f"Generating HTML for post {getattr(target, 'id', 'NEW')}")
        allowed_tags = {
            'p', 'br', 'strong', 'b', 'em', 'i', 'u', 'strike', 'sub', 'sup',
            'ul', 'ol', 'li', 'a', 'blockquote', 'code', 'pre', 'h3', 'h4', 'h5', 'h6',
        }
        allowed_attrs = {'a': ['href', 'title', 'rel'], '*': ['class'],}
        allowed_protocols = {'http', 'https', 'mailto'}
        try:
            md_html = markdown(value, output_format='html', extensions=['fenced_code', 'nl2br']) # Use 'html'
            cleaned_html = bleach.clean(md_html, tags=allowed_tags, attributes=allowed_attrs, protocols=allowed_protocols, strip=True, strip_comments=True)
            target.body_html = bleach.linkify(cleaned_html, parse_email=True)
            log.debug(f"Successfully generated sanitized HTML for post {getattr(target, 'id', 'NEW')}")
        except Exception:
            log.exception(f"CRITICAL: Error during Markdown/Bleach processing for Post ID {getattr(target, 'id', 'NEW')}. Falling back.")
...
class ForumCategory(db.Model):
    ...
    threads = db.relationship('ForumThread', back_populates='category', lazy='dynamic', cascade="all, delete-orphan")

    # Corrected type hints for optional parameters
    def __init__(self, name: str, description: Optional[str] = None, icon: Optional[str] = None, sort_order: int = 0):
        self.name = name
    ...
    def thread_count(self) -> int:
        try: return self.threads.count()
        except Exception as e: log.error(f"Error counting threads for category {self.id}: {e}"); return 0

    @property
    def post_count(self) -> int:
        try:
            count = db.session.query(db.func.count(ForumPost.id))\
                .join(ForumThread, ForumPost.thread_id == ForumThread.id)\
                .filter(ForumThread.category_id == self.id)\
                .scalar()
            return count or 0
        except Exception as e: log.error(f"Error counting posts for category {self.id}: {e}"); return 0
    ...
    def last_activity_thread(self) -> Optional['ForumThread']:
        try:
            ...
            log.error(f"Error fetching last activity thread for category {self.id}: {e}")
            return None
    ...
    def generate_slug(self):
        if self.name:
            ...
            while ForumCategory.query.filter(
...
class ForumThread(db.Model):
    ...
    created_at = db.Column(db.DateTime(timezone=True), index=True, default=lambda: datetime.now(timezone.utc))
    updated_at = db.Column(db.DateTime(timezone=True), index=True, default=lambda: datetime.now(timezone.utc)) # Bruges af event listener
    is_sticky = db.Column(db.Boolean, default=False, index=True)
    is_locked = db.Column(db.Boolean, default=False)
    view_count = db.Column(db.Integer, default=0) # Denne opdateres sandsynligvis i din view-funktion

    # --- TILFØJ DENNE LINJE ---
    post_count = db.Column(db.Integer, default=0, nullable=False, index=True) # Kolonne til at gemme antal posts
    # --------------------------
    ...
    first_post_id = db.Column(db.Integer, db.ForeignKey('forum_post.id', use_alter=True, name='fk_thread_first_post'), nullable=True) # Bruges af event listener
    last_post_id = db.Column(db.Integer, db.ForeignKey('forum_post.id', use_alter=True, name='fk_thread_last_post'), nullable=True)   # Bruges af event listener

    first_post = db.relationship('ForumPost', foreign_keys=[first_post_id], post_update=True)
    ...
    def calculated_post_count(self) -> int: # Omdøbt fra post_count
        ...
        try:
            ...
            return 0 # Eller måske return self.post_count (den gemte værdi) som fallback?
...
def _update_thread_stats_on_post_change(mapper, connection, target):
    ...
    try:
        ...
        if not thread_id:
            log.warning(f"Could not get thread_id from target in _update_thread_stats event (Target ID: {post_id}, Type: {type(target)})")
            return

        log.debug(f"--- Event Triggered: Post ID {post_id} changed in Thread ID {thread_id} ---")
        ...
        # Pak resultatet ud
        first_id, last_id, last_time, post_count_actual = result if result else (None, None, None, 0)
        # ----------------------------------------------------------------

        # Find trådens egen oprettelsestid
        thread_created_result = connection.execute(
            db.select(ForumThread.created_at).where(ForumThread.id == thread_id) # <--- Også her db.select()
        ).scalar_one_or_none()
        thread_created = thread_created_result if thread_created_result else datetime.now(timezone.utc)
        ...
        log.debug(f"***** INSIDE _update_thread_stats_on_post_change for thread {thread_id} *****")
        log.debug(f"  Triggered by Post ID: {post_id}")
        log.debug(f"  Attempting to update with (Values from connection query using db.select):") # Opdateret log tekst
        log.debug(f"    -> first_post_id = {first_id} (Type: {type(first_id)})")
        log.debug(f"    -> last_post_id = {last_id} (Type: {type(last_id)})")
        log.debug(f"    -> post_count_actual = {post_count_actual} (Type: {type(post_count_actual)})") # <= TJEK VÆRDIEN HER!
        ...
        log.info(f"Executed update for thread stats (ThreadID: {thread_id}). Target Count was {post_count_actual}.")

    except Exception:
        log.exception(f"Error updating thread stats from ForumPost event for thread_id={thread_id} triggered by post_id={post_id}")
...
class PrivateMessage(db.Model):
    ...
    is_read = db.Column(db.Boolean, default=False, nullable=False, index=True) # Read by recipient (Added index)

    # Threading support
    # Points to the ID of the first message in the thread. NULL if this IS the first message.
    thread_id = db.Column(db.Integer, db.ForeignKey('private_message.id', name='fk_pm_thread_id'), nullable=True, index=True)
    # Relationship to easily get all replies to this message (if it's the start of a thread)
    replies = db.relationship('PrivateMessage', backref=db.backref('parent_message', remote_side=[id]), lazy='dynamic', foreign_keys='PrivateMessage.thread_id')
    ...
    def __init__(self, sender_id: int, recipient_id: int, subject: str, body: str, thread_id: Optional[int] = None, **kwargs):
        super().__init__(**kwargs) # Pass extra kwargs to parent constructor if needed
        self.sender_id = sender_id
        self.recipient_id = recipient_id
        self.subject = subject
        self.body = body # The event listener will handle body_html
        self.thread_id = thread_id # Set the thread ID (will be None for the first message)
    ...
    def _handle_pm_body_set(target, value, oldvalue, initiator):
        ...
        try:
            ...
            cleaned_html = bleach.clean(md_html, tags=allowed_tags, attributes=allowed_attrs, protocols=allowed_protocols, strip=True, strip_comments=True)
            target.body_html = bleach.linkify(cleaned_html, parse_email=True)
            log.debug(f"Successfully generated sanitized HTML for PM {getattr(target, 'id', 'NEW')}")
        except Exception:
            log.exception(f"CRITICAL: Error during Markdown/Bleach processing for PM ID {getattr(target, 'id', 'NEW')}. Falling back.")
            escaped_body = html.escape(value)
            target.body_html = f"<p><b>Fejl under formatering:</b></p><pre>{escaped_body}</pre>"
...
db.event.listen(PrivateMessage.body, 'set', PrivateMessage._handle_pm_body_set, retval=False)
...
class Notification(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False, index=True) # The user receiving the notification
    message = db.Column(db.Text, nullable=False) # The notification text
    link = db.Column(db.String(512), nullable=True) # Optional URL the notification links to
    icon = db.Column(db.String(100), nullable=True) # Optional icon class (e.g., 'bi-trophy-fill')
    category = db.Column(db.String(50), nullable=True, index=True) # e.g., 'badge', 'message', 'session', 'system'
    ...
    # Explicit __init__
    def __init__(self, user_id: int, message: str, link: Optional[str] = None,
                 icon: Optional[str] = None, category: Optional[str] = None,
                 is_read: bool = False, **kwargs):
...
# ==========================================================
#         SQLAlchemy Models for Game Sessions & Sports
# ==========================================================

class GameSession(db.Model):
    __tablename__ = 'game_session'
    id = db.Column(db.String(36), primary_key=True, default=lambda: str(uuid.uuid4())) # Use UUID from Redis as PK
    name = db.Column(db.String(200), nullable=False)
    creator_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False, index=True)
    game_mode = db.Column(db.String(50), nullable=False) # e.g., 'yesno', 'fictional_sport', 'live_sport_coupon'
    status = db.Column(db.String(50), default='open', nullable=False, index=True) # open, locked, resolved, cancelled
    created_at = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc), nullable=False)
    resolved_at = db.Column(db.DateTime(timezone=True), nullable=True)
    # Link to a real sport event if applicable
    event_id = db.Column(db.String(255), db.ForeignKey('sport_event.id'), nullable=True, index=True)
    # For coupon type sessions, store the selections as JSON
    coupon_details = db.Column(db.JSON, nullable=True) # Using generic JSON, or db.JSONB for PostgreSQL
 
    creator = db.relationship('User', backref=db.backref('created_sessions', lazy='dynamic'))
    event = db.relationship('SportEvent', backref=db.backref('game_sessions', lazy='dynamic'))
    ...
    # Explicit __init__
    def __init__(self, id: str, name: str, creator_id: int, game_mode: str,
                 status: str = 'open', created_at: Optional[datetime] = None,
                 resolved_at: Optional[datetime] = None, event_id: Optional[str] = None,
                 coupon_details: Optional[Any] = None, **kwargs):
        super().__init__(**kwargs)
        self.id = id
        self.name = name
    ...
    def __repr__(self):
        return f"<GameSession(id={self.id}, name='{self.name}', status='{self.status}')>"

class Sport(db.Model):
    __tablename__ = 'sport'
    key = db.Column(db.String(100), primary_key=True) # From Odds API (e.g., 'soccer_epl')
    title = db.Column(db.String(100), nullable=False)
    group = db.Column(db.String(100), nullable=False)
    active = db.Column(db.Boolean, default=True, nullable=False)
    last_checked = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))

    events = db.relationship('SportEvent', backref='sport', lazy='dynamic')

    # Explicit __init__
    def __init__(self, key: str, title: str, group: str, active: bool = True,
                 last_checked: Optional[datetime] = None, **kwargs):
        super().__init__(**kwargs)
        self.key = key
        self.title = title
        self.group = group
        self.active = active
        self.last_checked = last_checked if last_checked is not None else datetime.now(timezone.utc)
...
class SportEvent(db.Model):
    ...
    id = db.Column(db.String(255), primary_key=True) # Event ID from Odds API
    sport_key = db.Column(db.String(100), db.ForeignKey('sport.key'), nullable=False, index=True)
    ...
    last_update_api = db.Column(db.DateTime(timezone=True), nullable=True) # When was data last fetched from API?
    # Potential fields for results if you store them
    # home_score = db.Column(db.Integer, nullable=True)
    # away_score = db.Column(db.Integer, nullable=True)
    # status = db.Column(db.String(50), nullable=True) # e.g., Scheduled, InPlay, Finished

    outcomes = db.relationship('SportOutcome', backref='event', lazy='dynamic', cascade="all, delete-orphan")

    # Explicit __init__
    def __init__(self, id: str, sport_key: str, commence_time: datetime,
                 home_team: Optional[str] = None, away_team: Optional[str] = None,
                 last_update_api: Optional[datetime] = None, **kwargs):
        super().__init__(**kwargs)
        self.id = id
    ...
    def __repr__(self):
        return f"<SportEvent(id={self.id}, sport='{self.sport_key}', home='{self.home_team}', away='{self.away_team}')>"

class SportOutcome(db.Model):
    __tablename__ = 'sport_outcome'
    id = db.Column(db.Integer, primary_key=True) # Auto-incrementing PK
    event_id = db.Column(db.String(255), db.ForeignKey('sport_event.id'), nullable=False, index=True)
    bookmaker = db.Column(db.String(100), nullable=False) # Which bookmaker provided the odds
    market_key = db.Column(db.String(100), nullable=False) # e.g., 'h2h', 'totals'
    name = db.Column(db.String(100), nullable=False) # e.g., Home team name, Away team name, Over, Under
    price = db.Column(db.Float, nullable=False) # The odds
    point = db.Column(db.Float, nullable=True) # For spread or total markets
    last_update_api = db.Column(db.DateTime(timezone=True), nullable=True) # When were odds last fetched?
    ...
    # Explicit __init__
    def __init__(self, event_id: str, bookmaker: str, market_key: str, name: str,
                 price: float, point: Optional[float] = None,
                 last_update_api: Optional[datetime] = None, **kwargs):
        super().__init__(**kwargs)
        self.event_id = event_id
...
class FailedLoginAttempt(db.Model):
    ...
    username_attempt = db.Column(db.String(100), nullable=True, index=True) # Username they tried to log in with
    user_agent = db.Column(db.String(255), nullable=True) # Browser/client info

    def __repr__(self):
        return f"<FailedLoginAttempt(id={self.id}, username='{self.username_attempt}', ip='{self.ip_address}', time='{self.timestamp.isoformat()}')>"
