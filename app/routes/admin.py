...
from datetime import datetime, timezone, timedelta # <<< TILFØJET timedelta
import json # For Redis session data
from sqlalchemy import text, exc as sqlalchemy_exc, select # For DB health check, error handling, and select statements
...
from ..data_access import (
    load_players,
    save_players,
    load_invite_codes,  # For liste af invite objekter
    save_invite_codes,  # For liste af invite objekter
    load_system_settings,
    save_system_settings,
    DEFAULT_SYSTEM_SETTINGS,
    add_audit_log,      # <<< TILFØJET for audit logs
    load_audit_logs     # <<< TILFØJET for audit logs
)
...
from ..extensions import db, redis_client, scheduler, socketio # <<< TILFØJET scheduler, socketio
from .. import utils # <<< TILFØJET admin_required decorator
from .sessions import SESSION_KEY_PREFIX, SESSION_PLAYERS_KEY_PREFIX # Import Redis prefixes from sessions.py
...
def generate_invite_code():
    ...
    try:
        ...
        if any(c.get('code') == new_code_str for c in invite_codes_list):
             log.error(f"Admin '{current_user.id}': UUID collision generating invite code: {new_code_str}. Retrying might help.")
             return jsonify({"error": "Kode genereringsfejl (kollision), prøv venligst igen."}), 500
        invite_codes_list.append(new_code_obj)
        if save_invite_codes(invite_codes_list):
            log.info(f"Admin '{current_user.id}' generated new invite code: {new_code_str}")
            # Audit Log
            add_audit_log(action="generate_invite", admin_user=current_user.id, target_resource=new_code_str)
            return jsonify({
                "message": "Ny invite-kode genereret",
                "invite_code": new_code_str
            }), 200
        else:
            log.error(f"Admin '{current_user.id}': FAILED to save invite codes file after attempting to add {new_code_str}.")
            return jsonify({"error": "Kunne ikke gemme den nye invite kode til filen."}), 500
    except Exception as e:
        log.exception(f"Unexpected error in generate_invite_code for admin {current_user.id}: {e}")
        return jsonify({"error": "Intern serverfejl ved generering af invite kode."}), 500
...
def admin_get_invites():
     ...
     try:
          invite_codes_list = load_invite_codes()
          log.info(f"Admin '{current_user.id}' requested list of {len(invite_codes_list)} invite codes.")
          def get_sort_key(item):
              try:
                  return datetime.fromisoformat(item.get('created_at', '1970-01-01T00:00:00Z').replace('Z', '+00:00'))
              except ValueError:
        ...
        return jsonify({"error": "Intern serverfejl ved hentning af invite koder."}), 500
...
def admin_delete_invite(code):
    ...
    try:
        ...
        if not code_to_delete_obj:
            log.warning(f"Admin '{current_user.id}' tried to delete non-existent invite code: {code}")
            return jsonify({"error": "Invite kode ikke fundet."}), 404
        updated_invite_list = [obj for obj in invite_codes_list if obj.get('code') != code]
        if len(updated_invite_list) < original_length:
            log.info(f"Admin '{current_user.id}' attempting to delete invite code: {code} "
            ...
            if save_invite_codes(updated_invite_list):
                ...
                # Audit Log
                add_audit_log(action="delete_invite", admin_user=current_user.id, target_resource=code)
                return jsonify({"message": f"Invite kode '{code}' blev slettet."}), 200
            else:
                log.error(f"Admin '{current_user.id}': FAILED to save invite codes file after removing {code}. File might be out of sync.")
                return jsonify({"error": "Kunne ikke gemme ændringer efter sletning af invite kode."}), 500
        else:
            log.error(f"Logic error: Invite code {code} found but list filtering failed for admin {current_user.id}.")
            return jsonify({"error": "Intern fejl ved filtrering af invite koder."}), 500
    except Exception as e:
        log.exception(f"Fejl i admin_delete_invite for code '{code}', admin {current_user.id}: {e}")
    ...
    try:
        ...
        while True:
            ...
            if keys_from_scan:
                ...
                valid_keys_and_session_ids = [] # Store (key, session_id) tuples for valid keys

                for key_bytes in keys_from_scan:
                    key_str = key_bytes if isinstance(key_bytes, str) else key_bytes.decode('utf-8')
                    parts = key_str.split(':', 1)
                    if len(parts) < 2:
                        log.warning(f"Malformed session key from Redis (cannot split session_id): {key_str}")
                ...
                if not pipe.command_stack: # No valid keys were processed to add commands
                    if cursor == 0: break
                    else: continue
                ...
                for idx, (processed_key_str, processed_session_id) in enumerate(valid_keys_and_session_ids):
                    result_start_index = idx * chunk_size
                    if result_start_index + chunk_size > len(results):
                        log.error(f"Mismatch in Redis pipe results and processed keys for key: {processed_key_str}. Results length: {len(results)}, expected index: {result_start_index}")
                    ...
                    if session_data_raw:
                        ...
                        player_usernames = {p.decode('utf-8') if isinstance(p, bytes) else str(p) for p in player_usernames_bytes}
                        
                        sdata_processed = {
                            "session_id": sdata.get("session_id", processed_session_id), # Use session_id for this specific key
                            "session_name": sdata.get("session_name", f"Session {processed_session_id[:8]}"),
        ...
        all_sessions_list.sort(key=lambda x: x.get('created_at', '0') or '0', reverse=True)
        log.info(f"Admin API: Returning {len(all_sessions_list)} total sessions.")
        return jsonify({"sessions": all_sessions_list})

    except redis.RedisError as e:
        log.exception(f"Redis error fetching all sessions for admin {current_user.id}: {e}")
        ...
        return jsonify({"error": "Intern serverfejl ved hentning af alle sessioner."}), 500
...
def admin_get_users():
    ...
    try:
        ...
        users_list = []
        default_avatar_url = url_for('static', filename='avatars/default_avatar.png', _external=False)
        for username_key, udata in players_data.items():
            if isinstance(udata, dict):
                safe_data = udata.copy()
                avatar_filename = safe_data.get('avatar')
                safe_data['avatar_url'] = url_for('static', filename=f'avatars/{avatar_filename}', _external=False) if avatar_filename else default_avatar_url
                ...
                users_list.append(safe_data)
                 ...
                 log.warning(f"Invalid data type for key '{username_key}' in players file: {type(udata)}. Skipping.")
        log.info(f"Admin '{current_user.id}' requested list of {len(users_list)} users.")
...
def admin_edit_user(username):
    ...
    if unknown_fields:
         log.warning(f"Admin '{current_user.id}' tried to edit unknown/forbidden fields for '{username}': {unknown_fields}")
         return jsonify({"error": f"Ugyldige felter angivet: {', '.join(unknown_fields)}."}), 400
    errors = {}
    ...
    if new_about_me is not None:
        MAX_ABOUT_ME_LENGTH = Config.MAX_ABOUT_ME_LENGTH if hasattr(Config, 'MAX_ABOUT_ME_LENGTH') else 500
    ...
    primary_admin_lower = (os.environ.get('ADMIN_USERNAME') or Config.ADMIN_USERNAME or '').lower()
    try:
        players_data = load_players()
        ...
        for key in players_data.keys():
            if key.lower() == username_lookup_key:
                ...
                break
        if original_user_key is None or not isinstance(players_data.get(original_user_key), dict):
            log.warning(f"Admin '{current_user.id}' failed edit: User '{username}' (lookup: {username_lookup_key}) not found.")
            return jsonify({"error": "Bruger ikke fundet."}), 404
        user_data_to_update = players_data[original_user_key]
        if (primary_admin_lower and
            original_user_key.lower() == primary_admin_lower and
            new_role is not None and new_role != 'admin'):
            log.error(f"DENIED: Admin '{current_user.id}' attempted to change role of primary admin '{original_user_key}' to '{new_role}'.")
            return jsonify({"error": "Kan ikke ændre rollen for den primære admin bruger."}), 403
        updated_fields = []
        ...
        if new_about_me is not None and user_data_to_update.get('about_me') != new_about_me:
            user_data_to_update['about_me'] = new_about_me
            updated_fields.append('about_me')
        if not updated_fields:
            log.info(f"Admin '{current_user.id}' submitted edit for '{original_user_key}', but no changes detected.")
            return jsonify({"message": "Ingen ændringer blev gemt.", "updated_fields": []}), 200
        if save_players(players_data):
            log.info(f"Admin '{current_user.id}' successfully edited user '{original_user_key}'. Fields changed: {', '.join(updated_fields)}.")
            # Audit Log
            add_audit_log(action="edit_user", admin_user=current_user.id, target_resource=original_user_key, details={"updated_fields": updated_fields})
            response_user_data = {field: user_data_to_update.get(field) for field in updated_fields}
            ...
            log.error(f"Admin '{current_user.id}': FAILED to save player data after editing '{original_user_key}'.")
            return jsonify({"error": "Kunne ikke gemme de opdaterede brugeroplysninger."}), 500
    except Exception as e:
        log.exception(f"Unexpected error editing user '{username}' for admin '{current_user.id}': {e}")
...
def admin_delete_user(username):
    ...
    if primary_admin_lower and username_lookup_key == primary_admin_lower:
        log.error(f"DENIED: Admin '{current_user.id}' attempted to delete primary admin user '{username}'.")
    ...
    try:
        ...
        if original_key_to_delete is None:
            log.warning(f"Admin '{current_user.id}' failed delete: User '{username}' not found.")
            return jsonify({"error": "Bruger ikke fundet"}), 404
        del players_data[original_key_to_delete]
        if save_players(players_data):
            log.info(f"Admin '{current_user.id}' successfully deleted user: {original_key_to_delete} (Requested as: {username})")
            log.warning(f"Note: Related session data for '{original_key_to_delete}' (if file-based) was not cleared.")
            # Audit Log
            add_audit_log(action="delete_user", admin_user=current_user.id, target_resource=original_key_to_delete)
            return jsonify({"message": f"Bruger '{original_key_to_delete}' blev slettet."}), 200
        else:
            log.error(f"Admin '{current_user.id}': FAILED to save player data after deleting {original_key_to_delete}.")
            return jsonify({"error": "Kunne ikke gemme ændringer efter sletning af bruger."}), 500
    except Exception as e:
        log.exception(f"Fejl i admin_delete_user for '{username}', admin {current_user.id}: {e}")
...
def update_balance(username):
    ...
    if not data or 'operation' not in data or 'amount' not in data:
        return jsonify({"error": "Manglende JSON data ('operation' og 'amount' påkrævet)"}), 400
    operation = data.get("operation", "").strip().lower()
    reason = data.get("reason")
    if operation not in ["add", "remove", "set"]:
        return jsonify({"error": "Ugyldig 'operation'. Skal være 'add', 'remove', eller 'set'."}), 400
    try:
        amount = float(data["amount"])
        if amount < 0:
            return jsonify({"error": "Beløb ('amount') må ikke være negativt."}), 400
    except (ValueError, TypeError, KeyError):
        return jsonify({"error": "Ugyldigt format for 'amount'. Skal være et tal."}), 400
    try:
        players_data = load_players()
        original_user_key = None
        for key in players_data.keys():
            if key.lower() == username_lookup_key:
                ...
                break
        if original_user_key is None or not isinstance(players_data.get(original_user_key), dict):
            log.warning(f"Admin '{current_user.id}' failed balance update: User '{username}' not found.")
            return jsonify({"error": "Bruger ikke fundet"}), 404
        ...
        if operation == "add":
            ...
            if new_balance < amount:
                log.warning(f"Admin '{current_user.id}' failed remove balance for '{original_user_key}'. Insufficient funds ({current_balance:.2f} < {amount:.2f})")
                return jsonify({"error": f"Utilstrækkelig saldo ({current_balance:.2f}). Kan ikke fjerne {amount:.2f}."}), 400
            new_balance -= amount
        elif operation == "set":
            new_balance = amount
        user_data["balance"] = round(new_balance, 2)
        if save_players(players_data):
            log.info(f"Admin '{current_user.id}' updated balance for '{original_user_key}'. "
            ...
            log.error(f"Admin '{current_user.id}': FAILED to save player data after balance update for {original_user_key}.")
            return jsonify({"error": "Kunne ikke gemme den opdaterede balance."}), 500
    except Exception as e:
        log.exception(f"Fejl i update_balance for '{username}', admin {current_user.id}: {e}")
...
def admin_update_session(session_id):
    ...
    log.error(f"Admin action skipped: Session update for '{session_id}' is disabled due to implementation limits.")
    return jsonify({"error": "Opdatering af sessioner er ikke implementeret på en sikker måde."}), 501
...
def apply_maintenance():
    ...
    if data is None or 'maintenance_mode' not in data:
        return jsonify({"error": "Manglende JSON data ('maintenance_mode' er påkrævet)"}), 400
    ...
    try:
        ...
        log.info(f"Admin '{current_user.id}' {status_text} maintenance mode. Message: '{message or 'Ingen'}'")
        # Audit Log (Note: This only changes app.config, not the persisted setting)
        add_audit_log(
            action="apply_maintenance",
            admin_user=current_user.id,
            details={"maintenance_mode": maintenance_mode, "message": message or "Ingen"}
        )
        return jsonify({
        ...
        log.exception(f"Error applying maintenance mode by admin {current_user.id}: {e}")
        return jsonify({"error": "Fejl ved opdatering af vedligeholdelsestilstand."}), 500
...
def generate_financial_report():
    ...
    try:
        players_data = load_players()
        total_balance = 0.0; total_staked = 0.0; total_won = 0.0; total_lost = 0.0
        valid_users = 0
        ...
        now_utc = datetime.now(timezone.utc)
        report = {
            "report_generated_at": now_utc.isoformat(timespec='seconds').replace('+00:00', 'Z'),
        ...
        log.info(f"Admin '{current_user.id}' generated financial report for {valid_users} users.")
        return jsonify({"report": report}), 200
    except Exception as e:
        log.exception(f"Error generating financial report for admin {current_user.id}: {e}")
...
def admin_get_system_settings():
    ...
    try:
        ...
        log.exception(f"Fejl i admin_get_system_settings for admin {current_user.id}: {e}")
        return jsonify({"error": "Intern serverfejl ved hentning af systemindstillinger."}), 500
...
def admin_update_system_settings():
            ...
            try:
                if expected_type == 'str':
                    ...
                    if isinstance(value, str):
                        updated_settings[key] = value.lower() in ['true', 'on', 'yes', '1']
                    else:
    ...
    min_bet = updated_settings.get('min_bet_amount', DEFAULT_SYSTEM_SETTINGS['min_bet_amount'])
    max_bet = updated_settings.get('max_bet_amount', DEFAULT_SYSTEM_SETTINGS['max_bet_amount'])
    if isinstance(min_bet, (int, float)) and isinstance(max_bet, (int, float)) and max_bet < min_bet:
        errors['max_bet_amount'] = "Max bet skal være større end eller lig med min bet."

    if errors:
        log.warning(f"Admin '{current_user.id}' system settings update failed validation: {errors}")
        return jsonify({"error": "Valideringsfejl", "details": errors}), 400

    try:
        if save_system_settings(updated_settings):
            log.info(f"Admin '{current_user.id}' successfully updated system settings. New settings: {updated_settings}")
            changed_details = {k: updated_settings[k] for k in updated_settings if k in data and updated_settings[k] != current_settings.get(k)}
            add_audit_log(
                action="update_system_settings",
                admin_user=current_user.id,
                details=changed_details or {"message": "No effective changes detected"}
            )
            return jsonify({"message": "Systemindstillinger blev gemt.", "settings": updated_settings}), 200
        else:
        ...
        log.exception(f"Unexpected error updating system_settings for admin '{current_user.id}': {e}")
        return jsonify({"error": "Intern serverfejl ved opdatering af systemindstillinger."}), 500
...
def admin_get_audit_logs():
    ...
    try:
        ...
        log.info(f"Admin '{current_user.id}' requested audit logs (limit: {limit}). Returning {len(logs)} entries.")
...
def admin_clear_cache():
    ...
    try:
        log.info(f"Admin '{current_user.id}' initiated cache clearing (Redis sessions).")
        ...
        if session_keys_to_delete:
            log.debug(f"Found {len(session_keys_to_delete)} Redis session keys to delete.")
        ...
        return jsonify({"message": f"Redis session cache ryddet. {cleared_count} nøgler blev fjernet."}), 200

    except redis.RedisError as e:
        log.exception(f"Redis error during cache clearing for admin {current_user.id}: {e}")
        errors_occurred = True
        # Ensure the error message is a string for jsonify
        return jsonify({"error": f"Redis fejl under cache rydning: {str(e)}"}), 500
    except Exception as e:
        log.exception(f"Unexpected error during cache clearing for admin {current_user.id}: {e}")
        errors_occurred = True
        ...
        if errors_occurred:
             add_audit_log(
                ...
                admin_user=current_user.id,
                details={"cache_type": "redis_sessions", "error": "An error occurred during clearing"}
             )
...
def admin_reindex_search():
    ...
    try:
        ...
        log.info(f"Successfully completed manual reindex triggered by admin '{current_user.id}'. Indexed {indexed_count} posts.")
        add_audit_log(
            action="reindex_search",
            admin_user=current_user.id,
            details={"indexed_count": indexed_count}
        )
        return jsonify({"message": f"Søgeindeks er blevet genopbygget. {indexed_count} posts indekseret."}), 200

    except Exception as e:
        log.exception(f"Error during manual search reindex for admin {current_user.id}: {e}")
        add_audit_log(
            action="reindex_search_failed",
            admin_user=current_user.id,
            details={"error": str(e)}
        )
        return jsonify({"error": f"Fejl under genopbygning af søgeindeks: {str(e)}"}), 500
...
def admin_get_scheduled_tasks():
   ...
   if scheduler is None or not hasattr(scheduler, 'get_jobs'):
       log.error("APScheduler (scheduler) is not available or not initialized correctly.")
       return jsonify({"error": "Scheduler service er ikke tilgængelig.", "tasks": []}), 503
   ...
   try:
       ...
       log.info(f"Admin '{current_user.id}' requested scheduled tasks. Returning {len(scheduled_tasks)} tasks.")
       return jsonify({"tasks": scheduled_tasks}), 200
   except Exception as e:
       log.exception(f"Fejl i admin_get_scheduled_tasks for admin {current_user.id}: {e}")
       return jsonify({"error": "Intern serverfejl ved hentning af planlagte opgaver.", "tasks": []}), 500
...
def admin_run_cleanup_job():
    ...
    try:
        ...
        invites_to_deactivate_stmt = db.select(InviteCode).where(
            ...
            (InviteCode.expires_at < now) | (InviteCode.uses_count >= InviteCode.max_uses)
        )
        ...
        if invites_to_deactivate:
            ...
            log.info(f"Cleanup job: Deactivating {len(invites_to_deactivate)} invite codes (IDs: {deactivated_ids}).")
        else:
        ...
        messages_to_delete_stmt = db.delete(PrivateMessage).where(
            PrivateMessage.sender_deleted == True,
            PrivateMessage.recipient_deleted == True,
        ...
        log.info(f"Cleanup job: Deleted {results['messages_deleted']} old soft-deleted private messages (older than {ninety_days_ago.date()}).")
        ...
        return jsonify({
            "message": "Oprydningsjob fuldført.",
            "details": results
        }), 200

    except sqlalchemy_exc.SQLAlchemyError as e:
        db.session.rollback()
        log.exception(f"Database error during cleanup job for admin {current_user.id}: {e}")
        add_audit_log(action="run_cleanup_job_failed", admin_user=current_user.id, details={"error": f"DB error: {str(e)}"})
        return jsonify({"error": f"Databasefejl under oprydning: {str(e)}"}), 500
    except Exception as e:
        db.session.rollback()
        log.exception(f"Unexpected error during cleanup job for admin {current_user.id}: {e}")
        add_audit_log(action="run_cleanup_job_failed", admin_user=current_user.id, details={"error": str(e)})
...
def check_server_health():
    ...
    try:
        ...
        current_app.logger.error(f"Health Check: Database connection failed: {e}", exc_info=True)
    ...
    if redis_client:
        try:
            ...
            current_app.logger.error(f"Health Check: Redis connection failed: {e}", exc_info=False)
            ...
            current_app.logger.error(f"Health Check: Redis check failed unexpectedly: {e}", exc_info=True)
    ...
    if api_key and isinstance(api_key, str) and len(api_key) > 5:
        ...
        current_app.logger.warning("Health Check: Sports API Key is missing or seems invalid.")
...
def admin_broadcast_message():
    ...
    try:
        log.info(f"Admin '{current_user.id}' is broadcasting message: '{message_text[:100]}...'")
...
def admin_get_failed_logins():
    ...
    try:
        ...
        attempts = FailedLoginAttempt.query.order_by(FailedLoginAttempt.timestamp.desc()).limit(limit).all()
        ...
        log.info(f"Admin '{current_user.id}' requested failed login attempts (limit: {limit}). Returning {len(attempts_data)} entries.")
        return jsonify({"failed_logins": attempts_data}), 200
    except Exception as e: # <<< Moved this except block here
        log.exception(f"Fejl i admin_get_failed_logins for admin {current_user.id}: {e}")
        return jsonify({"error": "Intern serverfejl ved hentning af mislykkede login forsøg."}), 500
...
def admin_get_forum_posts():
    ...
    try:
        ...
        posts_data = [{
            ...
            "body_snippet": (post.body[:100] + '...') if len(post.body) > 100 else post.body,
            "thread_title": post.thread_title,
            "thread_id": post.thread_id
        } for post in posts]

        log.info(f"Admin '{current_user.id}' requested latest forum posts (limit: {limit}). Returning {len(posts_data)} posts.")
...
def admin_delete_forum_post(post_id):
    ...
    try:
        ...
        if not post:
            log.warning(f"Admin '{current_user.id}' failed to delete post: ID {post_id} not found.")
        ...
        db.session.commit()
        log.info(f"Admin '{current_user.id}' successfully deleted forum post ID: {post_id}")
        ...
        db.session.rollback()
        log.exception(f"Error deleting forum post ID {post_id} for admin {current_user.id}: {e}")
...
def admin_get_password_reset_requests():
    ...
    try:
        # Import models needed
        from ..models import PasswordResetRequest, User as DBUser
        from sqlalchemy import select # Import select

        # Refactored query using db.select()
        # Pylance errors on select() arguments are likely type hint issues with SQLAlchemy
        stmt = select( # type: ignore[call-overload] Pylance struggles with select() overloads
        ...
        pending_requests = db.session.execute(stmt).all() # Execute the select statement
        ...
        log.info(f"Admin '{current_user.id}' requested pending password resets. Returning {len(requests_data)} requests.")
        return jsonify({"requests": requests_data}), 200

    except Exception as e:
        log.exception(f"Fejl i admin_get_password_reset_requests for admin {current_user.id}: {e}")
        return jsonify({"error": "Intern serverfejl ved hentning af password nulstillingsanmodninger."}), 500

@admin_bp.route("/api/password_reset_requests/<int:request_id>/generate_link", methods=["POST"])
@login_required
@utils.admin_required
def admin_generate_password_reset_link(request_id):
   ...
   Kræver admin.
   """
   log.info(f"Admin '{current_user.id}' attempting to generate reset link for request ID: {request_id}")
   try:
       # Import models needed
       from ..models import PasswordResetRequest, User as DBUser
       
       reset_request = db.session.get(PasswordResetRequest, request_id) # Use db.session.get for PK lookup

       if not reset_request:
           log.warning(f"Admin '{current_user.id}' failed: PasswordResetRequest ID {request_id} not found.")
           return jsonify({"error": "Password nulstillingsanmodning ikke fundet."}), 404

       if reset_request.status == 'used':
           log.warning(f"Admin '{current_user.id}' attempted to generate link for already used request ID {request_id}.")
           return jsonify({"error": "Denne anmodning er allerede blevet brugt."}), 409 # Conflict

       user = db.session.get(DBUser, reset_request.user_id)
       if not user:
           log.error(f"CRITICAL: User ID {reset_request.user_id} associated with PasswordResetRequest ID {request_id} not found in DB.")
           reset_request.status = 'error_user_not_found' # Mark as problematic
           db.session.commit()
           return jsonify({"error": "Tilknyttet bruger ikke fundet. Anmodningen kan ikke behandles."}), 500
       ...
       default_expiry_hours = current_app.config.get('PASSWORD_RESET_TOKEN_EXPIRES_HOURS', 1)
       expires_in_hours = default_expiry_hours

       if expires_in_hours_input is not None:
           try:
               candidate_hours = int(expires_in_hours_input)
               if candidate_hours > 0 and candidate_hours <= 72: # Max 3 days for example
                   expires_in_hours = candidate_hours
               else:
                   log.warning(f"Admin '{current_user.id}' provided invalid expires_in_hours '{expires_in_hours_input}' for request {request_id}. Using default {default_expiry_hours}h.")
           except ValueError:
               log.warning(f"Admin '{current_user.id}' provided non-integer expires_in_hours '{expires_in_hours_input}' for request {request_id}. Using default {default_expiry_hours}h.")
       ...
       db.session.commit()
       log.info(f"Admin '{current_user.id}' generated new reset token for request ID {request_id} (User: {user.username}). Token: {new_token[:8]}...")
       ...
       add_audit_log(
           ...
           details={"user_id": user.id, "username": user.username, "token_generated_at": now.isoformat()}
       )
       ...
       db.session.rollback()
       log.exception(f"Database error generating reset link for request ID {request_id} by admin {current_user.id}: {db_err}")
       return jsonify({"error": "Databasefejl under generering af link."}), 500
   except Exception as e:
       db.session.rollback()
       log.exception(f"Error generating reset link for request ID {request_id} by admin {current_user.id}: {e}")
...
def admin_reject_password_reset_request(request_id):
    ...
    Kræver admin.
    """
    log.info(f"Admin '{current_user.id}' attempting to reject password reset request ID: {request_id}")
    try:
        ...
        if not reset_request:
            log.warning(f"Admin '{current_user.id}' failed: PasswordResetRequest ID {request_id} not found for rejection.")
            return jsonify({"error": "Password nulstillingsanmodning ikke fundet."}), 404
        ...
        db.session.commit()
        log.info(f"Admin '{current_user.id}' successfully rejected password reset request ID: {request_id} for user_id: {user_id_for_log}")

        add_audit_log(
            action="reject_password_reset_request",
            admin_user=current_user.id,
            target_resource=f"ResetRequestID:{request_id}",
            details={"user_id": user_id_for_log, "username": username_for_log, "rejected_at": datetime.now(timezone.utc).isoformat()}
        )

        return jsonify({"message": f"Password nulstillingsanmodning #{request_id} blev afvist/slettet."}), 200

    except sqlalchemy_exc.SQLAlchemyError as db_err:
        db.session.rollback()
        log.exception(f"Database error rejecting reset request ID {request_id} by admin {current_user.id}: {db_err}")
        return jsonify({"error": "Databasefejl under afvisning af anmodning."}), 500
    except Exception as e:
        db.session.rollback()
        log.exception(f"Error rejecting reset request ID {request_id} by admin {current_user.id}: {e}")
