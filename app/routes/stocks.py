...
from flask import (
    Blueprint, request, jsonify, render_template, url_for, redirect, flash, abort, current_app # Samlet og tilføjet relevante
)
...
from ..extensions import db # <-- HØJST SANDSYNLIGT NØDVENDIG for at gemme/hente handler etc.
...
stocks_bp = Blueprint("stocks", __name__) # Definer uden prefix her, det sættes ved registrering
...
def buy_stock():
    ...
    try:
        ...
        if not data: return jsonify({"error": "Ugyldigt JSON format"}), 400
        ...
        stock_info["avg_price"] = new_total_cost / new_total_shares if new_total_shares else 0.0 # Undgå division med 0

        # --- Log Transaktion ---
        transaction = {
            "type": "buy", "symbol": symbol, "shares": shares, "price": current_price,
        ...
        if save_players(players):
            log.info(f"User '{user_key}' bought {shares} shares of {symbol} @ {current_price:.2f}.")
            return jsonify({
            ...
            log.error(f"Failed to save players data after stock purchase for {user_key}.")
...
def sell_stock():
    ...
    try:
        ...
        if not stock_info or current_shares < shares_to_sell:
            return jsonify({"error": f"Ikke nok aktier af {symbol} til salg ({current_shares} haves)."}), 400

        # Hent pris via lokalt API
        price_response = stock_data(symbol)
        price_data = price_response.get_json()
        if price_response.status_code != 200 or 'price' not in price_data:
             log.error(f"Failed to get stock price for {symbol} via internal API during sell.")
             return jsonify({"error": f"Kunne ikke verificere prisen for {symbol}"}), 500
        current_price = price_data['price']
        ...
        # --- Log Transaktion ---
        transaction = {
            "type": "sell", "symbol": symbol, "shares": shares_to_sell, "price": current_price,
            "total": total_gain, "timestamp": datetime.now().isoformat()
        }
        user_data.setdefault("stock_transactions", []).append(transaction)

        if save_players(players):
            log.info(f"User '{user_key}' sold {shares_to_sell} shares of {symbol} @ {current_price:.2f}.")
            return jsonify({
...
def portfolio_api():
    ...
    try:
        ...
        portfolio_data = current_user.portfolio if current_user.portfolio is not None else {}
        
        # Determine stock_balance: use user.settings['stock_balance'] if available, else user.balance
        stock_balance_setting = current_user.settings.get('stock_balance') if current_user.settings and isinstance(current_user.settings, dict) else None
        balance = stock_balance_setting if stock_balance_setting is not None else (current_user.balance if current_user.balance is not None else 0.0)
        
        transactions = current_user.stock_transactions if current_user.stock_transactions is not None else []
        # Ensure transactions is a list, and take the last 20
        if not isinstance(transactions, list):
            log.warning(f"User {username} stock_transactions is not a list (type: {type(transactions)}). Defaulting to empty list.")
            transactions = []
        ...
        portfolio_value = calculate_portfolio_value(portfolio_data, get_current_price_func=stock_data)
...
def aktiedyst_leaderboard_api():
    ...
    try:
        ...
        for username, user_data in players.items():
             ...
             if isinstance(user_data, dict) and not user_data.get('is_admin', False): # Exclude admin?
                # Genbrug beregning (kræver utils funktion er effektiv)
                # NB: Igen, dette kalder potentielt stock_data mange gange
                portfolio_value = calculate_portfolio_value(user_data, stock_data_func=stock_data)
                ...
                avatar_url = url_for('static', filename=f'avatars/{avatar_filename}') if avatar_filename else default_avatar_url
...
def aktiedyst_dashboard_page():
    ...
    try:
        ...
        transactions = user_data.get("stock_transactions", [])[-10:] # Kun de seneste 10
