...
import os
import bleach
from typing import Optional, List, Dict, Any, Set # Added Set, Any
import re
import uuid
...
from flask import (
    ...
    url_for, current_app, flash, abort, Response
)
...
from ..extensions import limiter, db, socketio, redis_client
from ..forms import UpdateAccountForm, ChangePasswordForm
from ..models import User, BetHistory, Notification, Badge, Sport, SportEvent, SportOutcome, ForumPost, ForumThread # Added ForumPost, ForumThread
from ..utils import (
...
def _parse_datetime_string(timestamp_str):
    ...
    for fmt in formats_aware:
        try: return datetime.strptime(timestamp_str, fmt).astimezone(timezone.utc)
        except (ValueError, TypeError): continue
    for fmt in formats_naive_assume_utc:
        try: return datetime.strptime(timestamp_str, fmt).replace(tzinfo=timezone.utc)
        except (ValueError, TypeError): continue
    log.warning(f"Could not parse timestamp string '{timestamp_str}'. Returning minimal aware datetime.")
    try: return datetime(max(MINYEAR, 1), 1, 1, tzinfo=timezone.utc)
    except ValueError: return datetime(1900, 1, 1, tzinfo=timezone.utc)
...
def index():
    ...
    try:
        ...
        try:
            ...
            log.exception(f"Failed to load fresh data for {user_id_lookup} in index view: {e}")
            flash("Kunne ikke indlæse dine aktuelle brugerdata.", "danger")
            return redirect(url_for('auth.login_route'))
        if not fresh_user_data:
             log.critical(f"Logged-in user '{user_id_lookup}' data not found during FRESH load in index!")
             flash("Kritisk brugerdata fejl. Du er blevet logget ud.", "danger")
             logout_user()
             return redirect(url_for('auth.login_route'))
        context = get_common_context()
        total_won = fresh_user_data.total_won if fresh_user_data.total_won is not None else 0.0
        total_lost = fresh_user_data.total_lost if fresh_user_data.total_lost is not None else 0.0
        context.update({
            ...
            "losses": fresh_user_data.losses if fresh_user_data.losses is not None else 0,
            "total_staked": fresh_user_data.total_staked if fresh_user_data.total_staked is not None else 0.0,
            "total_won": total_won, "total_lost": total_lost,
            "net": round(total_won - total_lost, 2),
            "largest_win": fresh_user_data.largest_win if fresh_user_data.largest_win is not None else 0.0,
            "largest_loss": fresh_user_data.largest_loss if fresh_user_data.largest_loss is not None else 0.0,
            "user_rank": fresh_user_data.rank if fresh_user_data.rank is not None else "N/A",
            "active_page": "home",
            "MAX_ABOUT_ME_LENGTH": current_app.config.get('MAX_ABOUT_ME_LENGTH', 500),
            "level": fresh_user_data.level if fresh_user_data.level is not None else 1,
            "balance": fresh_user_data.balance if fresh_user_data.balance is not None else 0.0,
        ...
        try:
            ...
            log.info(f"Prepared chart data ({len(chart_labels)} points) for user {current_user.id}")
        except Exception:
            log.exception(f"Failed to get/process chart data for user {current_user.id}")
            context['chart_labels_json'] = json.dumps([])
            context['chart_data_json'] = json.dumps([])
        return render_template("index.html", **context)
    except Exception:
        log.exception(f"Generic error rendering index for user {getattr(current_user, 'id', 'Unknown')}")
        abort(500)
...
def profile():
    ...
    if target_username_query:
        ...
        is_own_profile = (hasattr(current_user, 'username') and
                          isinstance(current_user.username, str) and
                          target_username_query.lower() == current_user.username.lower())
    else:
        target_username_for_lookup = current_user.username if hasattr(current_user, 'username') else str(current_user.id)
        is_own_profile = True
    log.info(f"Profile view request: Target='{target_username_for_lookup}', Viewer='{current_user.id}', Own={is_own_profile}")
    try:
        target_user_object = get_user_data_by_id(target_username_for_lookup)
        if not target_user_object:
            log.warning(f"Profile data for '{target_username_for_lookup}' not found in database.")
            flash("Brugerprofilen findes ikke.", "danger")
            return redirect(url_for('main.index'))
        actual_username = target_user_object.username
        try:
            total_bets_count = BetHistory.query.filter_by(user_id=target_user_object.id).count()
            ...
            log.debug(f"Fetched {total_bets_count} total bets and {len(recent_bets)} recent bets for user {actual_username}")
        except Exception as e_stats:
            log.exception(f"Error fetching bet stats/history for user {actual_username}: {e_stats}")
            total_bets_count = 0; recent_bets = []
            flash("Kunne ikke indlæse betting historik.", "warning")
        context = {}
        total_won = target_user_object.total_won if target_user_object.total_won is not None else 0.0
        total_lost = target_user_object.total_lost if target_user_object.total_lost is not None else 0.0
        xp_current = target_user_object.xp if target_user_object.xp is not None else 0
        xp_needed = target_user_object.xp_next_level if target_user_object.xp_next_level is not None else 1
        ...
        if not profile_avatar_url:
            default_avatar_rel_path = current_app.config.get('DEFAULT_AVATAR', 'avatars/default_avatar.png').lstrip('/').replace('static/', '', 1)
        ...
        if _badges_rel is not None:
            try: badges_iterable = list(_badges_rel) # type: ignore[assignment]
            except TypeError:
                 if hasattr(_badges_rel, 'all'): badges_iterable = _badges_rel.all() # type: ignore[assignment]
                 else: log.warning(f"Could not iterate or call .all() on badges for user {target_user_object.id}")
        badges_info = [{"id": b.id, "name": b.name, "icon": b.icon} for b in badges_iterable] # type: ignore[attr-defined]

        context.update({
            'profile_user': target_user_object, 'is_own_profile': is_own_profile,
            ...
            'profile_wins': target_user_object.wins if target_user_object.wins is not None else 0,
            'profile_losses': target_user_object.losses if target_user_object.losses is not None else 0,
            'profile_level': target_user_object.level if target_user_object.level is not None else 1,
            'profile_rank': target_user_object.rank or "N/A",
            'profile_total_staked': target_user_object.total_staked if target_user_object.total_staked is not None else 0.0,
            'profile_total_won': total_won, 'profile_total_lost': total_lost,
            'profile_net': round(total_won - total_lost, 2),
            'profile_largest_win': target_user_object.largest_win if target_user_object.largest_win is not None else 0.0,
            'profile_largest_loss': target_user_object.largest_loss if target_user_object.largest_loss is not None else 0.0,
            'about_me_text': target_user_object.about_me or "", 'badges': badges_info,
            'xp_current': xp_current, 'xp_needed': xp_needed, 'xp_percentage': xp_percentage,
            'profile_avatar_url': profile_avatar_url, 'active_page': "profile",
            'total_bets_placed': total_bets_count, 'recent_bet_history': recent_bets,
            'profile_email': target_user_object.email if is_own_profile else None,
            'profile_user_balance': target_user_object.balance if is_own_profile and target_user_object.balance is not None else None,
        ...
        log.exception(f"Unexpected error rendering profile for target '{target_username_for_lookup}', viewed by '{current_user.id}'. Error: {e}")
...
def search():
    ...
    try:
        ...
        if search_performed:
            ...
            matched_users = User.query.filter(User.username.ilike(f"%{query_search}%")).order_by(User.username).limit(search_limit).all()
            log.info(f"Database search for '{query_display}' yielded {len(matched_users)} results (limit: {search_limit}).")
        users_for_display = {}
        if matched_users:
            for user_obj in matched_users:
                formatted_join_date = dt_filter_func(user_obj.registration_date) if user_obj.registration_date else 'Ukendt'
                users_for_display[user_obj.username] = {
                    'username': user_obj.username, 'avatar_url': user_obj.avatar_url,
                    'level': user_obj.level if user_obj.level is not None else 1,
                    'rank': user_obj.rank or 'N/A', 'join_date': formatted_join_date,
                    'profile_url': url_for('main.profile', username=user_obj.username)
                }
        context.update({
            "query": query_display, "matched_users_data": users_for_display,
            "search_performed": search_performed, "active_page": "search"
        })
        return render_template("search_results.html", **context)
    except Exception:
         log.exception(f"Error in search for query '{query_display}', user '{current_user.id}'")
...
def upload_avatar():
    ...
    save_path_abs = None; new_safe_filename = None; old_avatar_fullpath_to_delete = None
    avatar_save_dir = None; user_obj = None
    try:
        original_filename = secure_filename(file.filename or "")
        allowed_extensions = current_app.config.get('ALLOWED_AVATAR_EXTENSIONS', {'png', 'jpg', 'jpeg', 'gif'})
        if '.' not in original_filename or original_filename.rsplit('.', 1)[1].lower() not in allowed_extensions:
            return jsonify({"error": f"Ugyldig filtype. Tilladte: {', '.join(allowed_extensions)}"}), 400
        max_size_mb = current_app.config.get('MAX_CONTENT_LENGTH', 3 * 1024 * 1024) / (1024*1024)
        max_size_bytes = int(max_size_mb * 1024 * 1024)
        file.seek(0, os.SEEK_END); file_length = file.tell(); file.seek(0)
        if file_length > max_size_bytes:
            return jsonify({"error": f"Filen er for stor (max {max_size_mb:.1f}MB)."}), 413
        ...
        if isinstance(current_user.uid, int):
            new_filename_base = f"{secure_filename(current_user.username)}_{current_user.uid}"
        else:
        ...
        try:
            ...
            if not user_obj: return jsonify({"error": "Bruger ikke fundet."}), 404
            old_avatar_filename = user_obj.avatar
            default_avatar_filename = os.path.basename(current_app.config.get('DEFAULT_AVATAR', 'avatars/default_avatar.png'))
            if old_avatar_filename and old_avatar_filename != default_avatar_filename:
                old_avatar_fullpath_to_delete = os.path.join(avatar_save_dir, old_avatar_filename)
        except Exception as e_old: log.exception(f"Error old avatar path: {e_old}")
        try: file.save(save_path_abs)
        except Exception: log.exception("Failed to save avatar"); return jsonify({"error": "Fejl ved gemning."}), 500
        if not user_obj: return jsonify({"error": "Intern serverfejl."}), 500
        user_obj.avatar = new_safe_filename
        try: db.session.commit()
        except Exception:
            db.session.rollback(); log.exception("DB commit failed for avatar")
            if os.path.exists(save_path_abs): os.remove(save_path_abs)
            return jsonify({"error": "Databasefejl ved avatar."}), 500
        else:
            if hasattr(current_user, 'avatar'): current_user.avatar = new_safe_filename
            if old_avatar_fullpath_to_delete and os.path.isfile(old_avatar_fullpath_to_delete):
                 try: os.remove(old_avatar_fullpath_to_delete)
                 except OSError as e_del: log.error(f"Error deleting old avatar: {e_del}")
            new_avatar_url_response = user_obj.avatar_url or url_for('static', filename=f"{avatar_subdir}/{new_safe_filename}")
            return jsonify({"message": "Avatar opdateret!", "new_avatar_url": new_avatar_url_response}), 200
...
def leaderboard():
    ...
    try:
        context = get_common_context(); context["active_page"] = "leaderboard"
        return render_template("leaderboard.html", **context)
    except Exception: log.exception("Error rendering leaderboard"); abort(500)
...
def api_leaderboard():
    ...
    try:
        ...
        query = User.query
        if sort_by == 'win_rate':
            query = query.order_by(db.case(((User.wins + User.losses) > 0, User.wins / (User.wins + User.losses)), else_=-1).desc(), User.balance.desc())
        elif sort_by == 'profit':
            query = query.order_by((User.total_won - User.total_lost).desc(), User.balance.desc())
        else: sort_by = 'balance'; query = query.order_by(User.balance.desc())
        users_query = query.limit(leaderboard_limit).all()
        if not users_query: return jsonify([])
        ranked_data = []
        for i, user_obj in enumerate(users_query):
            profile_url = url_for('main.profile', username=user_obj.username)
            wins = int(user_obj.wins or 0); losses = int(user_obj.losses or 0)
            total_bets = wins + losses
            win_rate = round((wins / total_bets) * 100, 1) if total_bets > 0 else 0.0
            net_profit = round((user_obj.total_won or 0.0) - (user_obj.total_lost or 0.0), 2)
            ranked_data.append({
                'rank': i + 1, 'username': user_obj.username,
                'balance': float(user_obj.balance or 0.0),
                'wins': wins, 'losses': losses, 'win_rate': win_rate, 'net_profit': net_profit,
                'level': int(user_obj.level or 1), 'rank_display': user_obj.rank or 'N/A',
                'avatar_url': user_obj.avatar_url, 'profile_url': profile_url,
                'largest_win': float(user_obj.largest_win or 0.0)
            })
        return jsonify({"leaderboard": ranked_data, "sorted_by": sort_by})
    except Exception as e:
        log.exception("Error generating leaderboard API"); return jsonify({"error": "Server error"}), 500
...
def api_auth_session_check():
    ...
    user_data = {
        ...
        "avatar_url": getattr(current_user, 'avatar_url', None) # Uses the model's property
    }
...
def get_user_stats_api(username) -> ResponseReturnValue: # Use ResponseReturnValue
    log.info(f"API: Fetching stats for user '{username}' requested by '{getattr(current_user, 'username', 'N/A')}'")
    target_user = User.query.filter(func.lower(User.username) == func.lower(username)).first()
    if not target_user:
    ...
    last_login_relative = dt_filter_func(target_user.last_login, relative=True) if target_user.last_login else 'Aldrig set'
    ...
    stats = {
        ...
        "total_staked": target_user.total_staked if target_user.total_staked is not None else 0.0,
        "total_won": total_won,
        "total_lost": total_lost,
        "net_profit_loss": round(total_won - total_lost, 2),
        "largest_win": target_user.largest_win if target_user.largest_win is not None else 0.0,
        "largest_loss": target_user.largest_loss if target_user.largest_loss is not None else 0.0,
...
@login_required
def get_user_recent_activity(username) -> ResponseReturnValue: # Use ResponseReturnValue
    log.info(f"API: Fetching recent activity for user '{username}' requested by '{getattr(current_user, 'username', 'N/A')}'")
    ...
    if not is_own_profile and not target_user.privacy_show_activity:
        ...
        log.info(f"User {target_user.id} has set their activity to private. Denying access to {getattr(current_user, 'id', 'Anonymous')}.")
    ...
    try:
        ...
        recent_bets = BetHistory.query.filter_by(user_id=target_user.id)\
        ...
        try:
            recent_posts = ForumPost.query.filter_by(author_username=target_user.username)\
            ...
            for post in recent_posts:
                ...
                post_url = url_for('forum.view_thread', thread_id=post.thread_id, _external=False) + f'#post-{post.id}' if post.thread_id and post.id else '#'
        ...
        activities.sort(key=lambda x: x.get('timestamp') or '0001-01-01T00:00:00+00:00', reverse=True)

        return jsonify({"activities": activities[:limit]}) # Return combined and limited list
...
def history():
    ...
    try:
        ...
        aggregated_list = [{
            "username": u.username, "balance": u.balance or 0.0, "wins": u.wins or 0,
            "losses": u.losses or 0, "total_won": u.total_won or 0.0, "total_lost": u.total_lost or 0.0,
            "last_login": u.last_login.isoformat() if u.last_login else None,
            "level": u.level or 1, 'profile_url': url_for('main.profile', username=u.username)
        ...
        detailed_hist_fmt = [{
            'id': b.id, 'username': bu, 'match_name': b.match_name, 'outcome_name': b.outcome_name,
            'stake': b.stake, 'payout': b.payout, 'result': b.result, 'status': b.status,
            'timestamp': b.timestamp.isoformat(), 'parsed_timestamp': b.timestamp
        } for b, bu in detailed_hist_db]
        context.update({
            "aggregated_data": aggregated_list, "detailed_history": detailed_hist_fmt[:HISTORY_LIMIT],
            "active_page": "history"
        })
        return render_template("history.html", **context)
    except Exception: log.exception("Error history page"); abort(500)
...
def render_simple_page(template_name, page_key):
    ...
    except Exception: log.exception(f"Error rendering {template_name}"); abort(500)

# --- Simple Page Route Implementations ---
@main_bp.route("/game_area")
@login_required
def game_area_page(): return render_simple_page("game_area.html", "game_area")

@main_bp.route("/live_sports")
@login_required
def live_sports_page(): return render_simple_page("live_sports.html", "live_sports")

@main_bp.route("/join_game")
@login_required
def join_game_page(): return render_simple_page("join_game.html", "join_game")
...
def aktiedyst():
    ...
    try:
        ...
        if not isinstance(portfolio, dict): portfolio = {}
        transactions = current_user.stock_transactions or []
        if not isinstance(transactions, list): transactions = []
        stock_bal_set = current_user.settings.get('stock_balance') if current_user.settings and isinstance(current_user.settings, dict) else None
        actual_stock_bal = stock_bal_set if stock_bal_set is not None else (current_user.balance or 0.0)
        context.update({
            "portfolio_value": calculate_portfolio_value(portfolio), "portfolio": portfolio,
            "transactions": sorted(transactions, key=lambda x: _parse_datetime_string(x.get('timestamp')) or datetime(MINYEAR,1,1,tzinfo=timezone.utc), reverse=True)[:20],
            "stock_balance": actual_stock_bal, "active_page": "aktiedyst"
        })
        return render_template("aktiedyst.html", **context)
    except Exception: log.exception("Error aktiedyst"); abort(500)
...
def settings():
    ...
    account_form = UpdateAccountForm(obj=current_user) # Pre-populate with current user's data
    ...
    if request.method == 'POST':
        ...
        if 'email' in request.form or 'privacy_profile_public' in request.form: # Check for a field from account_form
            if account_form.validate_on_submit():
                log.info(f"Attempting to update account settings for user {current_user.id}")
                try:
                    ...
                    if account_form.about_me.data:
                        ...
                        user_to_update.about_me_last_updated = datetime.now(timezone.utc)
                    else:
                        user_to_update.about_me = None # Clear if submitted empty
                        user_to_update.about_me_last_updated = datetime.now(timezone.utc)

                    # Update privacy settings
                    user_to_update.privacy_profile_public = account_form.privacy_profile_public.data
                    user_to_update.privacy_show_activity = account_form.privacy_show_activity.data
                    user_to_update.privacy_show_bet_history = account_form.privacy_show_bet_history.data
                    user_to_update.privacy_show_online_status = account_form.privacy_show_online_status.data
                    
                    db.session.commit()
                    flash('Dine kontoindstillinger er blevet opdateret.', 'success')
                    log.info(f"Account and privacy settings updated for user {current_user.id}")
                    return redirect(url_for('main.settings'))
                except Exception as e:
                    db.session.rollback()
                    log.exception(f"Error updating account/privacy settings for user {current_user.id}: {e}")
                    flash('Der opstod en fejl under opdatering af dine indstillinger.', 'danger')
            else:
                log.warning(f"UpdateAccountForm validation failed for user {current_user.id}: {account_form.errors}")
    ...
    if not fresh_user_data:
        log.critical(f"Fresh user data not found for settings page! User: {current_user.id}")
        flash("Kritisk brugerdata fejl. Du er blevet logget ud.", "danger")
        logout_user()
        return redirect(url_for('auth.login_route'))
    ...
    account_form.privacy_show_bet_history.data = fresh_user_data.privacy_show_bet_history
    account_form.privacy_show_online_status.data = fresh_user_data.privacy_show_online_status
    
    context = get_common_context()
    avatar_url = getattr(fresh_user_data, 'avatar_url', None) or url_for('static', filename=Config.DEFAULT_AVATAR)
...
def odds_api():
    ...
    if not sport_key: return jsonify({"error": "'sport' param missing."}), 400
    ...
    params = {'apiKey': api_key, 'regions': request.args.get("regions", "eu").strip(),
              'markets': request.args.get("markets", "h2h").strip(),
              'dateFormat': 'iso', 'oddsFormat': 'decimal'}
    try:
        response = requests.get(endpoint_url, params=params, timeout=timeout, headers={'User-Agent': 'Fattecentralen-Proxy/1.0'})
        response_data = response.json() if response.content else None
        if not response.ok:
            error_detail = response_data.get('message', response.text[:200]) if response_data and isinstance(response_data, dict) else response.text[:200]
            log.error(f"Odds API V4 HTTP Error ({response.status_code}): {error_detail}")
            return jsonify({"error": f"Odds service error ({response.status_code})."}), response.status_code if 400 <= response.status_code < 500 else 502
        if isinstance(response_data, list): return jsonify(response_data)
        else: return jsonify({"error": "Unexpected data from Odds service."}), 502
    except requests.exceptions.Timeout: return jsonify({"error": "Timeout Odds service."}), 504
    except requests.exceptions.RequestException: return jsonify({"error": "Network error Odds service."}), 502
    except Exception: log.exception("Odds API proxy error"); return jsonify({"error": "Server error odds."}), 500
...
def live_score(sportKey, eventId):
    ...
    try:
        ...
        status = "In Progress"; minute_disp = min(curr_min, 90)
        if curr_min >= 105 + (base_seed % 10): status = "Ended"; minute_disp = None
        elif 90 <= curr_min < 105 + (base_seed % 10) and (base_seed % 6 == 0): minute_disp = f"90+{curr_min - 90}"
        elif 45 <= curr_min < 60 and (base_seed % 7 == 0):
            status = "Halftime"; home = max(0, home - (base_seed % 2)); away = max(0, away - (base_seed % 3)); minute_disp = None
        elif curr_min == 0: status = "Scheduled"; home = 0; away = 0; minute_disp = None
        return jsonify({"status": status, "home_score": home, "away_score": away, "score": f"{home}-{away}",
                        "minute": minute_disp, "completed": status == "Ended", "last_updated": now.isoformat()})
    except Exception: log.exception("Error dummy live_score"); return jsonify({"error": "Server error livescore"}), 500
...
def sports_catalog():
    ...
    base_url_raw = current_app.config.get('SPORTS_API_BASE_URL')
    timeout = current_app.config.get('EXTERNAL_API_TIMEOUT', 10)
    if not api_key or not base_url_raw: return jsonify({"error": "Sports API not configured."}), 503
    base_url = clean_base_url(base_url_raw)
    catalog_url = f"{base_url}/v4/sports"; params = {"apiKey": api_key}
    try:
        response = requests.get(catalog_url, params=params, timeout=timeout, headers={'User-Agent': 'Fattecentralen-Proxy/1.0'})
        api_catalog_raw = response.json() if response.content else None
        if not response.ok:
            error_detail = api_catalog_raw.get('message', response.text[:200]) if api_catalog_raw and isinstance(api_catalog_raw, dict) else response.text[:200]
            return jsonify({"error": f"Catalog service error ({response.status_code})."}), response.status_code if 400 <= response.status_code < 500 else 502
        if not isinstance(api_catalog_raw, list): return jsonify({"error": "Unexpected data from Catalog API."}), 502
        catalog = [{"key": s['key'], "title": s.get('title', s['key'].replace('_', ' ').title()),
                    "group": s.get('group', 'Øvrige').title()} for s in api_catalog_raw if isinstance(s, dict) and s.get('key') and s.get('active')]
        return jsonify(sorted(catalog, key=lambda x: (x['group'].lower(), x['title'].lower())))
    except requests.exceptions.Timeout: return jsonify({"error": "Timeout Catalog service."}), 504
    except requests.exceptions.RequestException: return jsonify({"error": "Network error Catalog service."}), 502
    except Exception: log.exception("Error sports_catalog"); return jsonify({"error": "Server error catalog."}), 500
...
def get_open_sessions_and_invites(): # Route is synchronous
    ...
    try:
        ...
        log.debug(f"Found {len(session_ids)} session IDs in set {user_sessions_key} for user {user_id_str}")
        ...
        if session_ids:
            ...
            for i, session_key in enumerate(session_keys):
                pipe.hmget(session_key, ["session_id", "session_name", "status", "created_at"])
            ...
            for i in range(0, len(results), chunk_size):
                ...
                if session_details_values and isinstance(session_details_values, list) and len(session_details_values) >= 4 and session_details_values[0] is not None:
                    session_id = session_details_values[0]
                    session_name = session_details_values[1] or session_id # Use session_id as fallback
                    status = session_details_values[2] or "unknown"
                    created_at = session_details_values[3]
                    if status in ['open', 'in_progress']:
                        items.append({
                            "id": session_id, "type": "session", "title": session_name, "status": status,
                            "details": f"{player_count if isinstance(player_count, int) else '?'} spillere",
                            "timestamp": created_at, "icon": "bi-controller",
                            "iconColor": "text-success" if status == 'open' else "text-primary",
                            "url": url_for('sessions.session_detail_page', session_id=session_id)
                        })
                else:
                     idx = i // chunk_size
                     if idx < len(session_ids_list):
                          log.warning(f"Dashboard API: Session details for {session_ids_list[idx]} not found or incomplete. Details: {session_details_values}")
        # Dummy invites for testing UI - can be removed later
        dummy_invites = [
             {"id": "i1", "type": "invite", "title": "Duel Invitation", "details": "Fra: Kong Hans", "timestamp": (datetime.now(timezone.utc) - timedelta(minutes=15)).isoformat(),
              "icon": "bi-envelope-paper-heart", "iconColor": "text-info", "url": "#invite-i1"},
             {"id": "s_aktiedyst3", "type": "session", "title": "Aktiedyst #3", "status": "in_progress", "details": "Dag 2/7", "timestamp": (datetime.now(timezone.utc) - timedelta(hours=26)).isoformat(), "icon": "bi-graph-up-arrow", "iconColor": "text-primary", "url": "#"}
        ]
        items.extend(dummy_invites)
        items.sort(key=lambda x: x.get('timestamp', '0') or '0', reverse=True)
        return jsonify(items[:limit])
    except Exception as e:
        log.exception(f"Error fetching open sessions/invites for {current_user.id}: {e}")
        return jsonify([])
...
def badge_showcase_page():
    ...
    try:
        ...
        context = get_common_context()
        context.update({'title': 'Badge Oversigt', 'badges': all_badges, 'active_page': 'badges'})
        return render_template('badges/showcase.html', **context)
    except Exception as e:
        log.exception(f"Error badge showcase: {e}"); flash("Error loading badges.", "danger")
        return redirect(url_for('main.index'))
...
def get_bettable_events():
    ...
    try:
        ...
        events_query = db.session.query(SportEvent).join(Sport).filter(
            Sport.active == True, # type: ignore[arg-type]
            SportEvent.commence_time > now, # type: ignore[arg-type]
            SportEvent.commence_time < upcoming_threshold # type: ignore[arg-type]
        ).options(db.joinedload(SportEvent.sport)).order_by(SportEvent.commence_time).all() # type: ignore[attr-defined]
        sports_events_dict = {}
        for event in events_query:
            sport_key = event.sport.key # type: ignore[attr-defined]
            if sport_key not in sports_events_dict:
                sports_events_dict[sport_key] = {"title": event.sport.title, "group": event.sport.group, "events": []} # type: ignore[attr-defined]
            outcomes_iterable = event.outcomes # type: ignore[attr-defined]
            if not hasattr(outcomes_iterable, '__iter__') and hasattr(outcomes_iterable, 'all'):
            ...
            outcomes_list = [{"id": o.id, "name": o.name, "price": o.price, "bookmaker": o.bookmaker, "market": o.market_key}
                             for o in (outcomes_iterable or []) if o.market_key == 'h2h'] # type: ignore[attr-defined]
            if outcomes_list:
                sports_events_dict[sport_key]["events"].append({
                    "id": event.id, "home_team": event.home_team, "away_team": event.away_team,
                    "commence_time": event.commence_time.isoformat(), "outcomes": outcomes_list
                })
...
def all_notifications_page():
    ...
    try:
        ...
        if ids_to_mark_read:
            try:
                Notification.query.filter(Notification.id.in_(ids_to_mark_read), Notification.user_id == current_user.id)\
                    .update({'is_read': True}, synchronize_session=False)
                db.session.commit()
                log.info(f"Marked {len(ids_to_mark_read)} notifications read for {current_user.id}.")
                socketio.emit('notifications_marked_read', {'ids': ids_to_mark_read}, room=f'user_{current_user.id}') # type: ignore[attr-defined]
            except Exception as mark_err:
                db.session.rollback(); log.error(f"Error marking notifications read: {mark_err}")
        context = get_common_context()
        context.update({'title': 'Alle Notifikationer', 'notifications': notifications_on_page,
                        'pagination': pagination, 'active_page': 'notifications_all'})
        return render_template('notifications/index.html', **context)
    except Exception as e:
        log.exception(f"Error loading notifications page: {e}"); flash("Error loading notifications.", "danger")
        return redirect(url_for('main.index'))
...
def get_notifications_api(): # Renamed
    try:
        ...
        notifications_list = [{"id": n.id, "message": n.message, "link": n.link, "icon": n.icon,
                               "category": n.category, "created_at": n.created_at.isoformat(), "is_read": n.is_read}
                              for n in recent_notifications]
        return jsonify({"notifications": notifications_list, "unread_total": unread_total})
    except Exception as e:
...
def mark_notifications_read_api(): # Renamed
    ...
    try:
        notification_ids = data.get('ids'); mark_all = data.get('all', False); updated_count = 0
        if mark_all:
            ...
            updated_count = Notification.query.filter(
                Notification.id.in_(notification_ids), Notification.user_id == current_user.id,
                Notification.is_read == False # type: ignore[attr-defined]
            ).update({'is_read': True}, synchronize_session=False)
        else: return jsonify({"error": "No IDs or 'all' flag."}), 400
        db.session.commit()
        log.info(f"API: Marked {updated_count} notifications read for {current_user.id}.")
        if socketio:
        ...
        return jsonify({"success": True, "message": f"{updated_count} notifications marked read."})
    except Exception as e:
        db.session.rollback(); log.exception(f"API Error marking notifications read: {e}")
...
def get_user_bet_history_api(username: str) -> ResponseReturnValue:
    log.info(f"API: Fetching bet history for user '{username}' requested by '{getattr(current_user, 'username', 'N/A')}'")
    ...
    if not is_own_profile and not target_user.privacy_show_bet_history:
        log.info(f"User {target_user.id} has set their bet history to private. Denying access to {getattr(current_user, 'id', 'Anonymous')}.")
        return jsonify({"error": "This user's bet history is private."}), 403

    # Allow any authenticated user to fetch if public or their own
    if not current_user.is_authenticated: # Should be caught by @login_required, but defensive
    ...
    try:
        page = request.args.get('page', 1, type=int)
        per_page = request.args.get('per_page', 15, type=int) # Default to 15, matching JS
        ...
        if start_date_str:
            try:
                start_date = datetime.fromisoformat(start_date_str.split('T')[0] + 'T00:00:00').replace(tzinfo=timezone.utc)
        ...
        if end_date_str:
            try:
                end_date = datetime.fromisoformat(end_date_str.split('T')[0] + 'T23:59:59.999999').replace(tzinfo=timezone.utc)
...
def get_user_balance_history_api(username: str) -> ResponseReturnValue:
    log.info(f"API: Fetching balance history for user '{username}' requested by '{getattr(current_user, 'username', 'N/A')}'")
    ...
    try:
        ...
        chart_labels, chart_data = get_balance_history(target_user.id, days=days_to_fetch)
        
        log.info(f"API: Successfully fetched {len(chart_labels)} points of balance history for '{username}' over {days_to_fetch} days.")
        return jsonify({
            "username": target_user.username,
