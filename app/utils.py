...
from .models import User as DBUser, PrivateMessage, Sport, SportEvent, SportOutcome, Notification, Badge # Added Notification, Badge
...
def update_sports_catalog_from_api():
    ...
    try:
        ...
        existing_sports_db = {sport.key: sport for sport in Sport.query.all()}
        processed_keys = set()
        ...
        for sport_api in sports_data: 
            ...
            if existing_sport:
                if (existing_sport.title != title or
                        existing_sport.group != group or
                        not existing_sport.active):
                    existing_sport.title = title
                    existing_sport.group = group
                    existing_sport.active = True 
                    existing_sport.last_checked = datetime.now(timezone.utc)
                    db.session.add(existing_sport)
                    updated_count += 1
                    log.debug(f"Updating sport: {key}")
                else:
                    existing_sport.last_checked = datetime.now(timezone.utc)
                    db.session.add(existing_sport)
            else:
                new_sport = Sport( # type: ignore[call-arg]
                    key=key, title=title, group=group, active=True,
        ...
        for key, sport_db in existing_sports_db.items():
            if key not in processed_keys and sport_db.active:
                ...
                sport_db.last_checked = datetime.now(timezone.utc)
...
def update_sport_events():
    ...
    try:
        active_sports = Sport.query.filter_by(active=True).all()
        ...
        for sport in active_sports:
            if '_winner' in sport.key or 'politics' in sport.key:
                markets_to_fetch = 'outrights'
            else:
                markets_to_fetch = 'h2h'
            
            log.debug(f"Fetching events for sport: {sport.key} ({sport.title}) with market: {markets_to_fetch}")
            events_url = f"{base_url}/v4/sports/{sport.key}/odds"
            params = {
            ...
            try:
                ...
                if not isinstance(events_data, list):
                    log.warning(f"Received non-list data for sport {sport.key} from {events_url}. Skipping.")
                    api_errors += 1
                    continue

                log.debug(f"Received {len(events_data)} events for {sport.key}.")
                ...
                for event_api in events_data:
                    ...
                    home_team = event_api.get('home_team')
                    away_team = event_api.get('away_team')

                    if not event_id or not commence_time_str:
                        log.warning(f"Skipping event with missing ID or commence_time for sport {sport.key}: {event_api}")
                        continue
                    ...
                    if not event_db:
                        event_db = SportEvent( # type: ignore[call-arg]
                            id=event_id, sport_key=sport.key, commence_time=commence_time_aware,
                            home_team=home_team, away_team=away_team
                        )
                        db.session.add(event_db)
                        log.debug(f"Creating new SportEvent: {event_id} ({home_team} vs {away_team})")
                    else:
                        if event_db.commence_time != commence_time_aware or event_db.home_team != home_team or event_db.away_team != away_team:
                             event_db.commence_time = commence_time_aware
                             event_db.home_team = home_team
                             event_db.away_team = away_team
                             log.debug(f"Updating existing SportEvent: {event_id}")
                        event_db.last_update_api = datetime.now(timezone.utc)
                        db.session.add(event_db)
                ...
                db.session.commit() 
                log.debug(f"Committed changes for sport {sport.key}")
            except requests.exceptions.RequestException as req_err:
                log.error(f"Failed to fetch events for sport {sport.key} from {events_url}: {req_err}")
                api_errors += 1
                db.session.rollback()
            except json.JSONDecodeError as json_err:
                log.error(f"Failed to decode JSON for sport {sport.key} from {events_url}: {json_err}")
                api_errors += 1
                db.session.rollback()
            except Exception as e:
                log.exception(f"Unexpected error processing sport {sport.key}: {e}")
                api_errors += 1
                db.session.rollback()
