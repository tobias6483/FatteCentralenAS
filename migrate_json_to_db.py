...
try:
    ...
    from app.extensions import bcrypt, db  # Import db and bcrypt from extensions
    from app.models import Badge, BetHistory
    from app.models import (
        User as DbUser,  # The new SQLAlchemy User model + Badge & BetHistory
    )
except ImportError as e:
    print(
        f"Error importing Flask app components. Make sure this script is in the project root or adjust Python path."
    )
    print(f"Details: {e}")
    exit(1)
    ...
    with app.app_context():
        ...
        for username_key, data in players_data.items():
            ...
            if existing_user:
                ...
                if existing_user_by_email:
                    log.warning(
                        f"Email '{email}' already exists for user '{existing_user_by_email.username}'. Skipping creation of new user '{username}'."
                    )
                    users_skipped += 1
                    continue

                if (
                    not username or not email
                ):  # Should be caught by earlier username/email definition, but good check
                    log.warning(
                        f"Skipping entry due to missing username or email. Original key: {username_key}, Data: {data}"
                    )
                    users_skipped += 1
                    continue
                ...
                if not password_hash_json:
                    log.warning(
                        f"User '{username}' has no password_hash in JSON. Skipping creation."
                    )
                    users_skipped += 1
                    continue
                ...
                registration_date_dt = parse_datetime_from_json(
                ...
                if isinstance(uid_json, int):
                    uid_db = str(uid_json)
                elif isinstance(uid_json, str):
                    uid_db = uid_json
                else:
                    uid_db = str(uuid.uuid4())
                ...
                target_user = DbUser(
                    ...
                    backup_codes=(
                        json.dumps(data.get("backup_codes", []))
                    ...
                    largest_win=(
                        float(data.get("largest_win", 0.0))
                        if data.get("largest_win") is not None
                        else None
                    ),
                    largest_loss=(
                        float(data.get("largest_loss", 0.0))
                        if data.get("largest_loss") is not None
                        else None
                    ),
            ...
            if json_badges:
                for badge_data in json_badges:
                    ...
                    if not badge_name:
                        log.warning(
                            f"Skipping badge with no name for user '{username}'. Data: {badge_data}"
                        )
                        continue
                    ...
                    if badge_obj not in target_user.badges:
                        ...
                        log.debug(
                            f"Associated badge '{badge_name}' with user '{username}'"
                        )
            ...
            if json_bet_history:
                for bet_data in json_bet_history:
                    ...
                    bet_timestamp_dt = parse_datetime_from_json(
                        bet_timestamp_str
                    ) or datetime.now(timezone.utc)
                    ...
                    if existing_bet_entry:
                        log.debug(
                            f"Bet history entry for match '{bet_data.get('match_name')}' at '{bet_timestamp_dt}' already exists for user '{username}'. Skipping."
                        )
                        continue
                    ...
                    log.debug(
                        f"Added bet history entry for match '{bet_data.get('match_name')}' for user '{username}'"
                    )

            if is_new_user_flag:
                users_created += 1
                log.info(
                    f"Prepared new user '{username}' with badges and bet history for migration."
                )
            else:
                users_updated += 1  # User was existing, now updated
                log.info(
                    f"Prepared existing user '{username}' for update with badges and bet history."
                )
        ...
        if users_created > 0 or users_updated > 0:
            try:
                db.session.commit()
                log.info(
                    f"Successfully created {users_created} users and updated {users_updated} users in the database."
                )
            except Exception as e:
                db.session.rollback()
