...
from typing import Any, Dict, List, Optional, Set, Tuple, Union  # <<< Union tilføjet
...
try:
    ...
    logging.critical(
        "FATAL: Kan ikke importere Config fra .config. Data adgangsfunktioner kan ikke bestemme filstier!"
    )
    ...
    AUDIT_LOG_FILE_PATH = (
        "/path/to/instance/audit_log.json_ERROR"  # <<< TILFØJET placeholder
    )
...
log = logging.getLogger(
    __name__
)  # Omdøbt fra 'logger' for at undgå navnekonflikt med logging modul
...
def _load_json_file(file_path: str, default_value: Any = None) -> Any:
    ...
    if not file_path or "_ERROR" in file_path:  # Tjek for tom/ugyldig sti
        log.error(
            f"_load_json_file: Invalid or unconfigured file path: '{file_path}'. Returning default ({type(resolved_default).__name__})."
        )
    ...
    try:
        ...
        if not os.path.exists(file_path):
            log.warning(
                f"File not found: {file_path}. Returning default data ({type(resolved_default).__name__})."
            )
            return resolved_default
        # Undersøg filstørrelse - returner default hvis tom
        if os.path.getsize(file_path) == 0:
            log.warning(
                f"File is empty: {file_path}. Returning default data ({type(resolved_default).__name__})."
            )
            # Hvis default_value er [] eller {} er det okay, ellers log en warning om potentiel type mismatch
            if resolved_default not in ({}, []):
                log.warning(
                    f"Returning potentially unexpected default type ({type(resolved_default).__name__}) for empty file."
                )
            return resolved_default
        ...
        log.exception(
            f"JSONDecodeError reading {file_path}. File is likely corrupt. Error: {json_err}. Returning default ({type(resolved_default).__name__})."
        )
        # TODO: Overvej automatisk backup gendannelse her? Se _save_json_file
        return resolved_default
    except (IOError, OSError) as io_err:
        log.exception(
            f"IO/OS Error reading {file_path}: {io_err}. Returning default ({type(resolved_default).__name__})."
        )
        ...
        log.exception(
            f"Unexpected error loading JSON from {file_path}: {e}. Returning default ({type(resolved_default).__name__})."
        )
        return resolved_default
...
def _save_json_file(file_path: str, data: Any, is_list: bool = False) -> bool:
    ...
    if not file_path or "_ERROR" in file_path:
        log.error(
            f"_save_json_file: Invalid or unconfigured file path: '{file_path}'. Save aborted."
        )
        return False
    ...
    if not isinstance(data, expected_type):
        ...
        log.error(
            f"_save_json_file: Invalid data type for {file_path}. "
    ...
    try:
        ...
        if os.path.exists(file_path):
            try:
                ...
                log.critical(
                    f"CRITICAL: Failed to MOVE existing {file_path} to backup ({backup_file}). Aborting save to prevent potential data loss during replace. Error: {move_err}"
                )
                ...
                raise DataIOException(
                    f"Failed to create mandatory backup for {file_path}. Aborting save."
                ) from move_err
        ...
        try:
            ...
            log.exception(
                f"Error writing data to temporary file {temp_file_path}: {write_err}"
            )
            ...
            if os.path.exists(temp_file_path):
                try:
                    ...
                    log.warning(
                        f"Could not remove partially written temp file {temp_file_path} during write error handling."
                    )
            # Raise for at gå til ydre except (recovery)
            raise DataIOException(
                f"Failed writing to temp file for {file_path}"
            ) from write_err
        ...
        try:
            ...
            log.exception(
                f"CRITICAL SAVE FAILURE: os.replace() failed moving {temp_file_path} to {file_path}: {replace_err}"
            )
            # Temp filen er intakt, men kunne ikke flyttes.
            # Raise for at gå til ydre except (recovery)
            raise DataIOException(
                f"Atomic replace failed for {file_path}"
            ) from replace_err
        ...
        if os.path.exists(backup_file):
            try:
                ...
                log.warning(
                    f"Could not remove old backup file {backup_file} after successful save: {rm_err}"
                )
        ...
        Exception
    ) as save_err:  # Fanger fejl fra steps 1, 2, 3 (inkl. DataIOException)
        log.exception(
            f"Save operation failed overall for {file_path}. Initiating recovery. Error: {save_err}"
        )
        ...
        try:
            ...
            if os.path.exists(backup_file):
                ...
                if os.path.exists(file_path):
                    try:
                        os.remove(file_path)
                        log.debug(
                            f"Removed potentially corrupt target file {file_path} before recovery."
                        )
                    except OSError as rem_err:
                        log.error(
                            f"Failed to remove target file {file_path} during recovery. Trying move anyway... Error: {rem_err}"
                        )

                shutil.move(backup_file, file_path)
                log.warning(
                    f"RECOVERY: Successfully restored {file_path} from backup file {backup_file}."
                )
                ...
                log.critical(
                    f"RECOVERY WARNING: Backup file {backup_file} not found. "
            ...
            log.critical(
                f"CRITICAL: Exception during recovery process for {file_path}. Data state UNKNOWN! Error: {recovery_err}"
            )
            ...
            if os.path.exists(temp_file_path):
                try:
                    ...
                    log.warning(
                        f"Could not remove temp file {temp_file_path} during final recovery cleanup."
                    )
    ...
    with players_file_lock:
        ...
        try:
            ...
            if not isinstance(data, dict):
                log.error(
                    f"load_players: Data loaded from {PLAYER_FILE_PATH} was not a dict! Type: {type(data)}. Returning empty dict."
                )
                return {}
            log.debug(
                f"load_players successfully loaded {len(data)} player entries."
            )  # Removed Releasing lock log here
            return data
        except Exception:
...
def save_players(players_data: Dict[str, Any]) -> bool:
    ...
    if not isinstance(players_data, dict):
        log.error(
            f"save_players received non-dict data type: {type(players_data)}. Aborting save."
        )
        return False
    log.debug(
        f"Acquiring lock for save_players: {PLAYER_FILE_PATH} ({len(players_data)} players)"
    )
    with players_file_lock:
    ...
    with players_file_lock:
        ...
        try:
            ...
            if not isinstance(players, dict):
                log.error(
                    "save_avatar_path: Failed to load players data as dict. Aborting."
                )
                return False
            ...
            if isinstance(user_data, dict):
                if user_data.get("avatar") != new_avatar_path:
                    ...
                    data_changed = True
                    log.info(
                        f"Avatar path for user key '{user_key}' updated in memory to '{new_avatar_path}'."
                    )
                else:
                    log.debug(
                        f"Avatar path for user key '{user_key}' is already correct. No change."
                    )
            else:
                log.warning(
                    f"save_avatar_path: User key '{user_key}' not found or data invalid in players file."
                )
            ...
            if data_changed:
                ...
                log.info(
                    f"save_avatar_path: No data change needed for user '{user_key}'."
                )
                return True  # Ingen ændring nødvendig, men operationen var "succesfuld"

        except Exception:
            log.exception(
                f"Unexpected error in locked save_avatar_path for user '{user_key}'."
            )
            return False
    ...
    with invite_codes_lock:  # <<< Bruger lås
        ...
        try:
            ...
            data = _load_json_file(
                INVITE_CODES_FILE_PATH, default_value=[]
            )  # <<< Ny default

            # --- Grundig validering ---
            if not isinstance(data, list):
                log.error(
                    f"Invite codes data in {INVITE_CODES_FILE_PATH} is not a list. Type: {type(data)}. Returning empty list."
                )
                return []
            ...
            for idx, item in enumerate(data):
                if isinstance(item, dict):
                    ...
                    if "code" in item:
                        ...
                        log.warning(
                            f"load_invite_codes: Item at index {idx} in {INVITE_CODES_FILE_PATH} is a dict but missing 'code' key. Skipping item: {item}"
                        )
                        invalid_items_found = True
                else:
                    log.warning(
                        f"load_invite_codes: Item at index {idx} in {INVITE_CODES_FILE_PATH} is not a dictionary. Skipping item: {item}"
                    )
                    invalid_items_found = True

            if invalid_items_found:
                log.warning(
                    "load_invite_codes finished, but some items were skipped due to format issues."
                )
            else:
                log.debug(
                    f"load_invite_codes successfully loaded and validated {len(valid_codes)} invite code objects."
                )

            return valid_codes  # Returner kun de gyldige dicts
        except (
            DataIOException
        ) as e:  # Specifikt fange IO fejl fra _load_json_file hvis den raised
            log.error(f"Failed to load invite codes due to data access error: {e}")
            return []
        except Exception:
            log.exception(
                f"Unexpected error within locked load_invite_codes block: {e}"
            )
            return []
...
def save_invite_codes(
...
) -> bool:  # <<< Ændret parametertype og navn
    """
    ...
    if not isinstance(codes_list, list):
        log.error(
            f"save_invite_codes: Input data must be a list, got {type(codes_list)}. Aborting save."
        )
        return False
    ...
    for idx, item in enumerate(codes_list):
        if not isinstance(item, dict):
            log.error(
                f"save_invite_codes: Item at index {idx} is not a dictionary. Aborting save. Item: {item}"
            )
            return False
        if "code" not in item:
            log.error(
                f"save_invite_codes: Item at index {idx} is missing the required 'code' key. Aborting save. Item: {item}"
            )
            return False
    # --- Slut validering ---

    log.debug(
        f"Acquiring lock for save_invite_codes: {INVITE_CODES_FILE_PATH} ({len(codes_list)} codes)"
    )
    with invite_codes_lock:  # <<< Bruger lås
        log.debug("Lock acquired for save_invite_codes.")
        try:
            # Kald _save_json_file direkte med listen og is_list=True
            success = _save_json_file(
                INVITE_CODES_FILE_PATH, codes_list, is_list=True
            )  # <<< Gemmer liste direkte
...
def check_invite_code_validity(code: str) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
    ...
    try:
        ...
        if found_code_obj:
            if not found_code_obj.get(
                "used", False
            ):  # Default til False hvis 'used' mangler? Sikrest er True. Default til False her.
                ...
                log.warning(
                    f"Invite code '{code}' found but is already used (by {used_by} at {used_at})."
                )
                ...
                return (
                    False,
                    f"Invite koden er allerede brugt{usage_info}.",
                    found_code_obj,
                )
        else:
    ...
    try:
        ...
        for user_data in players.values():
            if (
                ...
                and user_data.get("email", "").lower() == lookup_email
            ):
                return True
        return False
    except Exception:
...
def load_system_settings() -> Dict[str, Any]:
    ...
    with system_settings_lock:
        ...
        try:
            ...
            data = _load_json_file(
                SYSTEM_SETTINGS_FILE_PATH, default_value=DEFAULT_SYSTEM_SETTINGS.copy()
            )
            if not isinstance(data, dict):
                log.error(
                    f"load_system_settings: Data loaded from {SYSTEM_SETTINGS_FILE_PATH} was not a dict! Type: {type(data)}. Returning default settings."
                )
            ...
            log.debug(
                f"load_system_settings successfully loaded {len(loaded_settings)} settings entries."
            )
            return loaded_settings
        except Exception:
            log.exception("Unexpected error within locked load_system_settings block.")
            return (
                DEFAULT_SYSTEM_SETTINGS.copy()
            )  # Returner en kopi af defaults ved fejl
...
def save_system_settings(settings_data: Dict[str, Any]) -> bool:
    ...
    if not isinstance(settings_data, dict):
        log.error(
            f"save_system_settings received non-dict data type: {type(settings_data)}. Aborting save."
        )
        return False
    ...
    with system_settings_lock:
        ...
        try:
            # Gem data - forventer dictionary
            success = _save_json_file(
                SYSTEM_SETTINGS_FILE_PATH, settings_data, is_list=False
            )
...
def add_audit_log(
    ...
    if not AUDIT_LOG_FILE_PATH or "_ERROR" in AUDIT_LOG_FILE_PATH:
        log.error(
            "add_audit_log: AUDIT_LOG_FILE_PATH is not configured. Audit log not saved."
        )
        return

    from datetime import (  # Importer her for at undgå cirkulær import ved modul load
        datetime,
        timezone,
    )
    ...
    with audit_log_lock:
        ...
        try:
            ...
            if not isinstance(audit_logs, list):
                log.error(
                    f"Audit log data in {AUDIT_LOG_FILE_PATH} is not a list. Resetting to empty list. Type: {type(audit_logs)}"
                )
                audit_logs = []
            ...
            if len(audit_logs) > MAX_AUDIT_LOG_ENTRIES:
                ...
                log.info(
                    f"Audit log trimmed to the latest {MAX_AUDIT_LOG_ENTRIES} entries."
                )

            if _save_json_file(AUDIT_LOG_FILE_PATH, audit_logs, is_list=True):
                log.info(
                    f"Audit log entry added: Action='{action}', Admin='{admin_user}', Target='{target_resource or 'N/A'}'"
                )
            else:
...
def load_audit_logs(limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    if not AUDIT_LOG_FILE_PATH or "_ERROR" in AUDIT_LOG_FILE_PATH:
        log.error(
            "load_audit_logs: AUDIT_LOG_FILE_PATH is not configured. Cannot load audit logs."
        )
        return []
    ...
    with audit_log_lock:
        ...
        try:
            ...
            if not isinstance(audit_logs, list):
                log.error(
                    f"Audit log data in {AUDIT_LOG_FILE_PATH} is not a list. Returning empty list. Type: {type(audit_logs)}"
                )
                return []
...
def load_sessions() -> Dict[str, Any]:
    ...
    if Config is not None:  # Check if Config was successfully imported
        sessions_file_path = getattr(
            Config, "SESSIONS_FILE_PATH", None
        )  # Hent fra Config

    if not sessions_file_path:
        log.error(
            "SESSIONS_FILE_PATH er ikke konfigureret. Kan ikke loade file sessions."
        )
        return {}
    log.critical(
        f"=== BRUGER FIL-BASEREDE SESSIONS ({sessions_file_path}) - IKKE EGNET TIL PRODUKTION! ==="
    )
...
def save_sessions(sessions_data: Dict[str, Any]) -> bool:
    ...
    if not sessions_file_path:
        log.error(
            "SESSIONS_FILE_PATH er ikke konfigureret. Kan ikke gemme file sessions."
        )
        return False
    if not isinstance(sessions_data, dict):
        log.error(
            f"save_sessions: Invalid data type {type(sessions_data)}. Expected dict."
        )
        return False
    log.critical(
        f"=== GEMMER TIL FIL-BASEREDE SESSIONS ({sessions_file_path}) - IKKE EGNET TIL PRODUKTION! ==="
    )
    # Overvej at tilføje sessions_file_lock herom, hvis det *skal* bruges midlertidigt
    # with sessions_file_lock:
