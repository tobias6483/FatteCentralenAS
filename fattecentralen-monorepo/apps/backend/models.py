...
class User(db.Model, UserMixin):
    ...
    firebase_uid = db.Column(db.String(128), unique=True, nullable=True, index=True) # For Firebase integration
    password_hash = db.Column(db.String(128), nullable=True) # Nullable if using Firebase auth primarily
...
class BetHistory(db.Model):
    ...
    def __init__(self, user_id: int, stake: float, session_id: Optional[str] = None,
                 ...
                 status: Optional[str] = "open", timestamp_val: Optional[datetime] = None, **kwargs): # Renamed timestamp to timestamp_val
...
class SportCategory(db.Model):
    ...
    leagues = db.relationship('League', back_populates='sport_category', lazy='dynamic', cascade="all, delete-orphan")

    def __init__(self, name: str, description: Optional[str] = None, icon: Optional[str] = None):
        self.name = name
        self.description = description
        self.icon = icon
        self.generate_slug()
    ...
    def generate_slug(self):
        if self.name:
            ...
            while SportCategory.query.filter(
...
def sport_category_name_set_listener(target, value, oldvalue, initiator):
    if value != oldvalue and target.name: # Check if name is set to avoid issues during init
        target.generate_slug()


class League(db.Model):
    __tablename__ = 'league'
    id = db.Column(db.Integer, primary_key=True) # Changed from string key to integer PK
    name = db.Column(db.String(150), nullable=False, index=True)
    slug = db.Column(db.String(170), unique=True, nullable=False, index=True)
    logo_url = db.Column(db.String(512), nullable=True)
    country = db.Column(db.String(100), nullable=True) # e.g., 'England', 'International'
    
    sport_category_id = db.Column(db.Integer, db.ForeignKey('sport_category.id'), nullable=False, index=True)
    sport_category = db.relationship('SportCategory', back_populates='leagues')
    
    active = db.Column(db.Boolean, default=True, nullable=False)
    last_api_check = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc)) # When was this league last checked/updated from an API

    events = db.relationship('SportEvent', back_populates='league', lazy='dynamic', cascade="all, delete-orphan")

    def __init__(self, name: str, sport_category_id: int, country: Optional[str] = None, logo_url: Optional[str] = None, active: bool = True):
        self.name = name
...
class Team(db.Model):
    ...
    home_events = db.relationship('SportEvent', foreign_keys='SportEvent.home_team_id', back_populates='home_team_obj', lazy='dynamic')
    away_events = db.relationship('SportEvent', foreign_keys='SportEvent.away_team_id', back_populates='away_team_obj', lazy='dynamic')

    def __init__(self, name: str, short_name: Optional[str] = None, logo_url: Optional[str] = None, country: Optional[str] = None):
        self.name = name
...
class SportEvent(db.Model):
    ...
    id = db.Column(db.String(255), primary_key=True) # Event ID from external API (e.g., "sportradar:match:12345")
    ...
    home_team_id = db.Column(db.Integer, db.ForeignKey('team.id'), nullable=True, index=True) # Nullable if team not in DB yet
    home_team_obj = db.relationship('Team', foreign_keys=[home_team_id], back_populates='home_events')
    
    away_team_id = db.Column(db.Integer, db.ForeignKey('team.id'), nullable=True, index=True) # Nullable if team not in DB yet
    away_team_obj = db.relationship('Team', foreign_keys=[away_team_id], back_populates='away_events')
    ...
    status = db.Column(db.String(50), nullable=True, index=True) # e.g., "scheduled", "live", "finished", "postponed", "cancelled"
    minute = db.Column(db.Integer, nullable=True) # Current minute of the game
    period = db.Column(db.String(50), nullable=True) # e.g., "1H", "2H", "OT", "P", "FT" (Full Time), "HT" (Half Time)
    ...
    venue_info = db.Column(db.JSON, nullable=True) # { "name": "Stadium Name", "city": "City", "country": "Country" }
    statistics = db.Column(db.JSON, nullable=True) # { "home_possession": 60, "away_shots": 10, ... }
    timeline_events = db.Column(db.JSON, nullable=True) # List of events: [{ "minute": 10, "type": "goal", "player": "X", "team": "home" }, ...]
    lineups = db.Column(db.JSON, nullable=True) # { "home": [{ "name": "Player A", "number": 10, "position": "FWD" }], "away": [...] }

    last_api_update = db.Column(db.DateTime(timezone=True), nullable=True) # When was data last fetched from API for this event?

    outcomes = db.relationship('SportOutcome', back_populates='event', lazy='dynamic', cascade="all, delete-orphan")
    game_sessions = db.relationship('GameSession', back_populates='event', lazy='dynamic') # Relationship to GameSession

    def __init__(self, id: str, league_id: int, commence_time: datetime,
                 home_team_id: Optional[int] = None, away_team_id: Optional[int] = None,
                 home_team_name_raw: Optional[str] = None, away_team_name_raw: Optional[str] = None,
                 status: Optional[str] = "scheduled", **kwargs):
        self.id = id
    ...
    def home_team_display_name(self) -> Optional[str]:
        return self.home_team_obj.name if self.home_team_obj else self.home_team_name_raw
    
    @property
    def away_team_display_name(self) -> Optional[str]:
        return self.away_team_obj.name if self.away_team_obj else self.away_team_name_raw
...
class GameSession(db.Model):
    ...
    event = db.relationship('SportEvent', back_populates='game_sessions') # Updated back_populates
    ...
    def __init__(self, id: str, name: str, creator_id: int, game_mode: str,
                 status: str = 'open', created_at_val: Optional[datetime] = None, # Renamed created_at to created_at_val
