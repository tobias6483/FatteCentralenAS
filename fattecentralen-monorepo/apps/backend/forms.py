...
from wtforms.validators import Optional as WTFormsOptionalValidator
...
try:
    from .data_access import check_invite_code_validity, email_exists, user_exists
    ...
    log.exception(
        "CRITICAL FORMS ERROR: Failed to import from app.data_access. Validation dependent on it will fail or be skipped!"
    )
    ...
    def check_invite_code_validity(code: str) -> Tuple[bool, str, TypingOptional[Dict]]:
        log.error(
            "SKIPPING check_invite_code_validity check: data_access module unavailable."
        )
...
FORUM_POST_MIN_LENGTH = getattr(
    Config, "FORUM_POST_MIN_LENGTH", 10
)  # Gælder for tråd start
FORUM_REPLY_MIN_LENGTH = getattr(
    Config, "FORUM_REPLY_MIN_LENGTH", 3
)  # Kortere for svar
...
MAX_AVATAR_SIZE_MB_CONFIG = getattr(
    Config, "MAX_AVATAR_SIZE_MB", 2
)  # Hent fra Config med fallback
try:
    MAX_AVATAR_BYTES = int(MAX_AVATAR_SIZE_MB_CONFIG * 1024 * 1024)
except (ValueError, TypeError):
    log.warning(
        f"Invalid MAX_AVATAR_SIZE_MB value ('{MAX_AVATAR_SIZE_MB_CONFIG}'). Using default 2MB."
    )
...
USERNAME_REGEX_MESSAGE = (
    "Må kun indeholde bogstaver (a-z, A-Z), tal (0-9) og underscore (_)."
)

ALLOWED_AVATAR_EXTENSIONS = ("png", "jpg", "jpeg", "gif", "webp")
ALLOWED_AVATAR_EXTENSIONS_STR = ", ".join(
    f".{ext}" for ext in ALLOWED_AVATAR_EXTENSIONS
)
...
def form_select_render_kw(**kwargs) -> Dict[str, Any]:
    attrs = {
        "class": "form-select"
    }  # Brug form-select for select elementer i Bootstrap 5+
    attrs.update(kwargs)
    return attrs
...
class InviteCodeValid:
    ...
    def __call__(self, form: FlaskForm, field: StringField):
        if not DATA_ACCESS_AVAILABLE:
            log.warning(
                f"Skipping invite code validation for '{field.name}': data_access unavailable."
            )
            ...
            try:
                ...
                if not is_valid:
                    ...
                    error_msg = (
                        self.message
                        or msg_from_check
                        or "Ugyldig eller allerede brugt invite kode."
                    )
                    raise ValidationError(error_msg)
            except Exception as e:
                # Undgå at vise interne fejl til brugeren
                log.exception(
                    f"Exception during check_invite_code_validity for code '{field.data}': {e}"
                )
                raise ValidationError(
                    "Systemfejl ved validering af invite kode. Kontakt support hvis problemet vedvarer."
                )
...
class UniqueEmail:
    ...
    def __init__(self, message: TypingOptional[str] = None):
        self.message = (
            message or "E-mailadressen er allerede registreret til en anden konto."
        )

    def __call__(self, form: FlaskForm, field: StringField):
        if not DATA_ACCESS_AVAILABLE:
            log.warning(
                f"Skipping unique email validation for '{field.name}': data_access unavailable."
            )
        ...
        if field.data:
            ...
            try:
                ...
                log.exception(
                    f"Exception during email_exists check for email '{field.data}': {e}"
                )
                raise ValidationError(
                    "Systemfejl ved validering af e-mail. Kontakt support."
                )
...
class UniqueUsername:
    ...
    def __call__(self, form: FlaskForm, field: StringField):
        if not DATA_ACCESS_AVAILABLE:
            log.warning(
                f"Skipping unique username validation for '{field.name}': data_access unavailable."
            )
        ...
        if field.data:
            ...
            try:
                ...
                log.exception(
                    f"Exception during user_exists check for username '{field.data}': {e}"
                )
                raise ValidationError(
                    "Systemfejl ved validering af brugernavn. Kontakt support."
                )
...
class AdminUniqueEmail:
    ...
    def __call__(self, form: FlaskForm, field: StringField):
        if not DATA_ACCESS_AVAILABLE:
            log.warning(
                f"Skipping admin unique email validation for '{field.name}': data_access unavailable."
            )
            return
        ...
        if field.data and hasattr(form, "original_email"):
            ...
            assert hasattr(
                form, "original_email"
            ), "Form must have 'original_email' attribute for AdminUniqueEmail validator"
            ...
            original_email_lower = (
                original_email_field.data or ""
            ).lower()  # Håndter hvis original ikke er sat
            ...
            try:
                ...
                log.exception(
                    f"Exception during email_exists check (admin) for email '{field.data}': {e}"
                )
                raise ValidationError(
                    "Systemfejl ved validering af e-mail (admin). Kontakt support."
                )
        elif not hasattr(form, "original_email"):
            log.error(
                "AdminUniqueEmail validator called on form without 'original_email' field."
            )
...
class RegistrationForm(FlaskForm):
    ...
    username = StringField(
        ...
        render_kw=form_control_render_kw(
            placeholder=f"Min {USERNAME_MIN_LENGTH} tegn (a-z, 0-9, _)",
            required=True,
    ...
    password = PasswordField(
        ...
        validators=[
            ...
            Length(
                min=PASSWORD_MIN_LENGTH, message=f"Mindst {PASSWORD_MIN_LENGTH} tegn."
            ),
        ...
        if DATA_ACCESS_AVAILABLE:
            try:
                if not email_exists(email.data):
                    raise ValidationError(
                        "Der findes ingen konto med denne e-mailadresse."
                    )
            except Exception as e:
                log.exception(
                    f"Exception during email_exists check (pw reset request) for email '{email.data}': {e}"
                )
                raise ValidationError("Systemfejl ved tjek af e-mail. Kontakt support.")
        else:
            # Cannot validate existence if data_access is down
            log.warning(
                "Skipping email existence check for password reset request: data_access unavailable."
            )
...
class UpdateAccountForm(FlaskForm):
    ...
    about_me = TextAreaField(
        ...
        render_kw=form_textarea_render_kw(
            rows=5, placeholder="Fortæl lidt om dig selv..."
        ),
    )

    # Privacy Settings
    privacy_profile_public = BooleanField(
        "Gør min profil offentlig",
        default=True,
        render_kw={"class": "form-check-input"},
    )
    privacy_show_activity = BooleanField(
        "Vis min seneste aktivitet på min profil",
        default=True,
        render_kw={"class": "form-check-input"},
    )
    privacy_show_bet_history = BooleanField(
        "Vis min spilhistorik på min profil",
        default=True,
        render_kw={"class": "form-check-input"},
    )
    privacy_show_online_status = BooleanField(
        "Vis min online status til andre brugere",
        default=True,
        render_kw={"class": "form-check-input"},
    )
...
class ChangePasswordForm(FlaskForm):
    ...
    confirm_new_password = PasswordField(
        ...
        validators=[
            ...
            EqualTo(
                "new_password", message="De nye adgangskoder stemmer ikke overens."
            ),
        ],
        render_kw=form_control_render_kw(required=True, autocomplete="new-password"),
    )
...
class ForumReplyForm(FlaskForm):
    ...
    body = TextAreaField(
        ...
        validators=[
            ...
            Length(
                min=FORUM_REPLY_MIN_LENGTH,
                max=FORUM_REPLY_MAX_LENGTH,
                message=f"Svar skal være mellem {FORUM_REPLY_MIN_LENGTH} og {FORUM_REPLY_MAX_LENGTH} tegn.",
            ),
        ],
        render_kw=form_textarea_render_kw(
            rows=6, placeholder="Skriv dit svar her...", required=True
        ),
    )
...
class ForumEditPostForm(FlaskForm):
    ...
    body = TextAreaField(
        ...
        validators=[
            ...
            Length(
                min=FORUM_REPLY_MIN_LENGTH,
                max=FORUM_POST_MAX_LENGTH,  # Tillad op til max post længde ved redigering
                message=f"Skal være mellem {FORUM_REPLY_MIN_LENGTH} og {FORUM_POST_MAX_LENGTH} tegn.",
            ),
        ],
        render_kw=form_textarea_render_kw(
            rows=12, placeholder="Rediger dit indlæg...", required=True
        ),
    )
    submit = SubmitField("Gem Ændringer")
...
class ForumNewThreadForm(FlaskForm):
    ...
    title = StringField(
        ...
        validators=[
            ...
            Length(
                min=FORUM_TITLE_MIN_LENGTH,
                max=FORUM_TITLE_MAX_LENGTH,
                message=f"Titel skal være mellem {FORUM_TITLE_MIN_LENGTH} og {FORUM_TITLE_MAX_LENGTH} tegn.",
            ),
        ],
        render_kw=form_control_render_kw(
            placeholder="Giv tråden en klar og beskrivende titel", required=True
        ),
    )
    body = TextAreaField(
        "Første Indlæg",
        ...
        render_kw=form_textarea_render_kw(
            rows=10, placeholder="Skriv det første indlæg i tråden...", required=True
        ),
    )
...
class AdminEditUserForm(FlaskForm):
    ...
    email = StringField(
        ...
        ),  # Slå autocomplete fra for admin felter
    )
    role = SelectField(
        "Rolle",
    ...
    about_me = TextAreaField(
        ...
        render_kw=form_textarea_render_kw(
            rows=3, placeholder="Administrator kan opdatere beskrivelsen..."
        ),
    )
    # Read-only felt til at vise status - skal sættes manuelt fra view
    twofa_enabled_display = StringField(
        "2FA Status",
...
class AdminUpdateBalanceForm(FlaskForm):
    ...
    balance = DecimalField(
        "Ny Saldo",
        validators=[
            InputRequired("Saldo skal angives."),
            NumberRange(min=0, message="Saldo kan ikke være negativ."),
        ],
        places=2,  # Tillad 2 decimaler
        render_kw=form_control_render_kw(
            type="number", step="0.01"
        ),  # Brug HTML5 number input
    )
...
class PrivateMessageForm(FlaskForm):
    ...
    recipient_username = StringField(
        ...
        render_kw=form_control_render_kw(
            placeholder="Indtast modtagerens brugernavn", required=True
        ),
    )
    subject = StringField(
        "Emne",
        ...
        render_kw=form_control_render_kw(
            placeholder="Indtast emne for beskeden", required=True
        ),
    )
    body = TextAreaField(
        "Besked",
        validators=[
            ...
            Length(
                min=1, max=FORUM_POST_MAX_LENGTH
            ),  # Reuse forum post max length for now
        ],
        render_kw=form_textarea_render_kw(
            rows=8, placeholder="Skriv din private besked her...", required=True
        ),
    )
    ...
    def validate_recipient_username(self, recipient_username: StringField):
        ...
        if not DATA_ACCESS_AVAILABLE:
            log.warning(
                f"Skipping recipient username validation for '{recipient_username.data}': data_access unavailable."
            )
            return
        if recipient_username.data:
            try:
                if not user_exists(recipient_username.data):
                    raise ValidationError(
                        f"Brugeren '{recipient_username.data}' blev ikke fundet."
                    )
                if (
                    current_user.is_authenticated
                    and recipient_username.data.lower() == current_user.username.lower()
                ):
                    raise ValidationError(
                        "Du kan ikke sende en privat besked til dig selv."
                    )
            except Exception as e:
                log.exception(
                    f"Exception during user_exists check for recipient '{recipient_username.data}': {e}"
                )
                raise ValidationError(
                    "Systemfejl ved validering af modtager. Kontakt support."
                )
