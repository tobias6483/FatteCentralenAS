...
from flask import g as flask_g  # Added jsonify and flask_g
...
from .models import User as DBUser  # Added Notification, Badge
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not current_user.is_authenticated or not getattr(
            ...
            flash(
                "Du har ikke de nødvendige rettigheder for at tilgå denne side.",
                "danger",
            )
...
def firebase_token_required(f):
    ...
    @wraps(f)
    def decorated_function(*args, **kwargs):
        decoded_token = None  # Initialize decoded_token
        auth_header = request.headers.get("Authorization")
        if not auth_header:
            current_app.logger.warning(
                "Firebase token required: Missing Authorization header."
            )
        ...
        if parts[0].lower() != "bearer":
            current_app.logger.warning(
                f"Firebase token required: Invalid Authorization header format (expected Bearer token). Header: {auth_header}"
            )
            return (
            ...
            current_app.logger.warning(
                f"Firebase token required: Token not found after Bearer. Header: {auth_header}"
            )
            return jsonify({"message": "Token not found"}), 401
        elif len(parts) > 2:
            current_app.logger.warning(
                f"Firebase token required: Invalid Authorization header format (too many parts). Header: {auth_header}"
            )
            return (
        ...
        try:
            ...
            flask_g.firebase_user = (
                decoded_token  # Store the decoded token in Flask's g object
            )
            current_app.logger.debug(
                f"Firebase token verified successfully for UID: {decoded_token.get('uid')}"
            )
        except firebase_auth.RevokedIdTokenError:
            uid_for_log = decoded_token.get("uid") if decoded_token else "unknown"
            current_app.logger.warning(
                f"Firebase token revoked for UID: {uid_for_log}. Token: {id_token[:20]}..."
            )
            return (
                jsonify({"message": "Token has been revoked. Please re-authenticate."}),
                401,
            )
        except firebase_auth.UserDisabledError:
            uid_for_log = decoded_token.get("uid") if decoded_token else "unknown"
            current_app.logger.warning(
                f"Firebase user disabled for UID: {uid_for_log}. Token: {id_token[:20]}..."
            )
            return jsonify({"message": "User account has been disabled."}), 403
        except firebase_auth.InvalidIdTokenError as e:
            current_app.logger.warning(
                f"Invalid Firebase ID token: {e}. Token: {id_token[:20]}..."
            )
            return jsonify({"message": f"Invalid ID token: {e}"}), 401
        except Exception as e:  # Catch any other Firebase Admin SDK errors
            current_app.logger.error(
                f"Error verifying Firebase ID token: {e}. Token: {id_token[:20]}..."
            )
...
def dt_filter_func(dt_obj, relative=False, default="N/A"):
    ...
    if not isinstance(dt_obj, datetime):
        log.warning(
            f"dt_filter_func received non-datetime object: {type(dt_obj)}, value: {dt_obj}"
        )
    ...
    if relative:
        try:
            ...
            delta = now - dt_aware
            if delta.days >= 365:
                return f"for {delta.days // 365} år siden"
            if delta.days >= 30:
                return f"for {delta.days // 30} måneder siden"
            if delta.days > 1:
                return f"for {delta.days} dage siden"
            elif delta.days == 1:
                return "i går"
                ...
                return f"for {int(delta.total_seconds())} sek. siden"
            elif delta.total_seconds() < 3600:
                return f"for {int(delta.total_seconds() // 60)} min. siden"
            else:
                return f"for {int(delta.total_seconds() // 3600)} timer siden"
        except Exception as e:
            log.exception(
                f"Error calculating relative time in dt_filter_func for: {dt_obj} - {e}"
            )
            try:
...
def get_user_data_by_id(user_identifier: Optional[Union[str, int]]) -> Optional[DBUser]:
    ...
    try:
        if isinstance(user_identifier, int):
            ...
            if user_obj:
                log.debug(f"get_user_data_by_id: Found user by PK '{user_identifier}'.")
            else:
                log.debug(
                    f"get_user_data_by_id: No user found for PK '{user_identifier}'."
                )
        elif isinstance(user_identifier, str):
            user_obj = DBUser.query.filter(
                func.lower(DBUser.username) == func.lower(user_identifier)
            ).first()
            if user_obj:
                log.debug(
                    f"get_user_data_by_id: Found user by username '{user_identifier}'."
                )
            else:
                log.debug(
                    f"get_user_data_by_id: No user found for username '{user_identifier}'."
                )
        else:
            log.warning(
                f"get_user_data_by_id: Invalid type for user_identifier: {type(user_identifier)}"
            )
            return None
        return user_obj
        ...
        log.exception(
            f"get_user_data_by_id: Unexpected error processing user_identifier '{user_identifier}'."
        )
        return None


def get_user_data_batch(
    usernames: Union[List[str], Set[str]]
) -> Dict[str, Optional[DBUser]]:
    if not usernames:
        return {}
    ...
    log.debug(
        f"get_user_data_batch: Fetching DBUser objects for {len(lower_usernames_to_query)} unique lowercase usernames."
    )
    results: Dict[str, Optional[DBUser]] = {
        original_name: None for original_name in usernames
    }
    try:
        found_users: List[DBUser] = DBUser.query.filter(
            func.lower(DBUser.username).in_(lower_usernames_to_query)
        ).all()
        for user_obj in found_users:
            original_username = original_case_map.get(user_obj.username.lower())
            if original_username:
                results[original_username] = user_obj
            else:
                log.warning(
                    f"get_user_data_batch: Found user '{user_obj.username}' in DB but couldn't map back..."
                )
        found_count = sum(1 for user_obj in results.values() if user_obj is not None)
        log.info(
            f"get_user_data_batch: Fetched {found_count} DBUser objects out of {len(usernames)} requested."
        )
        not_found_original_keys = [
            uname for uname, user_obj in results.items() if user_obj is None
        ]
        if not_found_original_keys:
            log.debug(
                f"get_user_data_batch: Could not find DBUser objects for: {not_found_original_keys}"
            )
        return results
    except RuntimeError as e:
        ...
        log.exception(
            f"get_user_data_batch: Unexpected error processing usernames {list(usernames)}."
        )
        return {name: None for name in usernames}
...
def generate_backup_codes(num_codes: int = 0, code_length: int = 10) -> List[str]:
    ...
    if final_num_codes <= 0:
        try:
            ...
            log.warning(
                f"Could not read/parse NUM_BACKUP_CODES from config ({e}). Using default: 10."
            )
            final_num_codes = 10
    if final_num_codes <= 0:
        return []
...
def is_safe_url(target: str) -> bool:
    ...
    try:
        ...
        ) or (not test_url.scheme and not test_url.netloc)
        if not is_safe:
            log.warning(
                f"Blocked unsafe redirect target: '{target}' (Evaluated against host: {request.host_url})"
            )
        return is_safe
    except RuntimeError:
...
def get_common_context() -> Dict[str, Any]:
    ...
    try:
        ...
        if cfg_default_avatar and isinstance(cfg_default_avatar, str):
            default_avatar_rel_path = cfg_default_avatar.lstrip("/").replace(
                "static/", "", 1
            )
        final_default_avatar_url = url_for("static", filename=default_avatar_rel_path)
    except RuntimeError:
        log.error("Cannot generate URLs in get_common_context: No application context.")
    except Exception as e:
        log.exception(f"Error generating default avatar URL in context: {e}")
    ...
    if current_user and current_user.is_authenticated:
        ...
        log.debug(
            f"Populating context for authenticated user: '{username}' (ID: {current_user.id})"
        )
        user_avatar_url = current_user.avatar_url
        if not user_avatar_url:
            user_avatar_url = final_default_avatar_url
            log.debug(
                f"User '{username}' avatar_url from model was empty or failed. Using default: {user_avatar_url}"
            )
        else:
            log.debug(f"User avatar URL from model property: '{user_avatar_url}'")
        context.update(
            {
                ...
                "user_rank": current_user.rank or "N/A",
                "app_name": app_name,
                "debug_mode": (
                    current_app.config.get("DEBUG", False)
                    if "current_app" in locals() and hasattr(current_app, "config")
                    else False
                ),
                "unread_message_count": PrivateMessage.query.filter_by(
                    recipient_id=current_user.id, is_read=False, recipient_deleted=False
                ).count(),
            }
        )
    else:
        log.debug("Context generated for anonymous user.")
    return context
...
def get_balance_history(user_id: str, days: int = 7) -> Tuple[List[str], List[float]]:
    ...
    try:
        ...
        log.error(
            f"App context potentially missing or other RuntimeError in get_balance_history: {e}"
        )
        return [], []
    except Exception as e:
    ...
    for symbol, holdings in portfolio_data.items():
        ...
        try:
            if get_current_price_func:
                ...
                current_price = max(
                    1.0,
                    (sum(ord(c) for c in symbol) % 500) / 10.0 + random.uniform(-2, 2),
                )
            ...
            if current_price >= 0:
                ...
                log.warning(
                    f"Received non-positive price ({current_price}) for symbol '{symbol}'. Ignored in total."
                )
        except Exception as price_err:
            log.exception(
                f"Error getting or processing price for symbol '{symbol}' during portfolio calculation: {price_err}"
            )
...
# ==================================================
from .models import (  # Import Badge model and association if needed directly
    Badge,
...
def award_badge(user_id: int, badge_key: str) -> bool:
    ...
    try:
        ...
        if not badge:
            log.warning(
                f"Cannot award badge: Badge with key '{badge_key}' not found in database."
            )
            return False
        ...
        already_has = (
            db.session.query(user_badge_association)
        ...
        if already_has:
            log.debug(
                f"User {user_id} already has badge '{badge_key}'. No action taken."
            )
        ...
        db.session.commit()
        log.info(
            f"Successfully awarded badge '{badge_key}' (ID: {badge.id}) to user {user_id} ('{user.username}')."
        )
        ...
        try:
            ...
            profile_link = url_for(
                "main.profile", username=user.username, _external=False
            )  # Assuming 'main.profile' is the route
            create_notification(
                user_id=user_id,
            ...
            log.debug(
                f"Notification creation initiated for badge '{badge_key}' awarded to user {user_id}."
            )
        except Exception as notify_err:
            log.error(
                f"Failed to create notification for badge award '{badge_key}' to user {user_id}: {notify_err}"
            )
...
def create_notification(
    ...
    """
    log.debug(
        f"Attempting to create notification for user {user_id}: '{message[:50]}...'"
    )
    try:
        # Basic validation
        ...
        db.session.commit()
        log.info(
            f"Notification created for user {user_id} (ID: {notification.id}, Category: {category})"
        )
        ...
        if socketio:
            ...
            log.warning(
                "SocketIO instance not available in create_notification. Cannot emit real-time update."
            )
...
def update_sports_catalog_from_api():
    ...
    try:
        ...
        if not api_key or not base_url_raw:
            log.error(
                "Sports API Key or Base URL not configured. Cannot update sports catalog."
            )
            return False
        ...
        response = requests.get(
            ...
            params=params,
            timeout=timeout,
            headers={"User-Agent": "Fattecentralen-Util/1.0"},
        )
        ...
        try:
            ...
            if response.ok:
                log.error(
                    f"External Catalog API V4 OK status but invalid JSON. Status: {response.status_code}, Content: {response.text[:200]}"
                )
            api_error = True
        ...
        if not response.ok:
            ...
            if (
                api_catalog_raw
                and isinstance(api_catalog_raw, dict)
                and "message" in api_catalog_raw
            ):
                error_detail = api_catalog_raw["message"]
            log.error(
                f"External Catalog API V4 HTTP Error ({status_code}): {error_detail}"
            )
            api_error = True
        ...
        if api_error or not isinstance(api_catalog_raw, list):
            if not api_error:
                log.error(
                    f"External Catalog API V4 returned OK but data is not a list. Type: {type(api_catalog_raw)}"
                )
            sports_data = []
            api_error = True
        else:
        ...
        for sport_api in sports_data:
            ...
            if not key or not title or not group:
                log.warning(
                    f"Skipping sport item with missing key, title, or group: {sport_api}"
                )
                skipped_count += 1
                continue
            ...
            if existing_sport:
                ...
                if (
                    ...
                    or sport_category_name_changed  # If category name from API differs
                    or not existing_sport.active
                ):
                    existing_sport.name = title
                ...
                new_sport = League(  # type: ignore[call-arg]
                    name=title,
                    sport_category_id=1,  # Assuming a default sport_category_id for now. TODO: Resolve SportCategory from 'group'.
                    active=True,
        ...
        for key, sport_db in existing_sports_db.items():
            if key not in processed_keys and sport_db.active:
                ...
                sport_db.last_api_check = datetime.now(
                    timezone.utc
                )  # Corrected attribute
        ...
        db.session.commit()
        log.info(
            f"Sports catalog DB update finished. Created: {created_count}, Updated: {updated_count}, Deactivated: {deactivated_count}, Skipped: {skipped_count}."
        )
        ...
        log.exception(
            f"Failed to fetch sports catalog from external API ({catalog_url}): {req_err}"
        )
        db.session.rollback()
        return False
    except json.JSONDecodeError as json_err:
        log.exception(
            f"Failed to decode JSON response from external API ({catalog_url}): {json_err}"
        )
        db.session.rollback()
        return False
    except Exception as e:
...
def update_sport_events():
    ...
    try:
        ...
        for league in active_sports:
            ...
            log.debug(
                f"Fetching events for league: {league.slug} ({league.name}) with market: {markets_to_fetch}"
            )
            ...
            try:
                response = requests.get(
                    ...
                    params=params,
                    timeout=timeout,
                    headers={"User-Agent": "Fattecentralen-EventUtil/1.0"},
                )
                ...
                if not isinstance(events_data, list):
                    log.warning(
                        f"Received non-list data for league {league.slug} from {events_url}. Skipping."
                    )
                    api_errors += 1
                    continue
                ...
                for event_api in events_data:
                    if not isinstance(event_api, dict):
                        continue
                    ...
                    if not event_id or not commence_time_str:
                        log.warning(
                            f"Skipping event with missing ID or commence_time for league {league.slug}: {event_api}"
                        )
                        continue
                    ...
                    if not commence_time_dt:
                        log.warning(
                            f"Could not parse commence_time '{commence_time_str}' for event {event_id}. Skipping."
                        )
                        continue
                    ...
                    if not event_db:
                        ...
                        log.debug(
                            f"Creating new SportEvent: {event_id} ({home_team_name_raw_api} vs {away_team_name_raw_api}) for league {league.id}"
                        )
                    else:
                        ...
                        ):  # Ensure it's linked to the correct league
                            event_db.league_id = league.id
                            needs_update = True

                        if needs_update:
                            log.debug(f"Updating existing SportEvent: {event_id}")
                        event_db.last_api_data_fetch = datetime.now(
                            timezone.utc
                    ...
                    for bookmaker in bookmakers:
                        ...
                        bookmaker_last_update_dt = _parse_datetime_string(
                            bookmaker_last_update_str
                        )
                        bookmaker_last_update_aware = (
                            bookmaker_last_update_dt.replace(tzinfo=timezone.utc)
                            if bookmaker_last_update_dt
                            else None
                        )
                        ...
                        for market in markets_data:
                            ...
                            if market_key_api != markets_to_fetch:
                                continue
                            ...
                            for outcome in outcomes_api:
                                ...
                                if outcome_name is None or outcome_price is None:
                                    continue
                                try:
                                    ...
                                    log.warning(
                                        f"Could not parse price/point for outcome in event {event_id}: {outcome}"
                                    )
                                    continue
                                ...
                                outcome_db = SportOutcome.query.filter_by(
                                    ...
                                    market_key=market_key_api,
                                    name=outcome_name,
                                    point=outcome_point_float,
                                ).first()
                                ...
                                if not outcome_db:
                                    outcome_db = SportOutcome(  # type: ignore[call-arg]
                                        ...
                                        market_key=market_key_api,
                                        name=outcome_name,
                                        price=outcome_price_float,
                                        point=outcome_point_float,
                                        last_update_api=bookmaker_last_update_aware,
                                    )
                                    db.session.add(outcome_db)
                                    log.debug(
                                        f"Creating new SportOutcome: Event {event_id}, Bookie {bookmaker_key}, Name {outcome_name}, Price {outcome_price_float}"
                                    )
                                else:
                                    if (
                                        outcome_db.price != outcome_price_float
                                        or outcome_db.last_update_api
                                        != bookmaker_last_update_aware
                                    ):
                                        outcome_db.price = outcome_price_float
                                        outcome_db.last_update_api = (
                                            bookmaker_last_update_aware
                                        )
                                        db.session.add(outcome_db)
                                        log.debug(
                                            f"Updating SportOutcome: Event {event_id}, Bookie {bookmaker_key}, Name {outcome_name}, Price {outcome_price_float}"
                                        )
                ...
                log.error(
                    f"Failed to fetch events for league {league.slug} from {events_url}: {req_err}"
                )
                api_errors += 1
                db.session.rollback()
            except json.JSONDecodeError as json_err:
                log.error(
                    f"Failed to decode JSON for league {league.slug} from {events_url}: {json_err}"
                )
                api_errors += 1
                db.session.rollback()
            except Exception as e:
                ...
                db.session.rollback()
        log.info(
            f"Event update process finished. Processed Events: {total_events_processed}, Processed Outcomes: {total_outcomes_processed}, API Errors: {api_errors}"
        )
        return api_errors == 0
    except Exception as e:
