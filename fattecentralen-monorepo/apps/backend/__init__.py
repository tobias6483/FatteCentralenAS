...
from flask_babel import format_currency as babel_format_currency
...
from .config import INSTANCE_FOLDER_PATH, Config
from .extensions import (  # Ensure jwt and migrate are imported
    db,
    jwt,
    migrate,
    socketio,
)
from .models import Notification, PasswordResetRequest, User  # Removed JsonUserWrapper
...
def get_locale_selector():
    ...
    if current_user.is_authenticated:
        ...
        if user_profile_lang and user_profile_lang in Config.LANGUAGES:
...
def format_datetime_filter(value, format="medium", relative=False, include_time=True):
    ...
    if relative:
        ...
        if diff.total_seconds() < 60:
            return f"{int(diff.total_seconds())} sek. siden"
        if diff.total_seconds() < 3600:
            return f"{int(diff.total_seconds() / 60)} min. siden"
        if diff.total_seconds() < 86400:
            return f"{int(diff.total_seconds() / 3600)} tim. siden"
        if diff.days < 2:
            return "I går"
        if diff.days < 7:
    ...
    if format == "medium":
        fmt = "dd. MMM yyyy" + (", HH:mm" if include_time else "")
    elif format == "full":
        fmt = "EEEE, d. MMMM yyyy" + (" 'kl.' HH:mm:ss zzzz" if include_time else "")
    elif format == "short":
        fmt = "dd/MM/yy" + (", HH:mm" if include_time else "")
    else:
        fmt = format
...
def format_currency_filter(value, currency_code=None):
    if value is None:
        return ""
    try:
        return babel_format_currency(float(value), "DKK")
    except (ValueError, TypeError):
        return str(value)
...
def create_app(config_class=Config):
    ...
    socketio.init_app(
        app, cors_allowed_origins=app.config.get("CORS_ALLOWED_ORIGINS", "*")
    )
    CSRFProtect(app)
    CORS(
        app,
    ...
    from .routes.api_aktiedyst import (
        aktiedyst_api_bp as new_aktiedyst_v1_api_bp,  # New V1 API
    )
    ...
    from .routes.api_sports import matches_api_bp
    ...
    from .routes.forum import forum_api_bp, forum_bp
    from .routes.main import main_bp

    # Potentially from .routes.api_bets import api_bets_bp (if defined in api_bets.py)
    from .routes.messages import (  # Potentially from .routes.messages import messages_bp (if defined in messages.py)
        messages_bp,
    )
    from .routes.sessions import (  # Potentially from .routes.sessions import sessions_bp (if defined in sessions.py)
        sessions_bp,
    )
    ...
    app.register_blueprint(
        new_aktiedyst_v1_api_bp
    )  # New V1 API, prefix is in the blueprint
    # app.register_blueprint(api_user_bp)
    # app.register_blueprint(api_admin_bp)
    app.register_blueprint(
        api_sports_bp
    )  # This blueprint already has /api/v1/sports prefix
    app.register_blueprint(
        matches_api_bp
    )  # This blueprint already has /api/v1/matches prefix
    ...
    app.register_blueprint(
        user_profile_api_bp
    )  # New User Profile API, prefix is in the blueprint

    @app.context_processor
    def inject_csrf_token_global():
    ...
    def handle_csrf_error(e):
        ...
        if request.blueprint and request.blueprint.startswith("api_"):
            return (
                jsonify(error="CSRF token missing or invalid", message=e.description),
                400,
            )
    ...
    def before_request_tasks():
        ...
        if current_user.is_authenticated and isinstance(current_user, User):
            ...
            if current_user.last_seen is None:
                ...
                app.logger.info(
                    f"User {current_user.id} last_seen was None, initialized to aware UTC min."
                )
            elif (
                current_user.last_seen.tzinfo is None
                or current_user.last_seen.tzinfo.utcoffset(current_user.last_seen)
                is None
            ):
                # If naive, make it timezone-aware (UTC) directly on the object
                current_user.last_seen = current_user.last_seen.replace(
                    tzinfo=timezone.utc
                )
                made_aware_in_this_request = True
                # Log this specific conversion, as it's correcting existing data
                app.logger.info(
                    f"User {current_user.id} last_seen was naive, converted to aware UTC."
                )
            ...
            if (
                datetime.now(timezone.utc) - comparison_last_seen
            ).total_seconds() > app.config.get(
                "USER_LAST_SEEN_UPDATE_INTERVAL_SECONDS", 60
            ):
                current_user.last_seen = datetime.now(
                    timezone.utc
                )  # This is already aware
                try:
                    ...
                    db.session.rollback()
                    app.logger.error(
                        f"Error updating last_seen for user {current_user.id} (periodic): {e}"
                    )
                ...
                try:
                    db.session.commit()
                    app.logger.info(
                        f"User {current_user.id} last_seen (awareness correction) committed."
                    )
                except Exception as e:
                    db.session.rollback()
                    app.logger.error(
                        f"Error committing timezone-awareness update for last_seen for user {current_user.id}: {e}"
                    )
