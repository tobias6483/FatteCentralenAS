...
from .extensions import redis_client, db, socketio # Added redis_client, db, and ensure socketio is from here
from .models import User, BetHistory, GameSession # Added User, BetHistory, GameSession models
from .routes.sessions import SESSION_KEY_PREFIX, SESSION_PLAYERS_KEY_PREFIX # Import key prefixes
...
def get_player_details_for_session(session_id: str, player_usernames: set) -> dict:
    """
    ...
    for username in player_usernames:
        ...
        if user_obj:
            try:
                avatar_url = url_for('static', filename=user_obj.avatar_url or Config.DEFAULT_AVATAR, _external=False)
            ...
            logger.warning(f"User '{username}' in session '{session_id}' player set, but not found in DB.")
            # Fallback if user not in DB (should ideally not happen)
            try:
                default_avatar_url = url_for('static', filename=Config.DEFAULT_AVATAR, _external=False)
            except RuntimeError:
...
def on_join_room(data): # Reverted to def
    ...
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated user (SID: {sid}) attempted to join room. Denying.")
        emit("server_error", {"msg": "Authentication required to join sessions."}, to=sid)
        return

    username = current_user.id
    try:
        room_name_from_client = data.get("room") # Expected format: "session_ACTUAL_SESSION_ID"
        if not room_name_from_client or not isinstance(room_name_from_client, str) or not room_name_from_client.startswith(SESSION_KEY_PREFIX.replace(":", "_")):
            logger.warning(f"Join room failed (invalid room format '{room_name_from_client}') by {username} ({sid})")
        ...
        logger.info(f"User '{username}' ({sid}) attempting to join room: {actual_room_name} (Session ID: {session_id})")

        session_key = f"{SESSION_KEY_PREFIX}{session_id}"
        session_players_key = f"{SESSION_PLAYERS_KEY_PREFIX}{session_id}"

        # Check if session exists in Redis
        if not redis_client.exists(session_key): # Removed await
            logger.warning(f"Session '{session_id}' not found in Redis for join request by {username}.")
            emit("server_error", {"msg": f"Session '{session_id}' does not exist."}, to=sid)
            return
        ...
        redis_results = pipe.execute() # Removed await

        session_data_raw = redis_results[0]
        # player_usernames_in_session is already a set of strings due to decode_responses=True in Redis client
        player_usernames_in_session = set(redis_results[1]) # Ensure it's a set of strings

        if not session_data_raw: # Should be caught by exists check, but good to be safe
            logger.error(f"Session {session_id} existed but hgetall returned empty. Race condition or error?")
            emit("server_error", {"msg": "Error fetching session details."}, to=sid)
            return

        # Deserialize outcomes and bets
        outcomes = json.loads(session_data_raw.get(b'outcomes', b'[]'))
        bets = json.loads(session_data_raw.get(b'bets', b'[]'))
        session_name = session_data_raw.get(b'session_name', b'Unnamed Session').decode()
        game_mode = session_data_raw.get(b'game_mode', b'unknown').decode()

        # Get details for all players currently in the session (including the new one)
        # The new user should have already been added to the session_players_key by the HTTP join_session route
        all_player_details = get_player_details_for_session(session_id, player_usernames_in_session)
        ...
        logger.info(f"Sent initial state of session {session_id} to new user {username} ({sid})")

        # Inform ALL OTHER users in the room about the new user
        new_user_detail = all_player_details.get(username)
        if new_user_detail:
            emit("user_joined", {"user": new_user_detail, "room": actual_room_name}, to=actual_room_name, include_self=False) # type: ignore[call-arg]
            logger.info(f"Notified room {actual_room_name} that user {username} joined.")
        else:
            logger.warning(f"User {username} joined room {actual_room_name} but their details were not found in all_player_details.")
        ...
        emit("server_error", {"msg": f"Server error during join room: {str(e)}"}, to=sid)
...
def on_place_bet(data): # Reverted to def
    sid = request.sid # type: ignore[attr-defined]
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated user (SID: {sid}) attempted to place bet. Denying.")
    ...
    try:
        ...
        if not session_id: raise ValueError("Session ID missing")
        if stake_raw is None: raise ValueError("Stake missing")

        stake_val = float(stake_raw)
        if stake_val <= 0: raise ValueError("Stake must be positive")
        ...
        if not session_data_raw: raise ValueError(f"Session '{session_id}' not found")
        ...
        if not redis_client.sismember(session_players_key, username): # Assuming sync
            if not redis_client.sismember(session_players_key, str(username)): # Assuming sync
                 raise ValueError(f"User '{username}' is not participating in session '{session_id}'")


        # --- Fetch Player from DB and Check Balance ---
        user_obj = User.query.filter_by(id=username).first()
        if not user_obj: raise ValueError(f"User '{username}' not found in database")
        if user_obj.balance < stake_val:
            return {"success": False, "error": f"Insufficient balance ({user_obj.balance:.2f} DKK) for stake ({stake_val:.2f} DKK)"}

        # --- Process Bet based on type (is_coupon flag from client) ---
        if is_coupon:
            # --- COUPON BET LOGIC ---
            logger.info(f"Processing COUPON bet for session {session_id} by {user_obj.username}, stake: {stake_val}, sid='{sid}'")
            ...
            if redis_is_coupon_flag_raw:
                try: redis_is_coupon = json.loads(decode_redis(redis_is_coupon_flag_raw))
                except json.JSONDecodeError: pass
            
            if not redis_is_coupon:
                logger.error(f"Bet received with is_coupon_bet=true, but session {session_id} is NOT marked as coupon in Redis.")
            ...
            if not selections_json_raw: raise ValueError("Coupon selections not found in Redis.")
            ...
            new_bet_record = BetHistory( # type: ignore[call-arg]
                user_id=user_obj.id, session_id=session_id,
                match_name=decode_redis(session_data_raw.get(b'session_name', b'Kupon Session')), # type: ignore[attr-defined] # Add ignore for .get
                outcome_name="Kupon Væddemål", stake=stake_val, payout=potential_payout,
                result="pending", status="open", timestamp=bet_time
            )
...
# --- Award Badges (First Bet, High Roller, Centurion) ---
            try:
                ...
                logger.error(f"Error checking/awarding badges (first_bet/high_roller/centurion) for user {user_obj.id} after coupon bet: {badge_err}")
            # --- End badge checks ---
            logger.info(f"DB: User {user_obj.username} balance updated to {user_obj.balance:.2f} and BetHistory for COUPON created.")
            ...
            try:
                ...
                bet_count = BetHistory.query.filter_by(user_id=user_obj.id).count()
                if bet_count == 1:
                    award_badge(user_obj.id, 'Første Bet') # Use the exact name from seeding
            except Exception as badge_err:
                logger.error(f"Error checking/awarding 'first_bet' badge for user {user_obj.id}: {badge_err}")
            # --- End badge check ---

            bets_json_raw = session_data_raw.get(b'bets') # type: ignore[attr-defined] # Add ignore for .get
            current_bets_list = json.loads(decode_redis(bets_json_raw)) if bets_json_raw else []
            bet_entry_redis = {
                "user": user_obj.username, "stake": stake_val, "total_odds": total_coupon_odds,
                "potential_payout": potential_payout, "timestamp": bet_time.isoformat(), "is_coupon_bet": True
            }
            current_bets_list.append(bet_entry_redis)
            redis_client.hset(session_key, "bets", json.dumps(current_bets_list)) # Assuming sync
            ...
            if not outcome_name_from_data: raise ValueError("Outcome missing for single bet")
            
            logger.info(f"Processing SINGLE bet for session {session_id} by {user_obj.username} on '{outcome_name_from_data}', stake: {stake_val}, sid='{sid}'")

            outcomes_json_raw = session_data_raw.get(b'outcomes') # type: ignore[attr-defined] # Add ignore for .get
            if not outcomes_json_raw: raise ValueError("Session outcomes not found in Redis for single bet.")
            outcomes = json.loads(decode_redis(outcomes_json_raw))
            if not isinstance(outcomes, list): raise ValueError("Outcomes data is not a list for single bet.")
            
            chosen_outcome_obj = None
            chosen_outcome_index = -1 # Keep track of index if needed for Redis update
            for i, o_dict in enumerate(outcomes):
                if isinstance(o_dict, dict) and o_dict.get("name") == outcome_name_from_data:
                    chosen_outcome_obj = o_dict
                    chosen_outcome_index = i
                    break
            
            if chosen_outcome_obj is None:
                raise ValueError(f"Invalid outcome '{outcome_name_from_data}' for single bet in session {session_id}")
            ...
            new_bet_record = BetHistory( # type: ignore[call-arg]
                ...
                outcome_name=actual_outcome_name, stake=stake_val, payout=None,
                result="pending", status="open", timestamp=bet_time
            )
            db.session.add(new_bet_record)
            ...
            try:
                ...
                logger.error(f"Error checking/awarding badges (first_bet/high_roller/centurion) for user {user_obj.id} after single bet: {badge_err}")
            # --- End badge checks ---

            logger.info(f"DB: User {user_obj.username} balance updated to {user_obj.balance:.2f} and BetHistory for SINGLE bet created.")
            ...
            bet_entry_redis = {
                ...
                "stake": stake_val, "timestamp": bet_time.isoformat(), "is_coupon_bet": False
            }
            current_bets_list.append(bet_entry_redis)
            ...
            logger.info(f"Redis: Updated bets and outcomes for SINGLE bet session {session_id}")

        # --- Common Post-Bet Actions (Emit updates, Send Ack) ---
        player_usernames_in_session_bytes = redis_client.smembers(session_players_key) # Sync call
        # player_usernames_in_session is already a set of strings due to decode_responses=True
        player_usernames_in_session = set(player_usernames_in_session_bytes) # type: ignore[misc, iterable-item]
        all_player_details_updated = get_player_details_for_session(session_id, player_usernames_in_session)
        
        final_outcomes_json_raw = redis_client.hget(session_key, "outcomes") # Assuming sync
        ...
        logger.debug(f"Emitted 'session_update' to room session_{session_id} after bet by {user_obj.username} (Coupon: {is_coupon})")

        return {"success": True, "message": "Bet placeret!", "new_balance": user_obj.balance}

    except ValueError as ve:
        logger.warning(f"Place bet validation error for user {current_user.id if current_user.is_authenticated else 'UNKNOWN'}, data {data} from SID {sid}: {ve}")
        db.session.rollback()
        return {"success": False, "error": str(ve)}
    except redis.RedisError as re:
        logger.exception(f"Redis error in place_bet for user {current_user.id if current_user.is_authenticated else 'UNKNOWN'}, SID {sid}: {re}")
        db.session.rollback()
        return {"success": False, "error": "Databasefejl under behandling af bet."}
    except Exception as e:
        logger.exception(f"Error in place_bet for user {current_user.id if current_user.is_authenticated else 'UNKNOWN'}, data {data} from SID {sid}: {e}")
        db.session.rollback()
        return {"success": False, "error": f"Serverfejl: {str(e)}"}


@socketio.on("finish_bet")
def on_finish_bet(data): # Reverted to def
    sid = request.sid # type: ignore[attr-defined]
    if not current_user.is_authenticated: # Assuming admin/creator finishes bet
        logger.warning(f"Unauthenticated user (SID: {sid}) attempted to finish bet. Denying.")
        emit("server_error", {"msg": "Authentication required."}, to=sid)
        return

    requesting_user = current_user.id
    try:
        session_id = data.get("session_id", "").strip()
        admin_code_from_client = data.get("admin_code")
        winner_index_raw = data.get("winner_index")

        logger.info(f"Finish bet received: session='{session_id}', winner_idx='{winner_index_raw}', admin_trigger='{admin_code_from_client is not None}', by_user='{requesting_user}', sid='{sid}'")

        if not session_id: raise ValueError("Session ID missing")
        if winner_index_raw is None: raise ValueError("Winner index missing")
        winner_index = int(winner_index_raw)

        session_key = f"{SESSION_KEY_PREFIX}{session_id}"

        # --- Validate Session and Admin Code ---
        session_data_raw = redis_client.hmget(session_key, ["creator", "admin_code", "outcomes", "bets", "game_mode"]) # Assuming sync
        if not session_data_raw[0]: raise ValueError(f"Session '{session_id}' not found") # type: ignore[index, unsupported-operands]

        session_creator = session_data_raw[0].decode() # type: ignore[index, union-attr, unsupported-operands]
        session_admin_code = session_data_raw[1].decode() if session_data_raw[1] else None # type: ignore[index, union-attr, unsupported-operands]
        outcomes_json = session_data_raw[2] # type: ignore[index, unsupported-operands]
        current_bets_json = session_data_raw[3] # type: ignore[index, unsupported-operands]
        game_mode = session_data_raw[4].decode() if session_data_raw[4] else "unknown" # type: ignore[index, union-attr, unsupported-operands]

        # Admin code check (or if user is creator)
        if requesting_user != session_creator and (not session_admin_code or admin_code_from_client != session_admin_code) :
        ...
        if not outcomes_json: raise ValueError("Session outcomes not found in Redis.")
        outcomes = json.loads(outcomes_json)
        current_bets_list = json.loads(current_bets_json) if current_bets_json else []

        # --- Determine Winner ---
        winner_name = "Unknown Winner"
        winner_odds = 1.0
        if winner_index == -1: # Draw
            if game_mode == "sports" and len(outcomes) == 2:
                winner_name = "Draw" # Payout logic for draw bets needs to be defined.
            else: raise ValueError("Invalid 'Draw' winner index for this game mode")
        elif 0 <= winner_index < len(outcomes):
            winner_outcome_obj = outcomes[winner_index]
            winner_name = winner_outcome_obj.get("name", "Unnamed Winning Outcome")
            winner_odds = float(winner_outcome_obj.get("odds", 1.0))
            if winner_odds <= 0: winner_odds = 1.0 # Ensure positive odds
        else: raise ValueError(f"Invalid winner index '{winner_index}'")
        ...
        bettor_users_db = User.query.filter(User.username.in_(list(bettor_usernames))).all()
        ...
        for bet in current_bets_list:
            ...
            if not player_db_obj:
                logger.error(f"User '{user_id}' from bet not found in DB during finish_bet! Skipping.")
                continue
...
# --- Award 'first_win' badge ---
            if bet_outcome_index == winner_index: # Only for the winning bettor
                try:
                    ...
                    win_count = BetHistory.query.filter_by(user_id=player_db_obj.id, result='won').count()
                    if win_count == 1: # The current win is now committed, so count should be 1 if it's the first
                        award_badge(player_db_obj.id, 'Første Gevinst')
                except Exception as badge_err:
                    logger.error(f"Error checking/awarding 'first_win' badge for user {player_db_obj.id}: {badge_err}")
            # --- End badge check ---
# --- Award 'first_win' badge ---
            ...
            if bet_outcome_index == winner_index: # Only check for winners
                try:
                    ...
                    win_count = BetHistory.query.filter_by(user_id=player_db_obj.id, result='won').count()
                    if win_count == 1:
                        award_badge(player_db_obj.id, 'Første Gevinst') # Use exact name
                except Exception as badge_err:
                    logger.error(f"Error checking/awarding 'first_win' badge for user {player_db_obj.id}: {badge_err}")
            # --- End badge check ---
                player_db_obj.total_won = (player_db_obj.total_won or 0.0) + net_gain # type: ignore[possibly-unbound]
                player_db_obj.largest_win = max(player_db_obj.largest_win or 0.0, net_gain) # type: ignore[possibly-unbound]
                payouts_summary.append({"user": user_id, "payout": payout_amount, "stake": stake, "net_gain": net_gain}) # type: ignore[possibly-unbound]
            else: # Loser
                player_db_obj.losses = (player_db_obj.losses or 0) + 1
                player_db_obj.total_lost = (player_db_obj.total_lost or 0.0) + stake
                player_db_obj.largest_loss = max(player_db_obj.largest_loss or 0.0, stake)
        ...
        logger.info(f"DB: Player balances and stats updated for session {session_id} finish.")

        # --- Update Redis: Reset Pots, Clear Bets ---
        for o in outcomes: o["pot"] = 0.0 # Reset pots
        ...
        session_players_key = f"{SESSION_PLAYERS_KEY_PREFIX}{session_id}"
        # player_usernames_in_session is already a set of strings due to decode_responses=True
        player_usernames_in_session = set(redis_client.smembers(session_players_key)) # type: ignore[misc, iterable-item] # Assuming sync
        all_player_details_updated = get_player_details_for_session(session_id, player_usernames_in_session)

        payload = {
            "winner_index": winner_index, "winner_name": winner_name, "winner_odds": winner_odds,
            "payouts": payouts_summary, "losers": losses_summary,
            "players": all_player_details_updated,
            "outcomes": outcomes, "bets": []
        }
        actual_room_name = f"{SESSION_KEY_PREFIX.replace(':', '_')}{session_id}"
        emit("bet_finished", payload, to=actual_room_name) # type: ignore[call-arg]
        logger.info(f"Bet finished event emitted to {actual_room_name}. Winner: '{winner_name}'.")

    except ValueError as ve:
        logger.warning(f"Finish bet validation error for user {requesting_user}, data {data} from SID {sid}: {ve}")
        db.session.rollback()
        emit("server_error", {"msg": str(ve)}, to=sid)
    except redis.RedisError as re:
        logger.exception(f"Redis error in finish_bet for user {requesting_user}, SID {sid}: {re}")
        db.session.rollback()
        emit("server_error", {"msg": "Database error processing bet finish."}, to=sid)
    except Exception as e:
        logger.exception(f"Error in finish_bet for user {requesting_user}, data {data} from SID {sid}: {e}")
...
def on_settle_coupon_leg(data): # Reverted to def
    ...
    sid = request.sid # type: ignore[attr-defined]
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated user (SID: {sid}) attempted to settle coupon leg. Denying.")
    ...
    try:
        ...
        if not session_id: raise ValueError("Session ID missing")
        if not event_id_to_settle: raise ValueError("Event ID missing")
        # Ensure a winning outcome is provided
        if winning_outcome_name is None: raise ValueError("Winning outcome name missing") 

        logger.info(f"Settle coupon leg received: session='{session_id}', event='{event_id_to_settle}', winner='{winning_outcome_name}', by_user='{requesting_user_id}', sid='{sid}'")

        session_key = f"{SESSION_KEY_PREFIX}{session_id}"
        session_players_key = f"{SESSION_PLAYERS_KEY_PREFIX}{session_id}" # Define player key
        ...
        def decode_redis(val): return val.decode('utf-8') if isinstance(val, bytes) else val
        ...
        if not session_creator_id_raw: raise ValueError("Session creator ID missing in Redis data.")
        ...
        is_coupon = False
        if is_coupon_raw:
            try: is_coupon = json.loads(decode_redis(is_coupon_raw))
            except json.JSONDecodeError: pass
        ...
        if not selections_json_raw: raise ValueError("Coupon selections not found in Redis.")
        selections = json.loads(decode_redis(selections_json_raw))
        if not isinstance(selections, list): raise ValueError("Coupon selections data is invalid.")

        leg_updated = False
        leg_found = False
        all_legs_settled = True # Assume true initially, set to false if any pending found
            ...
            if leg.get("eventId") == event_id_to_settle:
                ...
                if leg.get("status") == "pending": # Only update pending legs
                    ...
                    if selected_outcome == winning_outcome_name:
                        ...
                        leg["status"] = "lost"
                    leg_updated = True
                    logger.info(f"Updated leg status for event {event_id_to_settle} in coupon {session_id} to '{leg['status']}'")
                else:
                     logger.warning(f"Attempted to re-settle already settled leg for event {event_id_to_settle} in coupon {session_id}. Current status: {leg.get('status')}")
                     return {"success": False, "error": "Denne del af kuponen er allerede afgjort."}
                # Break after finding and processing the specific leg
                break 
        
        if not leg_found:
             logger.warning(f"Event ID {event_id_to_settle} not found in coupon {session_id} selections.")
             return {"success": False, "error": "Det angivne event findes ikke i denne kupon."}
        ...
        if leg_updated:
            for leg_check in selections: # Need to iterate again to check all statuses
                ...
                if not isinstance(leg_check, dict): continue 
        ...
        redis_client.hset(session_key, "outcomes", json.dumps(selections)) # Assuming sync

        # --- If all legs are settled, process final results ---
        final_message = f"Status for valg på event {event_id_to_settle} er opdateret." 
        if all_legs_settled:
            logger.info(f"All legs settled for coupon {session_id}. Overall result: {overall_coupon_result}")
            ...
            try:
                ...
                if db_session:
                    ...
                    logger.error(f"Could not find GameSession {session_id} in DB for final settlement.")

                # 2. Fetch coupon bets from BetHistory
                # Ensure outcome_name matches exactly what was stored for coupon bets
                coupon_bets_db = BetHistory.query.filter_by(session_id=session_id, outcome_name="Kupon Væddemål", result="pending").all() 
                
                # 3. Fetch involved users
                bettor_user_ids = {bet.user_id for bet in coupon_bets_db}
                # Ensure users are fetched correctly by ID
                bettor_users_map = {user.id: user for user in User.query.filter(User.id.in_(list(bettor_user_ids))).all()}
                ...
                for bet in coupon_bets_db:
                    ...
                    if not user_obj:
                        logger.error(f"User ID {bet.user_id} from BetHistory {bet.id} not found in DB during coupon settlement.")
                        continue
                    ...
                    if overall_coupon_result == "won":
                        ...
                        user_obj.largest_win = max(user_obj.largest_win or 0.0, net_gain)
                        payouts_summary.append({"user": user_obj.username, "payout": final_payout, "stake": bet.stake, "net_gain": net_gain})
                    else: # Lost
                        user_obj.losses = (user_obj.losses or 0) + 1
                        user_obj.total_lost = (user_obj.total_lost or 0.0) + bet.stake
                        user_obj.largest_loss = max(user_obj.largest_loss or 0.0, bet.stake)
                        losses_summary.append({"user": user_obj.username, "lost": bet.stake})
# --- Award 'first_win' badge to all winning users on this coupon ---
                    if overall_coupon_result == "won":
                        for bet_record in coupon_bets_db: # Iterate through the bets that were part of this winning coupon
                            winning_user_obj = bettor_users_map.get(bet_record.user_id)
                            if winning_user_obj:
                                try:
                                    # Check if this is the user's first win overall
                                    win_count = BetHistory.query.filter_by(user_id=winning_user_obj.id, result='won').count()
                                    if win_count == 1: # The current win is now committed
                                        award_badge(winning_user_obj.id, 'Første Gevinst')
# Check for Big Win (Net gain > 200)
                                    if net_gain > 200: # type: ignore[possibly-unbound]
                                        award_badge(winning_user_obj.id, 'Kassen Ringer')
                                except Exception as badge_err:
                                    logger.error(f"Error checking/awarding 'first_win' badge for user {winning_user_obj.id} after coupon win: {badge_err}")
                    # --- End badge check ---
# --- Award 'first_win' badge to all winning users on this coupon ---
                        if overall_coupon_result == "won":
                            for bet_record in coupon_bets_db: # Iterate through the bets that were part of this winning coupon
                ...
                # 4. Commit DB changes
                db.session.commit()
                logger.info(f"DB: Final settlement processed for coupon {session_id}. Result: {overall_coupon_result}")
                ...
                final_message = f"Kupon {session_id} er fuldt afgjort som: {overall_coupon_result.upper()}!"
                 ...
                 logger.exception(f"CRITICAL ERROR during final settlement of coupon {session_id}: {final_settle_ex}")
                 db.session.rollback()
                 socketio.emit("server_error", {"msg": f"Kritisk fejl under endelig afgørelse af kupon {session_id}."}, to=f"session_{session_id}")
                 return {"success": False, "error": "Kritisk fejl under endelig afgørelse."}


        # --- Emit session update regardless (to show updated leg status) ---
        player_usernames_in_session_bytes = redis_client.smembers(session_players_key) # Assuming sync
        player_usernames_in_session = {uname.decode('utf-8') for uname in player_usernames_in_session_bytes} # type: ignore[misc, iterable-item]
        all_player_details_updated = get_player_details_for_session(session_id, player_usernames_in_session)
        final_bets_json_raw = redis_client.hget(session_key, "bets") # Assuming sync
        final_status_raw = redis_client.hget(session_key, "status") # Assuming sync, Get potentially updated status
        ...
        socketio.emit("session_update", emit_data, to=f"session_{session_id}") # type: ignore[call-arg]
        logger.debug(f"Emitted 'session_update' after settling leg for coupon {session_id}")

        return {"success": True, "message": final_message}

    except ValueError as ve:
        logger.warning(f"Settle coupon leg validation error for user {requesting_user_id}, data {data} from SID {sid}: {ve}")
        db.session.rollback() 
        return {"success": False, "error": str(ve)} 
    except redis.RedisError as re:
        logger.exception(f"Redis error in settle_coupon_leg for user {requesting_user_id}, SID {sid}: {re}")
        db.session.rollback()
        return {"success": False, "error": "Databasefejl under behandling."} 
    except Exception as e:
        logger.exception(f"Error in settle_coupon_leg for user {requesting_user_id}, data {data} from SID {sid}: {e}")
        db.session.rollback()
        return {"success": False, "error": f"Serverfejl: {str(e)}"}
        db.session.rollback()
        emit("server_error", {"msg": f"Server error finishing bet: {str(e)}"}, to=sid)
...
def on_send_session_message(data):
    ...
    sid = request.sid # type: ignore[attr-defined]
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated user (SID: {sid}) attempted to send session message. Denying.")
    ...
    try:
        ...
        if not session_id:
            logger.warning(f"User {username} (SID: {sid}) sent message without session_id.")
            return
        if not message_text:
            logger.debug(f"User {username} (SID: {sid}) sent empty message to session {session_id}.")
            return # Ignore empty messages

        # Verify user is part of the session room (important for security)
        actual_room_name = f"session_{session_id}" # Assuming this is how rooms are named
        ...
        if len(sanitized_message) > MAX_MSG_LENGTH:
            ...
            logger.info(f"User {username} message to session {session_id} truncated to {MAX_MSG_LENGTH} chars.")

        # Construct message object
        avatar_url = current_user.avatar_url # Accessing the property
        
        # Fetch user's badges to include icons
        user_badges = current_user.badges.limit(5).all() # Limit for performance/display
        badge_icons = [badge.icon for badge in user_badges if badge.icon] if user_badges else []
        ...
        logger.info(f"User {username} sent message to session {session_id}: '{sanitized_message[:50]}...'")

    except Exception as e:
        logger.exception(f"Error in on_send_session_message for user {username}, data {data}, SID {sid}: {e}")
...
def on_leave_room(data): # Reverted to def
    sid = request.sid # type: ignore[attr-defined]
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated user (SID: {sid}) attempted to leave room. Ignoring.")
...
def handle_subscribe_to_live_scores(data):
    ...
    sid = request.sid # type: ignore[attr-defined]
    if not current_user.is_authenticated:
        logger.warning(f"Unauthenticated user (SID: {sid}) attempted to subscribe to live scores.")
    ...
    if not isinstance(event_ids, list):
        logger.warning(f"Invalid event_ids format received from {current_user.id} (SID: {sid}): {event_ids}")
        return
        ...
        if isinstance(event_id, str) and event_id: # Basic validation
            ...
            try:
                ...
                subscribed_rooms.append(room_name)
                 ...
                 logger.error(f"Error joining room '{room_name}' for SID {sid}: {e_join}")
    
    if subscribed_rooms:
        logger.info(f"User {current_user.id} (SID: {sid}) subscribed to live score rooms: {subscribed_rooms}")
...
# potential rooms if the list of active events is manageable.
    ...
    try:
        ...
        if not room_name_from_client or not isinstance(room_name_from_client, str) or not room_name_from_client.startswith(SESSION_KEY_PREFIX.replace(":", "_")):
            logger.warning(f"Leave room failed (invalid room format '{room_name_from_client}') by {username} ({sid})")
        ...
        logger.info(f"User '{username}' ({sid}) attempting to leave room: {actual_room_name}")
        leave_room(actual_room_name) # SocketIO's leave_room
        emit("status", {"msg": f"Successfully left {actual_room_name}."}, to=sid) # To the user leaving

        # Remove player from Redis set for the session
        removed_count = redis_client.srem(session_players_key, username) # Assuming sync
        if removed_count > 0: # type: ignore[operator]
            logger.info(f"Redis: Removed '{username}' from player set for session '{session_id}'.")

            # Fetch updated list of players in session
            # remaining_player_usernames is already a set of strings due to decode_responses=True
            remaining_player_usernames = set(redis_client.smembers(session_players_key)) # type: ignore[misc, iterable-item] # Assuming sync
            remaining_player_details = get_player_details_for_session(session_id, remaining_player_usernames)
            ...
            logger.info(f"Notified room {actual_room_name} that {username} left and sent updated player list.")
        else:
            logger.warning(f"User '{username}' tried to leave session '{session_id}' but was not found in its Redis player set.")

    except redis.RedisError as re:
        logger.exception(f"Redis error in leave_room for user {username}, SID {sid}: {re}")
        emit("server_error", {"msg": "Database error processing your request."}, to=sid) # Generic for user
    except Exception as e:
        logger.exception(f"Error in leave_room for user {username}, data {data} from SID {sid}: {e}")
