...
try:
    ...
    print(
        "ADVARSEL: dotenv pakken mangler. Kør 'pip install python-dotenv' for at indlæse .env filer."
    )
...
_basedir_for_env = os.path.abspath(os.path.dirname(__file__))  # This is apps/backend/
# _project_root_for_env = os.path.normpath(os.path.join(_basedir_for_env, '..')) # No longer needed for .env path
dotenv_path = os.path.join(_basedir_for_env, ".env")  # Look for .env in apps/backend/

if (
    _dotenv_loaded and load_dotenv
):  # Check both _dotenv_loaded and that load_dotenv is truthy
    if os.path.exists(dotenv_path):
        loaded = load_dotenv(
            dotenv_path
        )  # Now load_dotenv is guaranteed to be a function
        if loaded:
            log.info(f"--- Indlæst miljøvariabler fra: {dotenv_path}")
        else:
            log.warning(
                f"--- Forsøgte at indlæse .env fra {dotenv_path}, men den kan være tom eller fejlbehæftet."
            )
    else:
        log.info(
            f"--- .env fil ikke fundet ved: {dotenv_path}. Bruger OS miljøvariabler eller defaults."
        )
elif _dotenv_loaded and not load_dotenv:  # This case should ideally not be reached
    log.error(
        "--- Kritisk fejl: _dotenv_loaded er True, men load_dotenv funktionen er ikke tilgængelig. Tjek dotenv installation/import."
    )
else:  # This covers the case where _dotenv_loaded is False
    log.info(
        "--- Springer .env indlæsning over fordi 'dotenv' pakken ikke er installeret."
    )
...
try:
    ...
    if not all(
        ...
        for d in ["app", "instance", "static", "templates"]
    ):
        log.warning(
            f"Autodetected PROJECT_ROOT '{PROJECT_ROOT}' ser muligvis forkert ud - mangler kerne-mapper ('app', 'instance', 'static', 'templates')."
        )

    INSTANCE_FOLDER_PATH: str = os.path.join(PROJECT_ROOT, "instance")
    STATIC_FOLDER_PATH: str = os.path.join(
        PROJECT_ROOT, "static"
    )  # Forventes udenfor 'app'
    ...
    log.info(
        f"Projektstier bestemt. Rod: {PROJECT_ROOT}, Instans: {INSTANCE_FOLDER_PATH}"
    )
...
class Config:
    ...
    LANGUAGES: List[str] = ["en", "da"]  # Added LANGUAGES
    BABEL_DEFAULT_LOCALE: str = "da"
    BABEL_DEFAULT_TIMEZONE: str = "Europe/Copenhagen"

    # === Security ===
    SECRET_KEY: str = os.environ.get("SECRET_KEY", "")
    if not SECRET_KEY:
        log.warning(
            "SIKKERHED ADVARSEL: SECRET_KEY er ikke sat i miljøvariabler! Genererer midlertidig nøgle (IKKE EGNET TIL PRODUKTION). Sæt den i .env filen."
        )
        SECRET_KEY = secrets.token_hex(24)  # Stærkere default nøgle
    WTF_CSRF_ENABLED: bool = True
    WTF_CSRF_TIME_LIMIT: Optional[int] = (
        None  # Slå tidsgrænse fra for CSRF tokens (anbefales)
    )
    ...
    try:
        ...
        _session_days = 7
        log.warning(
            f"Ugyldig SESSION_LIFETIME_DAYS env variabel. Bruger default: {_session_days} dage."
        )
    PERMANENT_SESSION_LIFETIME: timedelta = timedelta(days=max(1, _session_days))
    # Sikkerhedsindstillinger for session cookie
    _session_cookie_secure_env: Optional[str] = os.environ.get("SESSION_COOKIE_SECURE")
    if _session_cookie_secure_env is not None:
        _temp_session_secure: bool = _session_cookie_secure_env.lower() in [
            "true",
            "1",
            "yes",
            "on",
        ...
        # Default to True if not in DEBUG or TESTING mode
        _temp_session_secure: bool = not (DEBUG or TESTING)
    SESSION_COOKIE_SECURE: bool = _temp_session_secure
    SESSION_COOKIE_HTTPONLY: bool = True  # Altid True for sikkerhed
    SESSION_COOKIE_SAMESITE: str = (
        "Lax"  # Revert to Lax, proxy should handle same-origin appearance
    )
    SESSION_COOKIE_DOMAIN: Optional[str] = (
        None  # Allow cookie for any hostname used to access
    )
    if not SESSION_COOKIE_SECURE and not DEBUG and not TESTING:
        log.warning(
            "SIKKERHED ADVARSEL: SESSION_COOKIE_SECURE er False i et ikke-debug/test miljø. HTTPS er PÅKRÆVET!"
        )
    ...
    if _db_uri_env.startswith("postgres://"):
        SQLALCHEMY_DATABASE_URI: str = _db_uri_env.replace(
    ...
    SQLALCHEMY_ECHO: bool = DEBUG and (
        _sql_echo_env.lower() in ["true", "1", "yes", "on"]
    )
    ...
    try:
        THREADS_PER_PAGE: int = max(1, int(os.environ.get("THREADS_PER_PAGE", "15")))
    except ValueError:
        THREADS_PER_PAGE = 15
        log.warning("Ugyldig THREADS_PER_PAGE env var, bruger default 15.")
    try:
        POSTS_PER_PAGE: int = max(1, int(os.environ.get("POSTS_PER_PAGE", "10")))
    except ValueError:
        POSTS_PER_PAGE = 10
        log.warning("Ugyldig POSTS_PER_PAGE env var, bruger default 10.")
    ...
    DEFAULT_AVATAR: str = os.path.join(
        AVATAR_UPLOAD_SUBDIR, "default_avatar.png"
    )  # -> avatars/default_avatar.png

    # *** MAX AVATAR STØRRELSE DEFINERET HER ***
    try:
        MAX_AVATAR_SIZE_MB: int = max(
            1, int(os.environ.get("MAX_AVATAR_MB", "2"))
        )  # Default 2MB
    except ValueError:
        MAX_AVATAR_SIZE_MB = 2
        log.warning(
            f"Ugyldig MAX_AVATAR_MB env variabel, bruger default {MAX_AVATAR_SIZE_MB} MB."
        )
    # --------------------------------------------
    ...
    if not ADMIN_USERNAME or not ADMIN_PASSWORD:
        log.warning(
            "SIKKERHED: Admin credentials (ADMIN_USERNAME, ADMIN_PASSWORD) er ikke sat i miljøet. Admin login vil fejle."
        )
    elif ADMIN_PASSWORD == "change_this_password":  # Tjekker for usikker standard
        log.critical(
            "SIKKERHED KRITISK: Standard 'change_this_password' fundet for ADMIN_PASSWORD. SKIFT DEN OMGÅENDE i .env!"
        )

    # === Data Filstier (Absolutte stier bruges af data_access) ===
    PLAYER_FILE_PATH: str = os.path.join(INSTANCE_FOLDER_PATH, "players.json")
    INVITE_CODES_FILE_PATH: str = os.path.join(
        INSTANCE_FOLDER_PATH, "invite_codes.json"
    )
    SYSTEM_SETTINGS_FILE_PATH: str = os.path.join(
        INSTANCE_FOLDER_PATH, "system_settings.json"
    )  # <<< TILFØJET
    AUDIT_LOG_FILE_PATH: str = os.path.join(
        INSTANCE_FOLDER_PATH, "audit_log.json"
    )  # <<< TILFØJET FOR AUDIT LOGS
    WHOOSH_BASE: str = os.path.join(
        INSTANCE_FOLDER_PATH, "whoosh_index"
    )  # <<< TILFØJET FOR SEARCH INDEX
    ...
    try:
        INITIAL_BALANCE: float = float(os.environ.get("INITIAL_BALANCE", "1000.0"))
    except ValueError:
        INITIAL_BALANCE = 1000.0
        log.warning("Ugyldig INITIAL_BALANCE env var, bruger default 1000.0.")
    try:
        MIN_PASSWORD_LENGTH: int = max(
            8, int(os.environ.get("MIN_PASSWORD_LENGTH", "8"))
        )
    except ValueError:
        MIN_PASSWORD_LENGTH = 8
        log.warning("Ugyldig MIN_PASSWORD_LENGTH env var, bruger default 8.")
    try:
        MAX_ABOUT_ME_LENGTH: int = max(
            10, int(os.environ.get("MAX_ABOUT_ME_LENGTH", "500"))
        )
    except ValueError:
        MAX_ABOUT_ME_LENGTH = 500
        log.warning("Ugyldig MAX_ABOUT_ME_LENGTH env var, bruger default 500.")

    # === External APIs ===
    SPORTS_API_KEY: Optional[str] = os.environ.get("SPORTS_API_KEY")  # Kan være None
    SPORTS_API_BASE_URL: str = os.environ.get(
        "SPORTS_API_BASE_URL", "https://api.the-odds-api.com/v4"
    )
    try:
        EXTERNAL_API_TIMEOUT: int = max(
            1, int(os.environ.get("EXTERNAL_API_TIMEOUT", "10"))
        )  # Sekunder
    except ValueError:
        EXTERNAL_API_TIMEOUT = 10
        log.warning("Ugyldig EXTERNAL_API_TIMEOUT env var, bruger default 10s.")
    if not SPORTS_API_KEY and not (DEBUG or TESTING):
        log.warning(
            "External SPORTS_API_KEY er ikke sat! Sports relaterede funktioner virker muligvis ikke."
        )

    # === Two-Factor Authentication (2FA) ===
    HASH_BACKUP_CODES: bool = (
        os.environ.get("HASH_BACKUP_CODES", "True").lower() == "true"
    )
    try:
        NUM_BACKUP_CODES: int = max(5, int(os.environ.get("NUM_BACKUP_CODES", "10")))
    except ValueError:
        NUM_BACKUP_CODES = 10
        log.warning("Ugyldig NUM_BACKUP_CODES env var, bruger default 10.")
    try:
        LOW_BACKUP_CODE_THRESHOLD: int = max(
            1, int(os.environ.get("LOW_BACKUP_CODE_THRESHOLD", "3"))
        )
    except ValueError:
        LOW_BACKUP_CODE_THRESHOLD = 3
        log.warning("Ugyldig LOW_BACKUP_CODE_THRESHOLD env var, bruger default 3.")
    ...
    MAINTENANCE_MESSAGE: str = os.environ.get(
        "MAINTENANCE_MESSAGE", "Siden er midlertidigt nede for planlagt vedligehold."
    )
    # Behandler kommaseparerede IPs fra env var, fjerner whitespace
    MAINTENANCE_ALLOW_IPS: List[str] = [
        ip.strip()
        for ip in os.environ.get("MAINTENANCE_ALLOW_IPS", "").split(",")
        if ip.strip()
    ]
...
def log_config_values():
    ...
    log.info(
        f"Maintenance Mode:       {conf.MAINTENANCE_MODE} (Allowed IPs: {conf.MAINTENANCE_ALLOW_IPS or 'None'})"
    )
    ...
    db_display = (
        f"{db_type}://..."
        if "@" in db_uri or db_type.startswith("postgresql")
        else db_uri
    )
    ...
    # Session info
    log.info(
        f"Session Lifetime:       {getattr(conf, 'PERMANENT_SESSION_LIFETIME', 'Default')}"
    )
    log.info(f"Session Secure Cookie:  {conf.SESSION_COOKIE_SECURE}")

    # File Paths
    log.info(
        f"Player File Path:       ...{conf.PLAYER_FILE_PATH[-50:]}"
    )  # Vis kun en del af stien
    log.info(f"Invite Codes Path:      ...{conf.INVITE_CODES_FILE_PATH[-50:]}")
    log.info(
        f"System Settings Path:   ...{conf.SYSTEM_SETTINGS_FILE_PATH[-50:]}"
    )  # <<< TILFØJET
    log.info(
        f"Audit Log File Path:    ...{conf.AUDIT_LOG_FILE_PATH[-50:]}"
    )  # <<< TILFØJET FOR AUDIT LOGS
...
try:
    ...
    if os.path.exists(gitignore_path):
        with open(gitignore_path, "r", encoding="utf-8") as f:
            ...
            ignored = any(
                pattern in line.strip().lstrip("/")
                for line in lines
                for pattern in patterns_to_check
            )
            if not ignored:
                log.warning(
                    "SECURITY WARNING: Sensitive files/folders (instance/, .env, *.db) do NOT appear to be ignored in .gitignore. Add them!"
                )
    else:
        log.info(
            ".gitignore file not found at project root. Recommended to create one."
        )
