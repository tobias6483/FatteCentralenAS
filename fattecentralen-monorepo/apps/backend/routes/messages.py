...
from apps.backend.models import (  # Assuming User is your SQLAlchemy User model
    PrivateMessage,
    User,
)
from apps.backend.utils import (  # For consistent template context & notifications
    create_notification,
    get_common_context,
)
...
from flask_sqlalchemy.pagination import (  # For manual pagination object creation
    Pagination,
)
...
def view_message(message_id):
    ...
    thread_id_to_find = (
        initial_message.thread_id if initial_message.thread_id else initial_message.id
    )
    ...
    conversation_messages = (
        PrivateMessage.query.filter(
            or_(
                PrivateMessage.id == thread_id_to_find,
                PrivateMessage.thread_id == thread_id_to_find,
            )
        )
    ...
    if initial_message.recipient_id == current_user.id and not initial_message.is_read:
        ...
        try:
            db.session.commit()
            log.info(
                f"Marked message {initial_message.id} as read for user {current_user.id}"
            )
    ...
    thread_subject = (
        conversation_messages[0].subject if conversation_messages else "(Ingen emne)"
    )
...
@messages_bp.route(
    "/compose/<string:recipient_username>", methods=["GET", "POST"]
)  # Keep for direct compose link
@messages_bp.route(
    "/reply/<int:reply_to_message_id>", methods=["GET", "POST"]
)  # Add route for replying
@login_required
def compose_message(recipient_username=None, reply_to_message_id=None):
    ...
    if reply_to_message_id:
        ...
        if not (
            ...
            or original_message.recipient_id == current_user.id
        ):
    ...
    if (
        request.method == "GET" and is_reply and original_message
    ):  # Ensure original_message exists
        ...
        formatted_timestamp = (
            original_message.timestamp.strftime("%d. %b %Y, %H:%M")
            if original_message.timestamp
            else "Ukendt tidspunkt"
        )
        sender_username = (
            original_message.sender.username
            if original_message.sender
            else "Ukendt afsender"
        )
        # Basic escaping for the quoted body to prevent HTML injection if body contains HTML-like chars
        escaped_original_body = (
            (original_message.body or "")
            .replace("&", "&")
            .replace("<", "<")
            .replace(">", ">")
        )
    ...
    if form.validate_on_submit():
        recipient = User.query.filter(
            User.username.ilike(form.recipient_username.data)
        ).first()
        if not recipient:
            flash(
                f"Bruger '{form.recipient_username.data}' blev ikke fundet.", "danger"
            )
            ...
            try:
                ...
                db.session.commit()
                log.info(
                    f"Message {new_message.id} (Thread: {new_message.thread_id}) sent from {current_user.id} to {recipient.id}"
                )

                # --- Create Notification for Recipient ---
                try:
                    notification_msg = f'Du har modtaget en ny besked fra {current_user.username} med emnet: "{new_message.subject}"'
                    message_link = url_for(
                    ...
                    log.info(
                        f"Notification created for new message (ID: {new_message.id}) sent to user {recipient.id}"
                    )
                except Exception as notify_err:
                    log.error(
                        f"Failed to create notification for new message (ID: {new_message.id}) to user {recipient.id}: {notify_err}"
                    )
                # --- End Notification ---
                ...
                db.session.rollback()
                log.error(
                    f"Error sending private message from {current_user.id} to {recipient.id}: {e}"
                )
                flash("Fejl under afsendelse af besked.", "danger")

    context = get_common_context()
    context.update(
        {"title": "Skriv Ny Besked", "form": form, "active_page": "messages_compose"}
    )
    ...
    if action_taken:
        try:
            ...
            if message.sender_deleted and message.recipient_deleted:
                log.info(
                    f"Permanently deleting message ID {message.id} as both users soft-deleted."
                )
...
def api_search_recipients():
    ...
    users = (
        User.query.filter(User.username.ilike(f"%{query}%"), User.id != current_user.id)
        .limit(limit)
        .all()
    )
...
def api_contacts_status():
    ...
    try:
        ...
        sent_to_ids = (
            db.session.query(getattr(PrivateMessage, "recipient_id"))
            .filter(PrivateMessage.sender_id == current_user.id)
            .distinct()
        )

        received_from_ids = (
            db.session.query(getattr(PrivateMessage, "sender_id"))
        ...
        contact_ids = {
            row[0]
            for row in sent_to_ids.union(received_from_ids).all()
        ...
        contact_users = (
            User.query.filter(User.id.in_(list(contact_ids))).limit(limit).all()
            if contact_ids
            else []
        )
        ...
        if additional_users_needed > 0:
            ...
            other_online_users = (
                User.query.filter(
                    ...
                    - timedelta(minutes=15),  # Example: active in last 15 mins
                )
                .order_by(desc(User.last_seen))
                .limit(additional_users_needed)
                .all()
            )
        ...
        for user_id, user_obj in unique_users_dict.items():
            ...
            # Check the target user's privacy setting for showing online status
                ...
                user_obj.privacy_show_online_status
            ):  # This field was added to the User model
                privacy_filtered_users.append(user_obj)
            else:
                log.debug(
                    f"User {user_obj.id} has privacy_show_online_status=False. Not including in contacts status for user {current_user.id}."
                )
