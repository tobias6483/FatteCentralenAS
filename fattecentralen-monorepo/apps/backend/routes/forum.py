...
from typing import (  # Ensure Optional and Callable are imported
    Any,
    Callable,
    Dict,
    List,
...
from ..models import (  # User is imported here
    ForumCategory,
    ForumPost,
    ForumThread,
    User,
...
def _dummy_get_user_data_by_id(
...
def _dummy_get_user_data_batch(
    usernames: Union[List[str], Set[str]]
...
try:
    ...
    if "User" not in globals() or User is None:
        log.error(
            "User model from app.models is not available. Fallback User type is None."
        )
        User = None  # type: ignore


# Importer Forms
from ..forms import ForumEditPostForm, ForumNewThreadForm, ForumReplyForm
...
def api_get_forum_categories():
    ...
    try:
        ...
        if category_ids:
            ...
            subq_last_thread = (
                select(
                    ThreadAlias.category_id,
                    func.max(ThreadAlias.updated_at).label("max_updated_at"),
                )
                .group_by(ThreadAlias.category_id)
                .where(ThreadAlias.category_id.in_(category_ids))
                .subquery()
            )
            ...
            stmt_last_thread_info = select(
                ...
                ForumThread.updated_at.label(
                    "last_activity_at"
                ),  # Use thread's updated_at as last_activity
            ).join(
                subq_last_thread,
                (ForumThread.category_id == subq_last_thread.c.category_id)
                & (ForumThread.updated_at == subq_last_thread.c.max_updated_at),
            )
            last_threads_mapping = (
                db.session.execute(stmt_last_thread_info).mappings().all()
            )
            last_threads_by_cat_id = {
                row["category_id"]: dict(row) for row in last_threads_mapping
            }

            last_post_ids = [
                row.get("last_post_id")
                for row in last_threads_by_cat_id.values()
                if row.get("last_post_id")
            ]
            if last_post_ids:
                stmt_latest_posts = select(
                    ForumPost.id, ForumPost.author_username, ForumPost.created_at
        ...
        authors_details = (
            get_user_data_batch_func(usernames_to_fetch) if usernames_to_fetch else {}
        )
        ...
        for category in categories:
            ...
            if last_thread_info:
                ...
                last_post_detail = (
                    latest_posts_data.get(last_post_id) if last_post_id else None
                )
                author_username = None
                if last_post_detail:  # If there's a last post, use its author
                    author_username = last_post_detail.get("author_username")
                elif last_thread_info.get(
                    "thread_id"
                ):  # Fallback to thread author if no last post (e.g. thread just created)
                ...
                last_activity_obj = {
                    ...
                    "last_post_by_username": (
                        author_username if author_username else None
                    ),
                }
...
def api_get_category_threads(category_id: int):
    ...
    per_page = request.args.get(
        "per_page", current_app.config.get("THREADS_PER_PAGE_API", 20), type=int
    )
    ...
    try:
        ...
        pagination = db.paginate(
            threads_query, page=page, per_page=per_page, error_out=False
        )
        ...
        for thread in threads_on_page:
            ...
            if thread.last_post:
                last_post_data = {
                    ...
                    "created_at": (
                        thread.last_post.created_at.isoformat()
                        if thread.last_post.created_at
                        else None
                    ),
                }
            ...
            thread_list_data.append(
                {
                    ...
                    ),  # Timestamp of the latest post or thread update
        ...
        log.exception(
            f"API Error: Database error fetching threads for category {category_id}."
        )
        return jsonify({"error": "Database error", "details": str(e)}), 500
    except Exception as e:
        log.exception(
            f"API Error: Unexpected error fetching threads for category {category_id}."
        )
        return jsonify({"error": "Unexpected server error", "details": str(e)}), 500
...
def api_get_thread_posts(thread_id: int):
    ...
    page = request.args.get("page", 1, type=int)
    per_page = request.args.get(
        "per_page", current_app.config.get("POSTS_PER_PAGE_API", 15), type=int
    )
    per_page = max(1, min(per_page, 50))  # Clamp per_page
    ...
    try:
        ...
        posts_query = (
            ...
            )  # Typically posts are ordered chronologically
        )

        pagination = db.paginate(
            posts_query, page=page, per_page=per_page, error_out=False
        )
        ...
        author_usernames = {
            post.author_username for post in posts_on_page if post.author_username
        }
        authors_details_map = (
            get_user_data_batch_func(author_usernames) if author_usernames else {}
        )
            ...
            post_list_data.append(
                {
                    ...
                    "user_avatar_url": author_details.get(
                        "avatar_url"
                    ),  # If fetching full user details
        ...
        log.exception(
            f"API Error: Database error fetching posts for thread {thread_id}."
        )
        return jsonify({"error": "Database error", "details": str(e)}), 500
    except Exception as e:
        log.exception(
            f"API Error: Unexpected error fetching posts for thread {thread_id}."
        )
        return jsonify({"error": "Unexpected server error", "details": str(e)}), 500
...
def api_create_thread_post(thread_id: int):
    ...
    try:
        ...
        if not firebase_user_data:  # Check if firebase_user itself is None or empty
            log.error(
                f"API Create Post: firebase_user not found in flask_g for thread {thread_id}."
            )
            return (
                jsonify(
                    {
                        "error": "Authentication context error: Firebase user data not found."
                    }
                ),
                401,
            )
        ...
        if not firebase_uid:
            log.error(
                f"API Create Post: Firebase UID not found in firebase_user data for thread {thread_id}. Data: {firebase_user_data}"
            )
            return (
                jsonify(
                    {"error": "Authentication error: Firebase UID not found in token."}
                ),
                401,
            )
        ...
        if not author or not author.username:
            log.warning(
                f"API Create Post: Local user not found or username missing for Firebase UID {firebase_uid} in thread {thread_id}."
            )
            return (
                jsonify(
                    {
                        "error": "User profile not found or incomplete. Cannot determine author."
                    }
                ),
                403,
            )
        ...
        new_post = ForumPost(
            thread_id=thread_id, author_username=author_username, body=post_body
        )
        ...
        try:
            ...
            log.error(
                f"API Create Post: Failed to index new post {new_post.id} for thread {thread_id}: {index_err}"
            )

        log.info(
            f"API: User '{author_username}' (Firebase UID: {firebase_uid}) created post ID {new_post.id} in thread {thread_id}."
        )
            ...
            jsonify(
                {
                    ...
                    "created_at": (
                        new_post.created_at.isoformat() if new_post.created_at else None
                    ),
        ...
        db.session.rollback()
        log.exception(
            f"API Error: Unexpected error creating post in thread {thread_id}."
        )
        return jsonify({"error": "Unexpected server error", "details": str(e)}), 500
...
def api_create_category_thread(category_id: int):
    ...
    try:
        ...
        if not firebase_user_data:
            log.error(
                f"API Create Thread: firebase_user not found in flask_g for category {category_id}."
            )
            return (
                jsonify(
                    {
        ...
        if not firebase_uid:
            log.error(
                f"API Create Thread: Firebase UID not found in firebase_user data for category {category_id}. Data: {firebase_user_data}"
            )
            return (
                jsonify(
                    {"error": "Authentication error: Firebase UID not found in token."}
        ...
        if not author or not author.username:
            log.warning(
                f"API Create Thread: Local user not found or username missing for Firebase UID {firebase_uid} in category {category_id}."
            )
            return (
                jsonify(
                    {
        ...
        first_post = ForumPost(
            thread_id=new_thread.id, author_username=author_username, body=body
        )
        ...
        try:
            ...
            log.error(
                f"API Create Thread: Failed to index first post {first_post.id} for new thread {new_thread.id}: {index_err}"
            )

        log.info(
            f"API: User '{author_username}' (Firebase UID: {firebase_uid}) created thread ID {new_thread.id} ('{new_thread.title}') in category {category_id} with first post ID {first_post.id}."
        )
        ...
        db.session.rollback()
        log.exception(
            f"API Error: Database error creating thread in category {category_id}."
        )
        return jsonify({"error": "Database error", "details": str(e)}), 500
    except Exception as e:
        db.session.rollback()
        log.exception(
            f"API Error: Unexpected error creating thread in category {category_id}."
        )
        return jsonify({"error": "Unexpected server error", "details": str(e)}), 500
...
def api_update_thread(thread_id: int):
    ...
    try:
        ...
        if not firebase_user_data:
            log.error(
                f"API Update Thread: firebase_user not found in flask_g for thread {thread_id}."
            )
        ...
        if not firebase_uid:
            log.error(
                f"API Update Thread: Firebase UID not found in token for thread {thread_id}."
            )
            return jsonify({"error": "Authentication error: Firebase UID missing"}), 401

        if User is None:  # ADDED: Check if User model is available
            log.critical(
                "API Update Thread: User model is not available. This is a critical configuration error."
            )
            return (
                jsonify(
                    {"error": "Server configuration error: User model unavailable."}
                ),
                500,
            )
        ...
        if not acting_user:
            log.warning(
                f"API Update Thread: Local user not found for Firebase UID {firebase_uid} for thread {thread_id}."
            )
        ...
        if thread.author_username != acting_user.username and not acting_user.is_admin:
            log.warning(
                f"API Update Thread: User '{acting_user.username}' (Firebase UID: {firebase_uid}) is not authorized to update thread {thread_id} (author: '{thread.author_username}')."
            )
            return (
                jsonify(
                    {
                        "error": "Forbidden: You are not authorized to update this thread."
                    }
                ),
                403,
            )
        ...
        log.info(
            f"API: Thread ID {thread.id} title updated to '{new_title}' by user '{acting_user.username}' (Firebase UID: {firebase_uid})."
        )
...
def api_delete_thread(thread_id: int):
    ...
    try:
        ...
        if not firebase_user_data:
            log.error(
                f"API Delete Thread: firebase_user not found in flask_g for thread {thread_id}."
            )
            return jsonify({"error": "Authentication context error"}), 401
        ...
        if not firebase_uid:
            log.error(
                f"API Delete Thread: Firebase UID not found in token for thread {thread_id}."
            )
            return jsonify({"error": "Authentication error: Firebase UID missing"}), 401
        ...
        if not acting_user:
            log.warning(
                f"API Delete Thread: Local user not found for Firebase UID {firebase_uid} for thread {thread_id}."
            )
            return jsonify({"error": "User profile not found"}), 403
        ...
        if thread.author_username != acting_user.username and not acting_user.is_admin:
            log.warning(
                f"API Delete Thread: User '{acting_user.username}' (Firebase UID: {firebase_uid}) is not authorized to delete thread {thread_id} (author: '{thread.author_username}')."
            )
            return (
                jsonify(
                    {
                        "error": "Forbidden: You are not authorized to delete this thread."
                    }
                ),
                403,
            )
        ...
        posts_to_delete = db.session.scalars(
            select(ForumPost).filter_by(thread_id=thread_id)
        ).all()
        ...
        for post_id_to_remove in post_ids_for_index_removal:
            try:
                ...
                log.error(
                    f"API Delete Thread: Failed to remove post {post_id_to_remove} from index for deleted thread {thread_id}: {index_err}"
                )
        ...
        log.info(
            f"API: Thread ID {thread_id} and its posts deleted by user '{acting_user.username}' (Firebase UID: {firebase_uid}). Category {category_id} counts need update."
        )

        return (
            jsonify(
                {"message": "Thread and all associated posts deleted successfully."}
            ),
            200,
        )
...
@forum_api_bp.route(
    "/threads/&lt;int:thread_id&gt;/posts/&lt;int:post_id&gt;", methods=["PUT"]
)
@firebase_token_required
def api_update_thread_post(thread_id: int, post_id: int):
    ...
    try:
        ...
        if post.thread_id != thread_id:
            log.warning(
                f"API Update Post: Post {post_id} does not belong to thread {thread_id}."
            )
            return jsonify({"error": "Post not found in this thread"}), 404

        thread = db.session.get(
            ForumThread, thread_id
        )  # Fetch thread for context if needed (e.g. locked status)
        if not thread:  # Should not happen if post.thread_id is valid, but good check
            return jsonify({"error": "Thread not found"}), 404

        if thread.is_locked and not (
            getattr(flask_g, "firebase_user_is_admin", False)
        ):  # Allow admins to edit posts in locked threads
        ...
        if not firebase_user_data:
            log.error(
                f"API Update Post: firebase_user not found in flask_g for post {post_id}."
            )
            return jsonify({"error": "Authentication context error"}), 401
        ...
        if not firebase_uid:
            log.error(
                f"API Update Post: Firebase UID not found in token for post {post_id}."
            )
            return jsonify({"error": "Authentication error: Firebase UID missing"}), 401
        ...
        if not acting_user:
            log.warning(
                f"API Update Post: Local user not found for Firebase UID {firebase_uid} for post {post_id}."
            )
            return jsonify({"error": "User profile not found"}), 403
        ...
        if post.author_username != acting_user.username and not acting_user.is_admin:
            log.warning(
                f"API Update Post: User '{acting_user.username}' (Firebase UID: {firebase_uid}) is not authorized to update post {post_id} (author: '{post.author_username}')."
            )
            return (
                jsonify(
                ...
                jsonify(
                    {
                        ...
                        "updated_at": (
                            ...
                            if post.updated_at
                            else post.created_at.isoformat()
                        ),
                    }
                ),
                200,
        ...
        try:
            ...
            log.error(
                f"API Update Post: Failed to update index for post {post.id}: {index_err}"
            )

        log.info(
            f"API: Post ID {post.id} updated by user '{acting_user.username}' (Firebase UID: {firebase_uid})."
        )
        ...
        db.session.rollback()
        log.exception(
            f"API Error: Database error updating post {post_id} in thread {thread_id}."
        )
        return jsonify({"error": "Database error", "details": str(e)}), 500
    except Exception as e:
        db.session.rollback()
        log.exception(
            f"API Error: Unexpected error updating post {post_id} in thread {thread_id}."
        )
        return jsonify({"error": "Unexpected server error", "details": str(e)}), 500


@forum_api_bp.route(
    "/threads/&lt;int:thread_id&gt;/posts/&lt;int:post_id&gt;", methods=["DELETE"]
)
@firebase_token_required
def api_delete_thread_post(thread_id: int, post_id: int):
    ...
    try:
        ...
        if post.thread_id != thread_id:
            log.warning(
                f"API Delete Post: Post {post_id} does not belong to thread {thread_id}."
            )
            return jsonify({"error": "Post not found in this thread"}), 404
        ...
        if not thread:
            ...
            log.error(
                f"API Delete Post: Thread {post.thread_id} for post {post_id} not found. Data inconsistency?"
            )
        ...
        if thread.first_post_id == post.id:
            log.warning(
                f"API Delete Post: Attempt to delete first post {post_id} of thread {thread.id}. Denied."
            )
            return (
                jsonify(
                    {
                        "error": "Cannot delete the first post of a thread. Delete the thread instead."
                    }
                ),
                403,
            )
        ...
        if not firebase_user_data:
            log.error(
                f"API Delete Post: firebase_user not found in flask_g for post {post_id}."
            )
            return jsonify({"error": "Authentication context error"}), 401
        ...
        if not firebase_uid:
            log.error(
                f"API Delete Post: Firebase UID not found in token for post {post_id}."
            )
            return jsonify({"error": "Authentication error: Firebase UID missing"}), 401
        ...
        if not acting_user:
            log.warning(
                f"API Delete Post: Local user not found for Firebase UID {firebase_uid} for post {post_id}."
            )
            return jsonify({"error": "User profile not found"}), 403
        ...
        if post.author_username != acting_user.username and not acting_user.is_admin:
            log.warning(
                f"API Delete Post: User '{acting_user.username}' (Firebase UID: {firebase_uid}) is not authorized to delete post {post_id} (author: '{post.author_username}')."
            )
            return (
                jsonify(
        ...
        thread.post_count = max(
            0, (thread.post_count or 1) - 1
        )  # Decrement, ensure not negative

        if is_last_post_in_thread:
            if (
                thread.post_count == 0
            ):  # This was the only post, and it's being deleted (should be handled by first_post_id check, but as a safeguard)
        ...
        db.session.add(
            thread
        )  # Add thread to session to save changes to post_count, last_post_id, updated_at
        db.session.commit()
        ...
        try:
            ...
            log.error(
                f"API Delete Post: Failed to remove post {post_id_for_index} from index: {index_err}"
            )

        log.info(
            f"API: Post ID {post_id_for_index} deleted by user '{acting_user.username}' (Firebase UID: {firebase_uid}). Thread {thread.id} counters updated."
        )
        ...
        db.session.rollback()
        log.exception(
            f"API Error: Database error deleting post {post_id} in thread {thread_id}."
        )
        return jsonify({"error": "Database error", "details": str(e)}), 500
    except Exception as e:
        db.session.rollback()
        log.exception(
            f"API Error: Unexpected error deleting post {post_id} in thread {thread_id}."
        )
        return jsonify({"error": "Unexpected server error", "details": str(e)}), 500
...
def fetch_user_details(usernames: Set[str]) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        from flask import (  # Local import for context-dependent calls
            current_app,
            url_for,
        )
        ...
        log.exception(
            "Could not generate default avatar URL in fetch_user_details. Using hardcoded fallback."
        )
    ...
    try:
        ...
        log.debug(
            f"Batch fetch (get_user_data_batch_func) returned {len(user_object_map)} entries."
        )
        ...
        for original_case_username, user_obj_or_dict in user_object_map.items():
            ...
            if User and isinstance(user_obj_or_dict, User):  # Check User is not None
                ...
                log.debug(
                    f"Processing User object for: '{user_obj.username}' (original request: '{original_case_username}')"
                )
                user_avatar_url = getattr(
                    user_obj, "avatar_url", default_avatar_fallback_url
                )
                if not user_avatar_url:
                    user_avatar_url = default_avatar_fallback_url
                ...
                processed_data[original_case_username] = {
                    ...
                    "join_date": (
                        registration_date_obj.isoformat()
                        if registration_date_obj
                        else None
                    ),
                ...
                log.warning(
                    f"No valid User object found for '{original_case_username}'. Using default details."
                )
                processed_data[original_case_username][
                    "avatar_url"
        ...
        log.exception(
            f"CRITICAL error during fetch_user_details processing {len(usernames)} usernames: {e}"
        )
        for name in usernames:
            if name not in processed_data:
                processed_data[name] = {
                    ...
                    "post_count": 0,
                    "join_date": None,
                    "avatar_url": default_avatar_fallback_url,
                    "is_online": False,
                }
    ...
    try:
        ...
        if not category_ids:
            return render_template(
                "forum/index.html", categories_data=category_render_data
            )
        ...
        log.debug(
            f"Step 2 DONE: Found last thread info for {len(last_threads_by_cat_id)} categories."
        )
        ...
        if last_post_ids:
            ...
            for post_row in post_results:
                ...
                if post_row.get("author_username"):
                    usernames_to_fetch.add(post_row["author_username"])
        log.debug(
            f"Step 3 DONE: Fetched details for {len(latest_posts_data)} posts. Usernames to fetch: {usernames_to_fetch}"
        )
        ...
        log.debug(
            f"Step 4 DONE: fetch_user_details returned {len(authors_details)} results."
        )
        ...
        for category in categories:
            ...
            if last_thread_info:
                ...
                if last_post_id:
                    ...
                    if last_post_detail:
                        last_post_author_username = last_post_detail.get(
                            "author_username"
                        )
                        if last_post_author_username:
                            last_author_detail = authors_details.get(
            ...
            category_render_data.append(
                {
                    ...
                    "last_thread_id": (
                        last_thread_info.get("id") if last_thread_info else None
                    ),
                    "last_thread_title": (
                        last_thread_info.get("title") if last_thread_info else None
                    ),
                    "last_post_created_at": (
                        last_post_detail.get("created_at") if last_post_detail else None
                    ),
                    "last_post_author_username": (
                        last_post_detail.get("author_username")
                        if last_post_detail
                        else None
                    ),
                    "last_post_author_avatar_url": (
                        last_author_detail.get("avatar_url")
                        if last_author_detail
                        else None
                    ),
                }
            )
        ...
        log.exception(
            f"Unexpected error preparing forum index data (at line ~{tb_lineno}): {e}"
        )
...
def view_category(slug: str):
    ...
    try:
        ...
        if not category:
            abort(404, "Kategori ikke fundet")
        ...
        if thread_ids:
            stmt_counts = (
                select(ForumPost.thread_id, func.count(ForumPost.id))
                .where(ForumPost.thread_id.in_(thread_ids))
                .group_by(ForumPost.thread_id)
            )
            thread_counts = {
                tid: count for tid, count in db.session.execute(stmt_counts).all()
            }
    ...
    return render_template(
        ...
        all_authors_data=all_authors_details,
...
def view_thread(thread_id: int):
    ...
    try:
        ...
        if not thread:
            abort(404, "Tråd ikke fundet.")

        reply_form = ForumReplyForm()

        if reply_form.validate_on_submit():
            if not current_user.is_authenticated:
                flash("Log ind for at svare.", "warning")
                return redirect(url_for("auth.login_route", next=request.url))
            if thread.is_locked:
                flash("Tråden er låst.", "warning")
                return redirect(
                    url_for("forum.view_thread", thread_id=thread.id, page=page)
                )
            try:
                author_name = getattr(current_user, "username", None)
                if not author_name:
                    raise ValueError("Current user object missing username.")
                ...
                try:
                    ...
                    log.error(
                        f"Failed to index new reply post {new_post.id}: {index_err}"
                    )
                log.info(
                    f"User '{author_name}' replied to thread {thread.id} (Post ID: {new_post.id})."
                )
                try:
                    if current_user.post_count == 10:
                        if callable(award_badge_func):
                            award_badge_func(current_user.id, "Forum Skribent")
                        else:
                            log.error(
                                "award_badge_func not available. Skipping badge check."
                            )
                except NameError:
                    log.error(
                        "award_badge_func not available (NameError). Skipping badge check."
                    )
                except Exception as badge_err:
                    log.error(
                        f"Error checking/awarding badge for user {current_user.id}: {badge_err}"
                    )
                ...
                return redirect(
                    url_for(
                        ...
                        thread_id=thread.id,
                        page=last_page,
                        _anchor=f"post-{new_post.id}",
                    )
                )
            except (SQLAlchemyError, ValueError, Exception) as e_save:
                db.session.rollback()
                log.exception(
                    f"Error saving reply by '{getattr(current_user, 'username', 'N/A')}' to thread {thread.id}: {e_save}"
                )
        ...
        pagination = db.paginate(
            posts_query, page=page, per_page=posts_per_page, error_out=False
        )
        posts_on_page: List[ForumPost] = pagination.items
        usernames_on_page = {
            p.author_username for p in posts_on_page if p.author_username
        }
        ...
        flash("Databasefejl ved hentning af indlæg.", "danger")
        abort(500)
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            try:
                ...
                log.error(f"Failed to index new post {first_post.id}: {index_err}")
            log.info(
                f"User '{author_name}' created thread ID {new_thread.id} in category '{category.slug}'."
            )
...
def edit_post(post_id: int):
    log.debug(
        f"Accessing edit post form for post ID: {post_id} by user '{getattr(current_user,'username','N/A')}'"
    )
    post_stmt = (
        select(ForumPost)
        .options(
    ...
    if form.validate_on_submit():
        log.debug(
            f"Edit post form validated for post {post_id} by '{current_username}'."
        )
        body_changed = post.body != form.body.data
        try:
            post.body = form.body.data
            if hasattr(post, "last_edited_by"):
                post.last_edited_by = current_username
            if hasattr(post, "last_edited_at"):
                post.last_edited_at = func.now()
            db.session.add(post)
            db.session.commit()
            if body_changed:
                try:
                    ...
                    log.error(
                        f"Failed to update index for edited post {post.id}: {index_err}"
                    )
            ...
            db.session.rollback()
            log.exception("DB error updating post.")
            flash("Databasefejl ved opdatering.", "danger")
        except Exception as e:
            db.session.rollback()
            log.exception("Unexpected error updating post.")
            flash("Systemfejl ved opdatering.", "danger")
...
def delete_post(post_id: int):
    log.debug(
        f"Attempting delete post ID: {post_id} by user '{getattr(current_user,'username','N/A')}'."
    )
    ...
    if (
        post.thread and post.thread.first_post_id == post.id
    ):  # Check if post.thread is not None
    ...
    try:
        ...
        try:
            ...
            log.error(f"Failed to remove post {post_id} from index: {index_err}")
        log.info(f"Post ID {post_id} deleted by '{current_username}'.")
        flash("Indlægget er slettet.", "success")
    except SQLAlchemyError as e:
        db.session.rollback()
        log.exception("DB error deleting post.")
        flash("Databasefejl ved sletning.", "danger")
    except Exception as e:
        db.session.rollback()
        log.exception("Unexpected error deleting post.")
        flash("Systemfejl ved sletning.", "danger")
    return redirect(url_for("forum.view_thread", thread_id=thread_id))
    ...
    try:
        ...
        for post_mapping in latest_posts_raw:
            ...
            if created_at_dt:
                try:
                    if callable(
                        dt_filter_func_global
                    ):  # Check if the global function is callable
                        relative_time = dt_filter_func_global(
                            created_at_dt, relative=True, default="dato-fejl"
                        )
                    else:
                        log.error(
                            "dt_filter_func_global not available. Cannot format relative time."
                        )
                        relative_time = created_at_dt.strftime("%d.%m.%Y %H:%M")
                except Exception as time_err:
                    log.error(
                        f"Error formatting relative time for post {post.get('id')}: {time_err}"
                    )
                    relative_time = created_at_dt.strftime("%d.%m.%Y %H:%M")
...
def search():
    ...
    if not query:
        ...
        try:
            search_result_data, total_hits = search_posts(
                query, page=page, per_page=posts_per_page
            )
            ...
            temp_results = [
                posts_map.get(int(hit["id"]))
                for hit in search_result_data
                if hit.get("id") and int(hit["id"]) in posts_map
            ]
            search_results_objects = [p for p in temp_results if p is not None]

            usernames_on_page = {
                p.author_username
                for p in search_results_objects
                if p and p.author_username
            }
            if usernames_on_page:
                authors_details = get_user_data_batch_func(usernames_on_page)
        except Exception as e:
    ...
    return render_template(
        ...
        authors_data=authors_details,
        title=f"Søgeresultater for '{query}'" if query else "Søg i Forum",
        current_page=page,
