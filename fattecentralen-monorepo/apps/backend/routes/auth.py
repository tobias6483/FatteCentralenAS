...
from flask_jwt_extended import (  # Added for JWT
    create_access_token,
...
from ..models import FailedLoginAttempt, InviteCode, PasswordResetRequest
from ..models import User as DBUser  # ADDED PasswordResetRequest

# from ..utils import get_user_data_by_id
from ..utils import firebase_token_required, generate_backup_codes, is_safe_url
        ...
        jsonify(
            {
                ...
                "user": {
                    ...
                    ),  # Assuming email might not always be present
...
def register_or_sync_firebase_user():
    ...
    firebase_display_name = firebase_user_data.get(
        "name"
    )  # Firebase 'name' is often display name

    log.info(
        f"Attempting to register/sync Firebase user. UID: {firebase_uid}, Email: {firebase_email}"
    )

    if not request.is_json:
        log.warning(
            f"Register/Sync Firebase user ({firebase_uid}) failed: Non-JSON request."
        )
    ...
    if not data:  # data can be an empty JSON object {} if only token is needed for sync
        log.debug(
            f"Register/Sync Firebase user ({firebase_uid}): Empty JSON payload, proceeding with sync logic if user exists."
        )
    ...
    try:
        ...
        if existing_user:
            ...
            log.info(
                f"Firebase user {firebase_uid} (local: {existing_user.username}) already exists. Syncing info."
            )
            ...
            if (
                firebase_email
                and firebase_email_verified
                and existing_user.email != firebase_email
            ):
                ...
                other_user_with_new_email = DBUser.query.filter(
                    DBUser.email == firebase_email, DBUser.id != existing_user.id
                ).first()
                if not other_user_with_new_email:
                    log.info(
                        f"Updating email for user {existing_user.username} from '{existing_user.email}' to '{firebase_email}' based on verified Firebase email."
                    )
                    existing_user.email = firebase_email
                else:
                    log.warning(
                        f"Could not update email for user {existing_user.username} to '{firebase_email}' as it's already in use by user {other_user_with_new_email.username}."
                    )
            ...
            log.info(
                f"Firebase user {firebase_uid} not found locally. Attempting new local profile creation."
            )
            invite_code_req = data.get("invite_code", "").strip()
            username_payload = data.get("username", "").strip()

            errors = {}
            if not invite_code_req:
                errors["invite_code"] = "Invite Kode er påkrævet for ny profil."
            ...
            if not final_username and firebase_display_name:
                final_username = firebase_display_name.replace(
                    " ", "_"
                ).lower()  # Basic derivation
            if not final_username and firebase_email:
                final_username = (
                    firebase_email.split("@")[0].replace(".", "_").lower()
                )  # Basic derivation

            if not final_username:  # Still no username
                errors["username"] = (
                    "Brugernavn kunne ikke bestemmes og blev ikke angivet."
                )
            elif not re.fullmatch(r"^[A-Za-z0-9_]+$", final_username):
                errors["username"] = (
                    "Brugernavn (angivet eller afledt) må kun indeholde bogstaver, tal og underscore."
                )

            if errors:
                log.warning(
                    f"New local profile for Firebase UID {firebase_uid} failed validation: {errors}"
                )
                return jsonify({"errors": errors}), 400

            # Validate derived/provided username for uniqueness (case-insensitive)
            existing_user_by_username = DBUser.query.filter(
                func.lower(DBUser.username) == func.lower(final_username)
            ).first()
            if existing_user_by_username:
                log.warning(
                    f"New local profile for Firebase UID {firebase_uid}: Username '{final_username}' already taken."
                )
                return (
                    jsonify(
                        {
                            "errors": {
                                "username": f"Brugernavnet '{final_username}' er allerede taget. Angiv et andet i payload."
                            }
                        }
                    ),
                    409,
            ...
            if not invite_code_obj or not invite_code_obj.is_valid:
                ...
                log.warning(
                    f"New local profile for Firebase UID {firebase_uid}: Invalid invite code '{invite_code_req}'."
                )
                return jsonify({"errors": {"invite_code": msg}}), 400

            # Create new local user
            now_utc = datetime.now(timezone.utc)
            default_avatar_config = current_app.config.get(
                "DEFAULT_AVATAR", "avatars/default_avatar.png"
            )
            default_avatar_filename = (
                default_avatar_config.split("/")[-1]
                if "/" in default_avatar_config
                else default_avatar_config
            )
            ...
            new_user = DBUser(
                ...
                email=(
                    firebase_email if firebase_email_verified else None
                ),  # Only store verified Firebase email
            ...
            db.session.commit()
            log.info(
                f"New local profile (ID: {new_user.id}, Username: {new_user.username}) created and linked for Firebase UID {firebase_uid}. Invite code '{invite_code_req}' consumed."
            )
        ...
        log.warning(
            f"Register/Sync Firebase user ({firebase_uid}) failed: Invalid JSON received."
        )
        return jsonify({"error": "Ugyldigt JSON format."}), 400
    except Exception as e:
        db.session.rollback()
        log.exception(
            f"Unexpected error during Firebase user register/sync (UID: {firebase_uid}): {e}"
        )
...
def login_route():
    ...
    if request.method == "POST":
        if request.is_json:
            ...
            ):  # Validates CSRF if present and form fields
                all_errors = form.errors
                is_csrf_failure = bool(all_errors.get("csrf_token"))
                err_msg = (
                    "Login fejlede: Ugyldig eller manglende sikkerhedstoken."
                    if is_csrf_failure
                    else "Formularvalideringsfejl."
                )
                log.warning(
                    f"API Login failed: Form validation failed. CSRF-related: {is_csrf_failure}. Errors: {all_errors}"
                )
            ...
            remember_me_from_json = (
                form.remember_me.data
            )  # Get 'remember' from the form populated by JSON
            log.info(
                f"Processing API JSON login for user: '{username_req}' (Remember: {remember_me_from_json})"
            )
            # ... (rest of JSON login logic from original code, adapted) ...
            user_obj = DBUser.query.filter(
                func.lower(DBUser.username) == func.lower(username_req)
            ).first()
            if user_obj and bcrypt.check_password_hash(
                user_obj.password_hash, password_req
            ):
                ...
                if user_obj.twofa_enabled:
                    ...
                    session["_2fa_api_login_remember"] = (
                        remember_me_from_json  # Store remember for 2FA flow
                    )
                    session.modified = True
                    log.debug(
                        f"2FA required for API user {user_obj.username}. Stored remember_me: {remember_me_from_json} in session."
                    )
                    return (
                        jsonify(
                            {
                    ...
                    try:
                        db.session.commit()
                        log.info(
                            f"User '{user_obj.username}' (API JSON login, no 2FA) logged in successfully via Flask-Login. Last login updated. Remember: {remember_me_from_json}"
                        )
                    except Exception as e_db_commit:
                        db.session.rollback()
                        log.exception(
                            f"DB error updating last_login for '{user_obj.username}' (API JSON login): {e_db_commit}"
                        )
                ...
                try:
                    ...
                    db.session.rollback()
                    log.error(f"Failed to log failed API login: {log_err}")
            ...
            if (
            ...
            ):  # Validates CSRF and form fields from request.form
                username = (
                    form.username.data or ""
                ).strip()  # Ensure data is not None before strip
                password = form.password.data
                remember = form.remember_me.data

                log.info(f"Processing form login for user: '{username}'")
                user = DBUser.query.filter(
                    func.lower(DBUser.username) == func.lower(username)
                ).first()
                ...
                if user and bcrypt.check_password_hash(user.password_hash, password):
                    ...
                    if user.twofa_enabled:
                        log.info(
                            f"2FA required for '{user.username}'. Storing user ID for 2FA verification."
                        )
                        session.clear()
                        ...
                        try:
                            db.session.commit()
                            log.info(
                                f"User '{user.username}' logged in successfully. Last login updated."
                            )
                        except Exception as e_db_commit:
                            db.session.rollback()
                            log.exception(
                                f"DB error updating last_login for '{user.username}': {e_db_commit}"
                            )
                    ...
                    log.warning(
                        f"Form login failed for user '{username}': Invalid credentials."
                    )
                    ...
                    try:
                        ...
                        db.session.rollback()
                        log.error(f"Failed to log failed form login: {log_err}")
    ...
    return render_template(
        "auth/login.html", title="Login", form=form, active_form=active_form_type
    )
...
def me_route():
    ...
    try:
        ...
        log.info(
            f"/api/v1/auth/me - Attempting to fetch user by firebase_uid: {firebase_uid}"
        )
        user = DBUser.query.filter_by(firebase_uid=firebase_uid).first()

        if not user:
            log.warning(
                f"/api/v1/auth/me - User with firebase_uid {firebase_uid} not found in local DB."
            )
            return jsonify({"error": "Brugerprofil ikke fundet i systemet."}), 404

        if not user.is_active:
            log.warning(
                f"/api/v1/auth/me - User {user.username} (firebase_uid: {firebase_uid}) is inactive."
            )
        ...
        try:
            ...
            log.warning(
                f"Could not generate avatar_url for {user.username} in /me response."
            )
...
def admin_login():
    ...
    try:
        data = request.get_json()
        if not data:
            return jsonify({"error": "Tom anmodning."}), 400

        username_req = data.get("username", "").strip()
        password_req = data.get("password", "")  # Undlad strip initielt
        if not password_req:
            password_req = " "  # Sæt til mellemrum for at undgå typefejl ved None, check sker nedenfor

        errors = {}
        if not username_req:
            errors["username"] = "Admin brugernavn påkrævet."
        if password_req == " " or not password_req:
            errors["password"] = (
                "Admin password påkrævet."  # Tjek for tom efter evt. " "
            )
        if errors:
            return jsonify({"errors": errors}), 400
        ...
        if not admin_config_user or not admin_config_pass:
            log.critical(
                "Admin login function accessed, but ADMIN credentials not set in config."
            )
        ...
        creds_match = username_req == admin_config_user and hmac.compare_digest(
            password_req.encode("utf-8"), admin_config_pass.encode("utf-8")
        )

        if creds_match:
            log.info(
                f"Admin config credentials VALID for '{username_req}'. Verifying DB user data & role..."
            )

            # Fetch admin user from DB (case-insensitive username check)
            admin_user_obj = DBUser.query.filter(
                func.lower(DBUser.username) == func.lower(username_req)
            ).first()

            if not admin_user_obj:
                log.critical(
                    f"FATAL: Admin '{username_req}' valid config credentials, but NO DBUser found in database!"
                )
                return (
                    jsonify(
                        {
                            "error": "Admin brugerkonto data mangler. Kontakt systemadministrator."
                        }
                    ),
                    500,
                )
            ...
            if admin_user_obj.role != "admin":
                log.critical(
                    f"SECURITY ALERT: User '{admin_user_obj.username}' (DB ID: {admin_user_obj.id}) used valid admin config creds, but DB role is '{admin_user_obj.role}'! Denying login."
                )
                return jsonify({"error": "Adgang nægtet (konto er ikke admin)."}), 403

            if not admin_user_obj.is_active:
                log.critical(
                    f"SECURITY ALERT: Admin account '{admin_user_obj.username}' (DB ID: {admin_user_obj.id}) is INACTIVE in DB but correct config credentials were used!"
                )
                return (
                    jsonify({"error": "Adgang nægtet (admin konto deaktiveret)."}),
                    403,
                )
            ...
            if login_user(admin_user_obj, remember=True):  # Remember admin session
                ...
                try:
                    db.session.commit()
                    log.info(
                        f"Admin user '{admin_user_obj.username}' (DB ID: {admin_user_obj.id}) logged in successfully. Last login updated."
                    )
                except Exception as e_db_commit:
                    db.session.rollback()
                    log.exception(
                        f"DB error updating admin last_login for '{admin_user_obj.username}': {e_db_commit}"
                    )
                ...
                # except Exception:
                #     log.warning(
                #         f"Could not generate avatar_url for admin {admin_user_obj.username} during login response."
                #     )
                ...
                log.critical(
                    f"Flask-Login login_user() failed for verified admin DBUser '{admin_user_obj.username}'!"
                )
                return jsonify({"error": "Intern admin login systemfejl."}), 500
        else:  # Config credentials did not match
            log.warning(
                f"Failed admin login attempt for user '{username_req}': Invalid config credentials."
            )
...
def verify_2fa_login():
    ...
    user_pk_from_session = session.get(
        "_2fa_api_login_user_id"
    )  # Use API-specific session key

    if not user_pk_from_session:
        log.warning(
            "Verify 2FA TOTP failed: No _2fa_api_login_user_id found in session for API flow."
        )
        return (
            jsonify(
                {"error": "Ingen aktiv 2FA login session. Prøv at logge ind igen."}
            ),
            403,
        )
    ...
    try:
        remember = session.get(
            "_2fa_api_login_remember", False
        )  # Get remember status from session
        data = request.get_json()
        if not data:
            return jsonify({"error": "Tom anmodning."}), 400
        ...
        errors = {}
        if not code:
            errors["code"] = "2FA kode er påkrævet."
        elif not re.fullmatch(r"\d{6}", code):
            errors["code"] = "Ugyldigt format (forventede 6 tal)."
        if errors:
            return jsonify({"errors": errors}), 400
        ...
        if not user_obj:
            log.error(
                f"Verify 2FA CRITICAL: DBUser NOT found for PK '{user_pk_from_session}' from session."
            )
            session.pop(
                "_2fa_api_login_user_id", None
            )  # Clear API-specific session key
        ...
        if not user_obj.twofa_enabled or not user_obj.twofa_secret:
            log.error(
                f"Verify 2FA attempted for '{user_obj.username}', but 2FA not properly configured in DB."
            )
            session.pop("_2fa_login_user_id", None)
            session.pop("_2fa_login_remember", None)
            return (
                jsonify(
                    {
                        "error": "2FA opsætning mangler eller er korrupt. Prøv evt. login med backup kode."
                    }
                ),
                400,
            )
        ...
        if totp.verify(code, valid_window=1):
            log.info(
                f"2FA TOTP code VALID for user '{user_obj.username}'. Finalizing login."
            )
            if login_user(user_obj, remember=remember):
                ...
                try:
                    db.session.commit()
                    log.info(
                        f"DB: last_login updated for user '{user_obj.username}' post-2FA."
                    )
                except Exception as e_db_commit:
                    db.session.rollback()
                    log.exception(
                        f"DB error updating last_login post-2FA for '{user_obj.username}': {e_db_commit}"
                    )

                session.pop("_2fa_api_login_user_id", None)
                redirect_url = (
                    url_for("admin.admin_menu")
                ...
                # except (
                #     RuntimeError
                # ):  # Handle if url_for fails (e.g. outside app context, though unlikely here)
                #     log.warning(
                #         f"Could not generate avatar_url for {user_obj.username} during 2FA login response due to RuntimeError."
                #     )
                ...
                #     log.warning(
                #         f"Could not generate avatar_url for {user_obj.username} during 2FA login response: {e_avatar}"
                #     )
                #     default_avatar_path = current_app.config.get(
                #         "DEFAULT_AVATAR", "avatars/default_avatar.png"
                #     )
                ...
                log.critical(
                    f"Flask-Login login_user() failed AFTER valid 2FA for '{user_obj.username}'."
                )
                session.pop("_2fa_api_login_user_id", None)
                session.pop("_2fa_api_login_remember", None)
                return (
                    jsonify({"error": "Intern systemfejl ved afslutning af login."}),
                    500,
                )
        else:  # TOTP code invalid
            log.warning(
                f"Invalid 2FA TOTP code provided by user '{user_obj.username}'."
            )
        ...
        log.exception(
            f"Unexpected error in /verify_2fa_login for user PK '{user_pk_from_session}': {e}"
        )
...
def verify_backup_code():
    ...
    if not user_pk_from_session:
        log.warning(
            "Verify Backup Code failed: No _2fa_api_login_user_id in session for API flow."
        )
    ...
    try:
        ...
        errors = {}
        if not backup_code_input:
            errors["backup_code"] = "Backup kode er påkrævet."
        if errors:
            return jsonify({"errors": errors}), 400
        ...
        if not user_obj:
            log.error(
                f"Verify Backup Code CRITICAL: DBUser not found for PK '{user_pk_from_session}'."
            )
            session.pop("_2fa_api_login_user_id", None)
        ...
        if not user_obj.twofa_enabled:
            log.error(
                f"Backup code use attempted for '{user_obj.username}', but 2FA is not enabled in DB."
            )
            session.pop("_2fa_login_user_id", None)
            session.pop("_2fa_login_remember", None)
            return jsonify({"error": "2FA er ikke aktiveret for denne konto."}), 400

        stored_backup_codes_json = (
            user_obj.backup_codes
        )  # This is a JSON string from DB
        stored_codes_list = (
            json.loads(stored_backup_codes_json) if stored_backup_codes_json else []
        )
        ...
        if code_found_and_valid:
            log.info(
                f"Backup code VERIFIED for user '{user_obj.username}'. Finalizing login and removing code."
            )
            if login_user(user_obj, remember=remember):
                ...
                try:
                    db.session.commit()
                    log.info(
                        f"DB: Backup code removed for '{user_obj.username}'. Remaining: {codes_remaining}. Last login updated."
                    )
                    if codes_remaining <= current_app.config.get(
                        "LOW_BACKUP_CODE_THRESHOLD", 3
                    ):
                        flash(
                            f"ADVARSEL: Kun {codes_remaining} backup kode(r) tilbage! Generer nye via indstillinger.",
                            "warning",
                        )
                except Exception as e_db_commit:
                    db.session.rollback()
                    log.exception(
                        f"DB error updating backup codes/last_login for '{user_obj.username}': {e_db_commit}"
                    )
                    flash(
                        "Serverfejl ved opdatering af konto status efter backup login.",
                        "danger",
                    )
                ...
                # except RuntimeError:
                #     log.warning(
                #         f"Could not generate avatar_url for {user_obj.username} during backup code login response due to RuntimeError."
                #     )
                #     default_avatar_path = current_app.config.get(
                #         "DEFAULT_AVATAR", "avatars/default_avatar.png"
                #     )
                ...
                # except Exception as e_avatar:
                #     log.warning(
                #         f"Could not generate avatar_url for {user_obj.username} during backup code login response: {e_avatar}"
                #     )
                #     default_avatar_path = current_app.config.get(
                #         "DEFAULT_AVATAR", "avatars/default_avatar.png"
                #     )
                ...
                if user_obj.firebase_uid is None:
                    response_data["message"] = (
                        "Login succesfuldt med backup kode! Overvej at forbinde din konto til Firebase for forbedret sikkerhed og funktioner."
                    )
                    response_data["needs_firebase_link"] = True
                else:
                    response_data["message"] = (
                        "Login succesfuldt med backup kode! Tip: Du kan også logge ind via Firebase."
                    )
                ...
                log.critical(
                    f"Flask-Login login_user() failed AFTER valid backup code for '{user_obj.username}'."
                )
                session.pop("_2fa_api_login_user_id", None)
                session.pop("_2fa_api_login_remember", None)
        ...
        log.exception(
            f"Unexpected error in /verify_backup_code for user PK '{user_pk_from_session}': {e}"
        )
        db.session.rollback()
        session.pop("_2fa_api_login_user_id", None)
...
def setup_2fa_qr():
    ...
    log.info(
        f"Initiating 2FA QR code setup for user '{user_obj.username}' (ID: {user_obj.id})."
    )

    try:
        if user_obj.twofa_enabled:
            log.info(
                f"User '{user_obj.username}' accessed /setup_2fa, but 2FA is already enabled."
            )
        ...
        if not secret:
            ...
            try:
                db.session.commit()
                log.info(
                    f"New 2FA secret generated and SAVED to DB for '{user_obj.username}'."
                )
            except Exception as e_commit:
                db.session.rollback()
                log.exception(
                    f"DB error: Failed to save newly generated 2FA secret for '{user_obj.username}': {e_commit}"
                )
                flash("Serverfejl ved lagring af 2FA opsætning. Prøv igen.", "danger")
                return redirect(url_for("main.settings"))
        else:
            log.debug(
                f"Re-using existing 2FA secret from DB to generate QR for '{user_obj.username}'."
            )
        ...
        issuer_name = re.sub(
            r"[^\w\.\-]", "", current_app.config.get("APP_NAME", "FatteCentralen")
        )
        provisioning_uri = totp.provisioning_uri(
            name=safe_name_for_uri, issuer_name=issuer_name
        )
        ...
        log.exception(
            f"Unexpected error during /setup_2fa GET for '{user_obj.username}': {e}"
        )
        # Attempt rollback if a DB session was active and an error occurred before commit
        if (
            db.session.is_active
        ):  # Check if a session is active before trying to rollback
            db.session.rollback()
...
def verify_and_enable_2fa():
    ...
    log.info(
        f"Attempting 2FA verification and activation for user '{user_obj.username}' (ID: {user_obj.id})."
    )
    if not request.is_json:
        return jsonify({"error": "Forkert format (JSON påkrævet)."}), 415
    ...
    try:
        ...
        errors = {}
        if not code:
            errors["code"] = "Verifikationskode er påkrævet."
        elif not re.fullmatch(r"\d{6}", code):
            errors["code"] = "Ugyldigt format (6 tal)."
        if errors:
            return jsonify({"errors": errors}), 400
        ...
        if not secret:
            log.error(
                f"CRITICAL: 2FA secret missing for user '{user_obj.username}' on activation verify step. User should re-initiate setup."
            )
            return (
                jsonify(
                    {
                        "error": "2FA hemmelighed mangler. Genstart venligst opsætningen fra din profilside."
                    }
                ),
                400,
            )
        ...
        if user_obj.twofa_enabled:
            log.info(
                f"User '{user_obj.username}' attempted to re-enable already active 2FA."
            )
        ...
        if totp.verify(code, valid_window=1):
            log.info(
                f"2FA activation code VALID for '{user_obj.username}'. Enabling 2FA & generating backup codes..."
            )
            ...
            if hash_backup_codes_config:
                codes_to_store_in_db = [
                    bcrypt.generate_password_hash(c).decode("utf-8")
                    for c in new_backup_codes_plain
                ]
            else:
                codes_to_store_in_db = new_backup_codes_plain

            user_obj.twofa_enabled = True
            user_obj.backup_codes = json.dumps(
                codes_to_store_in_db
            ...
            try:
                db.session.commit()
                log.info(
                    f"Successfully ENABLED 2FA and saved backup codes for '{user_obj.username}'."
                )
                return (
                    jsonify(
                        {
                ...
                db.session.rollback()
                log.exception(
                    f"DB error: Failed to save 2FA activation state for '{user_obj.username}': {e_commit}"
                )
                return jsonify({"error": "Server datafejl ved aktivering af 2FA."}), 500
        else:
            log.warning(
                f"Invalid 2FA code provided during activation by '{user_obj.username}'."
            )
        ...
        log.exception(
            f"Unexpected error in /settings/verify-2fa for '{user_obj.username}': {e}"
        )
...
def disable_2fa():
    ...
    user_obj = current_user  # DBUser object
    log.info(
        f"Attempting to disable 2FA for user '{user_obj.username}' (ID: {user_obj.id}) via settings."
    )
    if not request.is_json:
        return jsonify({"error": "Forkert format (JSON påkrævet)."}), 415
    ...
    try:
        ...
        if not password_check:
            errors["password"] = "Password er påkrævet for at deaktivere."
        if errors:
            return jsonify({"errors": errors}), 400

        if not bcrypt.check_password_hash(user_obj.password_hash, password_check):
            log.warning(
                f"Disable 2FA FAILED for '{user_obj.username}': Incorrect password."
            )
            return (
                jsonify({"errors": {"password": "Forkert adgangskode angivet."}}),
                401,
            )

        log.debug(
            f"Password confirmed for '{user_obj.username}'. Proceeding with 2FA disable."
        )
        if not user_obj.twofa_enabled:
            log.info(
                f"User '{user_obj.username}' tried to disable already inactive 2FA."
            )
        ...
        try:
            ...
            return (
                jsonify(
                    {"message": "2FA deaktiveret succesfuldt.", "flashes": flashed_msgs}
                ),
                200,
            )
        except Exception as e_commit:
            db.session.rollback()
            log.exception(
                f"DB error: Failed to save data after disabling 2FA for '{user_obj.username}': {e_commit}"
            )
            return (
                jsonify({"error": "Server datafejl. Kunne ikke gemme deaktivering."}),
                500,
            )
        ...
        log.exception(
            f"Unexpected exception disabling 2FA for '{user_obj.username}': {e}"
        )
        if db.session.is_active:
            db.session.rollback()
...
def generate_new_backup_codes_endpoint():
    ...
    user_obj = current_user  # DBUser object
    log.info(
        f"Request to generate new backup codes for user '{user_obj.username}' (ID: {user_obj.id})."
    )
    ...
    try:
        if not user_obj.twofa_enabled:
            log.warning(
                f"User '{user_obj.username}' requested new backup codes, but 2FA is not enabled."
            )
            return (
                jsonify(
                    {"error": "2FA skal være aktiveret for at generere backup koder."}
                ),
                403,
            )

        log.debug(
            f"Generating new backup codes for 2FA-enabled user '{user_obj.username}'."
        )
        ...
        try:
            db.session.commit()
            log.info(
                f"Successfully generated and SAVED new backup codes to DB for user '{user_obj.username}'."
            )
            return (
                jsonify(
                    {
            ...
            db.session.rollback()
            log.exception(
                f"DB error: Failed to save new backup codes for '{user_obj.username}': {e_commit}"
            )
            return (
                jsonify({"error": "Server datafejl ved lagring af nye backup koder."}),
                500,
            )

    except Exception as e:
        log.exception(
            f"Unexpected error generating backup codes for '{user_obj.username}': {e}"
        )
        if db.session.is_active:
            db.session.rollback()
        return (
            jsonify({"error": "Intern serverfejl ved generering af backup koder."}),
            500,
        )
...
def logout_route():
    ...
    try:
        ...
        if hasattr(current_user, "username"):
            username_before = current_user.username
        log.info(
            f"Logout initiated for user '{username_before}' (ID: {user_id_before})."
        )
        ...
        log.info(
            f"User '{username_before}' (ID: {user_id_before}) logged out successfully and session cleared."
        )
    except Exception as e:
        log.exception(
            f"Exception during logout process for user '{username_before}' (ID: {user_id_before}): {e}"
        )
...
def request_password_reset():
    ...
    if request.method == "POST":
        ...
        if not username:
            ...
            return render_template(
                "auth/request_reset.html", title="Anmod om Nulstilling - Fejl"
            )
        ...
        if user:
            if user.firebase_uid:
                log.info(
                    f"Password reset requested for Firebase-linked user '{user.username}'. Guiding to Firebase reset."
                )
                flash(
                    "Din konto er forbundet med Firebase. Brug venligst Firebase til at nulstille dit password.",
                    "info",
                )
            else:
                ...
                try:
                    ...
                    reset_request = PasswordResetRequest(
                        user_id=user.id, token=token, expires_in_hours=24
                    )
                    db.session.add(reset_request)
                    db.session.commit()
                    log.info(
                        f"Password reset request CREATED in DB for local user '{user.username}' (ID: {user.id}). Token: {token[:8]}..."
                    )  # Log only prefix
                    flash(
                        f"Anmodning om nulstilling af password for '{user.username.title()}' er modtaget. En administrator vil behandle den snarest.",
                        "success",
                    )
                except Exception as e_req:
                    db.session.rollback()
                    log.exception(
                        f"Failed to create password reset request for local user '{user.username}': {e_req}"
                    )
                    flash(
                        "Der opstod en serverfejl under behandling af din anmodning. Prøv igen.",
                        "danger",
                    )
            ...
            log.warning(
                f"Password reset requested for potentially non-existent user: '{username}'"
            )
            # Capitalize the username entered by the user for the flash message
            flash(
                f"Hvis brugernavnet '{username.title()}' findes i systemet, er din anmodning modtaget og vil blive behandlet af en administrator.",
                "info",
            )
    ...
    try:
        return render_template(
            "auth/request_reset.html", title="Anmod om Nulstilling af Password"
        )
    except TemplateNotFound:
        log.exception(
            "CRITICAL: Password reset request template 'auth/request_reset.html' not found!"
        )
...
def reset_with_token(token):
    ...
    if not reset_request or not reset_request.is_valid:
        ...
        flash(
            "Linket til nulstilling af password er ugyldigt eller udløbet. Prøv at anmode om et nyt.",
            "danger",
        )
        return redirect(url_for(".login_route"))
    ...
    if not user:
        log.error(
            f"User ID {reset_request.user_id} for valid token {token} not found in DB."
        )
        flash(
            "Der opstod en fejl. Brugeren tilknyttet dette link findes ikke.", "danger"
        )
    ...
    if form.validate_on_submit():  # This handles POST request
        try:
            ...
            if new_password:
                ...
                if settings.get("password_require_uppercase", True) and not re.search(
                    r"[A-Z]", new_password
                ):
                    pw_errors.append("mindst ét stort bogstav")
                if settings.get("password_require_lowercase", True) and not re.search(
                    r"[a-z]", new_password
                ):
                    pw_errors.append("mindst ét lille bogstav")
                if settings.get("password_require_number", True) and not re.search(
                    r"[0-9]", new_password
                ):
                    pw_errors.append("mindst ét tal")
                if settings.get("password_require_symbol", False) and not re.search(
                    r"[!@#$%^&*(),.?\":{}|<>]", new_password
                ):
            ...
            if pw_errors:
                ...
                if hasattr(form.password, "errors") and isinstance(
                    form.password.errors, list
                ):
                    form.password.errors.append(
                        "Password skal indeholde: " + ", ".join(pw_errors) + "."
                    )
                else:
                    log.error(
                        "Could not append password complexity errors to form field."
                    )
                log.warning(
                    f"Password reset for user '{user.username}' failed complexity check."
                )
                ...
                reset_request.handled_at = datetime.now(
                    timezone.utc
                ...
                db.session.commit()
                log.info(
                    f"Password successfully reset for user '{user.username}' (ID: {user.id}) using token {token[:8]}..."
                )
                flash(
                    "Dit password er blevet nulstillet. Du kan nu logge ind med dit nye password.",
                    "success",
                )
                return redirect(url_for(".login_route"))
            ...
            db.session.rollback()
            log.exception(
                f"Error processing password reset for user '{user.username}' with token {token[:8]}...: {e_reset}"
            )
            flash(
                "Der skete en serverfejl under nulstilling af password. Prøv igen.",
                "danger",
            )
            # Render login template with reset form active
            return render_template(
                "auth/login.html",
                title="Nulstil Password - Fejl",
                form=form,
                token=token,
                active_form="reset",
            )
    ...
    try:
        ...
        log.exception(
            "CRITICAL: Password reset form template 'auth/reset_password_form.html' not found!"
        )
        return "<h1>Error 500: Template missing.</h1>", 500
    except Exception as render_err:
...
def link_local_account_to_firebase():
    ...
    """
    log.info(
        f"Attempting to link Firebase account for local user: {current_user.username} (ID: {current_user.id})"
    )

    if not request.is_json:
        log.warning(
            f"Link Firebase failed for {current_user.username}: Non-JSON request."
        )
        return jsonify({"error": "Forkert anmodningsformat (forventede JSON)."}), 415
    ...
    if not data:
        log.warning(
            f"Link Firebase failed for {current_user.username}: Empty JSON payload."
        )
    ...
    if not firebase_id_token:
        log.warning(
            f"Link Firebase failed for {current_user.username}: Missing 'firebase_id_token' in payload."
        )
        return (
            jsonify(
                {"errors": {"firebase_id_token": "Firebase ID token er påkrævet."}}
            ),
            400,
        )
    ...
    try:
        ...
        decoded_token = firebase_auth.verify_id_token(
            firebase_id_token, check_revoked=True
        )
        firebase_uid = decoded_token["uid"]
        log.info(
            f"Firebase ID token verified for linking. UID: {firebase_uid}. Local user: {current_user.username}"
        )
        ...
        other_user_with_firebase_uid = DBUser.query.filter(
            DBUser.firebase_uid == firebase_uid, DBUser.id != current_user.id
        ).first()

        if other_user_with_firebase_uid:
            log.warning(
                f"Link Firebase failed for {current_user.username}: Firebase UID {firebase_uid} is already linked to local user {other_user_with_firebase_uid.username} (ID: {other_user_with_firebase_uid.id})."
            )
            return (
                jsonify(
                    {
                        "error": "Denne Firebase konto er allerede forbundet til en anden lokal brugerprofil."
                    }
                ),
                409,
            )  # Conflict
        ...
        if current_user.firebase_uid and current_user.firebase_uid != firebase_uid:
            log.warning(
                f"Link Firebase failed for {current_user.username}: Account already linked to a different Firebase UID ({current_user.firebase_uid}). Attempted to link to {firebase_uid}."
            )
            return (
                jsonify(
                    {
                        "error": "Din lokale konto er allerede forbundet til en anden Firebase konto. Kontakt support hvis du mener dette er en fejl."
                    }
                ),
                409,
            )  # Conflict
        ...
        if current_user.firebase_uid == firebase_uid:
            log.info(
                f"User {current_user.username} is already linked to this Firebase account (UID: {firebase_uid}). No action needed."
            )
            return (
                jsonify(
                    {
                        "message": "Konto er allerede forbundet til denne Firebase profil."
                    }
                ),
                200,
            )
        ...
        db.session.commit()
        log.info(
            f"Successfully linked local user {current_user.username} (ID: {current_user.id}) to Firebase UID: {firebase_uid}."
        )
        ...
        return (
            jsonify({"message": "Din konto er nu succesfuldt forbundet med Firebase!"}),
            200,
        )

    except firebase_auth.RevokedIdTokenError:
        log.warning(
            f"Link Firebase failed for {current_user.username}: Firebase token revoked."
        )
        return (
            jsonify(
                {
                    "error": "Firebase token er blevet tilbagekaldt. Prøv at logge ind på Firebase igen."
                }
            ),
            401,
        )
        ...
        log.warning(
            f"Link Firebase failed for {current_user.username}: Firebase user account is disabled."
        )
        return jsonify({"error": "Din Firebase brugerkonto er deaktiveret."}), 403
    except firebase_auth.InvalidIdTokenError as e:
        log.warning(
            f"Link Firebase failed for {current_user.username}: Invalid Firebase ID token: {e}"
        )
        return jsonify({"error": f"Ugyldigt Firebase ID token: {e}"}), 401
    except Exception as e:
        db.session.rollback()
        log.exception(
            f"Unexpected error linking Firebase account for {current_user.username}: {e}"
        )
        return (
            jsonify(
                {"error": "Intern serverfejl under sammenkædning af Firebase konto."}
            ),
            500,
        )
