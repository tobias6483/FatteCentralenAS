...
from ..models import User, SportEvent, SportOutcome, BetHistory
...
MAX_BETTING_INTO_LIVE_MINUTES = 5 # Allow betting up to 5 minutes after official start time
...
def place_bet():
    ...
    """
    current_user_identity = get_jwt_identity() # This is the user's ID (or username, depending on JWT setup)
    ...
    if not all([match_id, outcome_id, stake_str]):
        return jsonify({"error": "Manglende felter: match_id, outcome_id, stake er påkrævet."}), 400
    ...
    betting_deadline = match.commence_time + timedelta(minutes=MAX_BETTING_INTO_LIVE_MINUTES)
    
    if now_utc > betting_deadline:
        log.warning(f"Bet attempt on match {match_id} after betting deadline. Now: {now_utc}, Deadline: {betting_deadline}")
        return jsonify({"error": "Spil er lukket for denne kamp (kampen er startet eller afsluttet)."}), 403

    # 2. Check user balance
    if user.balance < stake:
        log.warning(f"User {user.id} insufficient balance. Has: {user.balance}, Needs: {stake}")
    ...
    try:
        ...
        match_name_display = f"{match.home_team} vs {match.away_team}" if match.home_team and match.away_team else f"Event ID: {match.id}"
        ...
        log.info(f"User {user.id} placed bet {new_bet.id} (stake: {stake}) on outcome {outcome.id} for match {match.id}")
...
def get_bet_history():
    ...
    try:
        ...
        log.error(f"Invalid user_id format in JWT for bet history: {current_user_identity}")
        return jsonify({"error": "Ugyldigt bruger ID format i token."}), 401
    
    if not user:
        log.warning(f"User not found for JWT identity for bet history: {current_user_identity}")
    ...
    try:
        ...
        paginated_bets = user_bets_query.paginate(page=page, per_page=per_page, error_out=False)
        ...
        return jsonify({"error": "Intern serverfejl ved hentning af spilhistorik."}), 500