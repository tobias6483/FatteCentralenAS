...
from ..extensions import (  # <-- Added redis_client and db
    csrf,
    db,
    redis_client,
    socketio,
)
from ..models import (  # <-- Added models
    BetHistory,
    GameSession,
    SportEvent,
    SportOutcome,
...
from ..utils import award_badge, get_common_context  # <-- Import award_badge
...
USER_SESSIONS_KEY_PREFIX = (
    "user_sessions:"  # Optional: For faster active session lookup
)
...
def create_session():  # Route is synchronous
    ...
    try:
        ...
        if game_mode == "live_sport":
            ...
            if not event_id:
                return (
                    jsonify({"error": "Event ID mangler for live_sport session"}),
                    400,
                )
            ...
            if not event_db:
                return (
                    jsonify(
                        {"error": f"Sportsbegivenhed med ID '{event_id}' ikke fundet."}
                    ),
                    404,
                )
            ...
            session_name = (
                ...
                or f"{event_db.home_team} vs {event_db.away_team}"
            )
            ...
            if not h2h_outcomes_db:
                return (
                    jsonify(
                        {
                            "error": f"Ingen H2H odds fundet for event '{event_id}' i databasen."
                        }
                    ),
                    400,
                )
            ...
            for outcome_db in h2h_outcomes_db:
                if outcome_db.bookmaker == bookmaker_key_used:
                    outcomes.append(
                        {
                            "name": outcome_db.name,
                            "odds": outcome_db.price,
                            "pot": 0.0,
            ...
            if len(outcomes) < 2:
                logger.warning(
                    f"Found event {event_id} but insufficient H2H outcomes from bookmaker {bookmaker_key_used}."
                )
                return (
                    jsonify(
                        {
                            "error": f"Utilstrækkelige H2H odds fundet for event '{event_id}'."
                        }
                    ),
                    400,
                )
            ...
            for o in outcomes_in:
                name = o.get("name")
                odds_str = o.get("odds")
                if not name or odds_str is None:
                    return (
                        jsonify(
                            {
                                "error": "Ugyldigt outcome-format (mangler navn eller odds)"
                            }
                        ),
                        400,
                    )
                try:
                    ...
                    return (
                        jsonify({"error": "Ugyldigt odds-format (skal være tal)"}),
                        400,
                    )
                if odds_val < 1.01:
                    return (
                        jsonify({"error": f"Odds for '{name}' skal være mindst 1.01"}),
                        400,
                    )
                outcomes.append({"name": name, "odds": odds_val, "pot": 0.0})
            if not outcomes:
                return jsonify({"error": "Der skal være mindst ét udfald"}), 400
            ...
            if (
                not selections_in
                or not isinstance(selections_in, list)
                or len(selections_in) == 0
            ):
                return jsonify({"error": "Kuponen skal indeholde mindst ét valg."}), 400

            logger.info(
                f"Processing live_sport_coupon with {len(selections_in)} selections."
            )
            ...
            for sel_idx, sel in enumerate(selections_in):
                ...
                if not all([event_id, outcome_name, odds_val]):
                    logger.warning(
                        f"Invalid selection at index {sel_idx} in coupon: {sel}"
                    )
                    return (
                        jsonify(
                            {
                                "error": f"Ugyldigt valg #{sel_idx + 1} i kuponen (manglende data)."
                            }
                        ),
                        400,
                    )
                try:
                    ...
                    if odds_float < 1.01:
                        return (
                            jsonify(
                                {
                                    "error": f"Odds for '{outcome_name}' i valg #{sel_idx + 1} skal være mindst 1.01"
                                }
                            ),
                            400,
                        )
                    ...
                    logger.warning(
                        f"Invalid odds format for selection {sel_idx} in coupon: {sel}"
                    )
                    return (
                        jsonify(
                            {"error": f"Ugyldigt odds-format for valg #{sel_idx + 1}."}
                        ),
                        400,
                    )
        ...
        try:
            ...
            try:
                new_db_session = GameSession(
                    ...
                    coupon_details=(
                        outcomes if game_mode == "live_sport_coupon" else None
                    ),
                )
                db.session.add(new_db_session)
                db.session.commit()
                logger.info(
                    f"Session '{session_id}' ('{session_name}') also created in SQL DB by user ID '{creator}'. Linked Event ID: {linked_event_id}"
                )
            except Exception as db_err:
                db.session.rollback()
                logger.exception(
                    f"DB error creating GameSession record for {session_id}: {db_err}. Redis data might persist."
                )
                return (
                    jsonify({"error": "Databasefejl ved oprettelse af session (SQL)."}),
                    500,
                )
            ...
            if game_mode == "live_sport_coupon":
                try:
                    coupon_session_count = GameSession.query.filter_by(
                        creator_id=creator, game_mode="live_sport_coupon"
                    ).count()
                    if coupon_session_count == 1:
                        award_badge(creator, "Kupon Bygger")
                except Exception as badge_err:
                    logger.error(
                        f"Error checking/awarding 'coupon_creator' badge for user {creator}: {badge_err}"
                    )
            try:
                total_sessions_created = GameSession.query.filter_by(
                    creator_id=creator
                ).count()
                if total_sessions_created == 5:
                    award_badge(creator, "Spilmester")
            except Exception as badge_err:
                logger.error(
                    f"Error checking/awarding 'session_starter_5' badge for user {creator}: {badge_err}"
                )
            # --- End Badge Checks ---

            logger.info(
                f"Session '{session_id}' ('{session_name}') created in Redis by '{creator}'. Added to {user_sessions_key}. TTL: {ttl_seconds}s"
            )

        except redis.RedisError as e:
            logger.exception(
                f"Redis error creating session {session_id} for {creator}: {e}"
            )
            return (
                jsonify({"error": "Databasefejl ved oprettelse af session (Redis)."}),
                500,
            )
        except Exception as e:
            logger.exception(
                f"Unexpected error creating session {session_id} for {creator}: {e}"
            )
            return (
                jsonify({"error": "Intern serverfejl ved oprettelse af session"}),
                500,
            )
        ...
        logger.exception(
            f"Error in create_session by {current_user.id if current_user.is_authenticated else 'anonymous'}: {e}"
        )
...
def join_session_route():  # Route is synchronous
    ...
    try:
        ...
        if not session_id:
            return jsonify({"error": "Session ID mangler"}), 400

        player_balance = current_user.balance
        # player_avatar = url_for(
        #     "static", filename=f"avatars/{player.user.avatar_filename}"
        # )
        ...
        try:
            ...
            # --- Check Max Players ---
            try:
                max_players = int(max_players_str) if max_players_str else 0
            except ValueError:
                max_players = 0

            if max_players > 0:
                current_player_count = redis_client.scard(
                    session_players_key
                )  # Sync call
                # Ensure comparison is valid (scard returns int)
                if (
                    isinstance(current_player_count, int)
                    and current_player_count >= max_players
                ):
                    logger.warning(
                        f"Join denied for {player_id} to session {session_id}: Max players ({max_players}) reached."
                    )
                    return (
                        jsonify(
                            {
                                "error": f"Sessionen er fuld (max {max_players} spillere)."
                            }
                        ),
                        403,
                    )
                    ...
                    logger.error(
                        f"Redis scard returned non-integer for {session_players_key}: {current_player_count}"
                    )

            # --- Check Privacy/Password ---
            try:
                is_private = json.loads(is_private_str) if is_private_str else False
            ...
            if is_private:
                ...
                if not provided_password:
                    logger.warning(
                        f"Join denied for {player_id} to private session {session_id}: No password provided."
                    )
                    return (
                        jsonify(
                            {
                                "error": "Kodeord mangler for at deltage i denne private session."
                            }
                        ),
                        401,
                    )
                if not stored_password or provided_password != stored_password:
                    logger.warning(
                        f"Join denied for {player_id} to private session {session_id}: Incorrect password provided."
                    )
                    return jsonify({"error": "Forkert kodeord angivet."}), 401
                logger.info(
                    f"Password verified for user {player_id} joining private session {session_id}."
                )

            # Check if player is already in the set
            is_member = redis_client.sismember(
                session_players_key, player_id_str
            )  # Sync call
            if is_member:
            ...
            logger.info(
                f"User '{player_id}' successfully joined session '{session_id}' in Redis. Added to {user_sessions_key}."
            )

            # --- Award 'active_participant' badge ---
            try:
                session_count_for_user = redis_client.scard(
                    user_sessions_key
                )  # Sync call
                if (
                    isinstance(session_count_for_user, int)
                    and session_count_for_user == 5
                ):
                    award_badge(player_id, "Aktiv Deltager")
            except Exception as badge_err:
                logger.error(
                    f"Error checking/awarding 'active_participant' badge for user {player_id}: {badge_err}"
                )
            # --- End badge check ---

        except redis.RedisError as e:
            logger.exception(
                f"Redis error joining session {session_id} for {player_id}: {e}"
            )
            return jsonify({"error": "Databasefejl ved tilslutning til session"}), 500
        except Exception as e:
            logger.exception(
                f"Unexpected error joining session {session_id} for {player_id}: {e}"
            )
            return (
                jsonify({"error": "Intern serverfejl ved tilslutning til session"}),
                500,
            )
        ...
        session_id_err = (
            data.get("session_id", "N/A") if isinstance(data, dict) else "N/A"
        )
        user_id_err = current_user.id if current_user.is_authenticated else "anonymous"
        logger.exception(
            f"Fejl i join_session_route for user {user_id_err}, session {session_id_err}: {e}"
        )
...
def active_sessions_route():  # Reverted to synchronous def
    ...
    logger.info(
        f"Active sessions requested by user {current_user_id} (Method: {request.method})"
    )
    ...
    try:
        ...
        logger.debug(
            f"Found {len(session_ids)} session IDs in set {user_sessions_key} for user {current_user_id}"
        )
        ...
        if session_ids:  # Now session_ids is correctly typed as Set[str]
            ...
            for i in range(0, len(results), chunk_size):
                ...
                if (
                    ...
                    and len(session_details_values) >= 6
                    and session_details_values[0] is not None
                ):
                    sdata = {
                        "session_id": session_details_values[0],
                        "session_name": session_details_values[1]
                        or session_details_values[0],
                        ...
                        "players_count": (
                            player_count if isinstance(player_count, int) else 0
                        ),
                    }
                    ...
                    if expired_session_id_index < len(session_ids_list):
                        ...
                        logger.warning(
                            f"Session details for {expired_session_id} (from user set {user_sessions_key}) not found or incomplete in Redis, likely expired. Details: {session_details_values}"
                        )
        ...
        logger.exception(
            f"Fejl i active_sessions_route for user {current_user_id}: {e}"
        )
        if (
            request.accept_mimetypes.accept_json
            and not request.accept_mimetypes.accept_html
...
def session_detail_page(session_id):  # Route is synchronous
    ...
    try:
        ...
        # Decode player IDs
        player_ids_decoded = {
            pid for pid in player_ids_bytes
        }  # Removed .decode('utf-8') as Redis client decodes responses
        ...
        for k_bytes, v_bytes in session_data_raw.items():
            ...
            if k in ["is_coupon", "is_private"]:
                try:
                    session_data_processed[k] = json.loads(v) if v else False
                except json.JSONDecodeError:
                    session_data_processed[k] = False
            elif k in ["outcomes", "bets"]:
                try:
                    session_data_processed[k] = json.loads(v) if v else []
                except json.JSONDecodeError:
                    session_data_processed[k] = []
            else:
                session_data_processed[k] = v

        is_coupon = session_data_processed.get("is_coupon", False)
        if "game_mode" not in session_data_processed:
            session_data_processed["game_mode"] = "unknown"
        if "status" not in session_data_processed:
            session_data_processed["status"] = "unknown"
        if "outcomes" not in session_data_processed:
            session_data_processed["outcomes"] = []
        if "bets" not in session_data_processed:
            session_data_processed["bets"] = []
        ...
        if player_ids_decoded:
            try:
                ...
                logger.error(
                    f"Could not convert player IDs to integers for session {session_id}. IDs: {player_ids_decoded}"
                )
                players_details = {
                    pid: {"username": f"User {pid}"} for pid in player_ids_decoded
                }
            except Exception as db_err:
                logger.exception(
                    f"Error fetching player details from DB for session {session_id}: {db_err}"
                )
                players_details = {
                    pid: {"username": f"User {pid}"} for pid in player_ids_decoded
                }
        ...
        try:
            creator_id_int = int(session_data_processed.get("creator", -1))
        except (ValueError, TypeError):
            creator_id_int = -1
        ...
        logger.exception(
            f"Fejl i session_detail_page for session {session_id} by user {current_user.id}: {e}"
        )
        return render_template("errors/500.html"), 500
...
def join_session_via_invite_code(code):  # Route is synchronous
    ...
    logger.info(
        f"User '{user_id_str}' attempting to join session via invite code: {code}"
    )
    ...
    try:
        ...
        is_member = redis_client.sismember(
            session_players_key, user_id_str
        )  # Sync call
        if is_member:
            flash("Du er allerede deltager i denne session.", "info")
        else:
            # --- Check Max Players before adding ---
            max_players_bytes = redis_client.hget(
                session_key, "max_players"
            )  # Sync call
            max_players_str = max_players_bytes  # type: ignore[assignment]
            try:
                max_players = int(max_players_str) if max_players_str else 0  # type: ignore[arg-type]
            except ValueError:
                max_players = 0
            ...
            if max_players > 0:
                ...
                if (
                    ...
                    logger.warning(
                        f"Join via invite denied for {user_id_str} to session {session_id}: Max players ({max_players}) reached."
                    )
            ...
            logger.info(
                f"User '{user_id_str}' successfully joined session '{session_id}' via invite code '{code}'."
            )
            ...
            if user_obj:
                try:
                    ...
                    avatar_url = f"/static/{user_obj.avatar or Config.DEFAULT_AVATAR}"
                ...
                logger.error(
                    f"Could not find user object for ID {user_id} after joining session via invite."
                )
        ...
        logger.exception(
            f"Redis error processing invite code {code} for user {user_id_str}: {e}"
        )
        flash("Der opstod en databasefejl. Prøv venligst igen.", "danger")
    except Exception as e:
        logger.exception(
            f"Unexpected error processing invite code {code} for user {user_id_str}: {e}"
        )
