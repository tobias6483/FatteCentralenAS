...
user_profile_api_bp = Blueprint('user_profile_api_bp', __name__, url_prefix='/api/v1/users/me')
...
def get_user_profile():
    ...
    profile_data = {
        ...
        "email": user.email,
        "role": user.role.name if user.role else 'user', # Assuming role is an enum or object
        ...
        "privacy_settings": {
            "profile_public": user.privacy_settings.get("profile_public", True) if user.privacy_settings else True,
            "show_activity": user.privacy_settings.get("show_activity", False) if user.privacy_settings else False,
            "show_bet_history": user.privacy_settings.get("show_bet_history", True) if user.privacy_settings else True,
            "show_online_status": user.privacy_settings.get("show_online_status", True) if user.privacy_settings else True
        }
    }
...
def update_user_profile():
    ...
    if 'privacy_settings' in data and isinstance(data['privacy_settings'], dict):
        ...
        for key, value in data['privacy_settings'].items():
            ...
            if key in ["profile_public", "show_activity", "show_bet_history", "show_online_status"]:
