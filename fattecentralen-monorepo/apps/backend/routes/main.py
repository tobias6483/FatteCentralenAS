...
from ..models import User, BetHistory, Notification, Badge, League, SportEvent, SportOutcome, ForumPost, ForumThread # Added ForumPost, ForumThread
from ..utils import (
...
def get_bettable_events():
    ...
    try:
        ...
        events_query = db.session.query(SportEvent).join(League).filter(
        ...
        ).options(db.joinedload(SportEvent.league)).order_by(SportEvent.commence_time).all() # type: ignore[attr-defined]
        sports_events_dict = {}
        for event in events_query:
            ...
            if outcomes_list:
                sports_events_dict[league_slug]["events"].append({
                    "id": event.id, "home_team": event.home_team_display_name, "away_team": event.away_team_display_name,
