...
from apps.backend.models import (  # Add other models as needed
    BetHistory,
    ForumPost,
    ForumThread,
    GameSession,
...
from sqlalchemy import Date, cast, desc, func  # Explicitly import desc

api_dashboard_bp = Blueprint(
    "api_dashboard_bp", __name__, url_prefix="/api/v2/dashboard"
)
...
def get_main_dashboard_data():
    ...
    user_stats = {
        ...
        "last_login_relative": (
            format_datetime_relative(user.last_login) if user.last_login else "Aldrig"
        ),
        ...
        "net_profit_loss": (user.total_won or 0)
        - (user.total_lost or 0),  # Simple calculation
    }
    ...
    for i in range(6, -1, -1):  # Last 7 days including today
        ...
        balance_chart_data.append(
            user.balance
        )  # Replace with actual historical balance

    balance_chart = {"labels": balance_chart_labels, "data": balance_chart_data}

    # 3. Recent Activity (e.g., last 5 bet history entries)
    recent_activities_query = (
        BetHistory.query.filter_by(user_id=user.id)
        ...
        recent_activities.append(
            {
                ...
                "amount": (
                    activity.payout if activity.result == "won" else activity.stake
                ),
                "type": (
                    activity.result if activity.result in ["won", "lost"] else "stake"
                ),  # Simplified type
    ...
    latest_posts_query = (
        ...
        .order_by(desc(cast(ForumPost.created_at, db.Column)))
        .limit(3)
        .all()
    )
    ...
    open_sessions_query = (
        GameSession.query.filter_by(status="open")
