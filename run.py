...
if os.path.exists(dotenv_path):
    ...
    log.warning(
        f"--- NOTE: .env file not found at expected location ({dotenv_path}). Using system environment variables only."
    )
...
try:
    ...
    # -------------------
    log.info(
        "--- Successfully imported create_app from 'apps.backend' and socketio from 'apps.backend.extensions' ---"
    )
except ImportError as e:
    log.critical(
        f"--- CRITICAL ERROR: Failed importing core components. Error: {e} ---",
        exc_info=True,
    )
    ...
    log.critical(
        f"--- CRITICAL UNEXPECTED ERROR during imports. Error: {e} ---", exc_info=True
    )
    import sys
...
def update_sports_command():
    ...
    with app.app_context():
        try:
            ...
            log.exception(
                "CLI: Failed to import update_sports_catalog_from_api from app.utils."
            )
            sys.exit(1)
        except Exception as e:
            log.exception(
                f"CLI: An unexpected error occurred during update-sports: {e}"
            )
            sys.exit(1)
...
def update_events_command():
    ...
    with app.app_context():
        try:
            ...
            log.exception(
                f"CLI: An unexpected error occurred during update-events: {e}"
            )
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    log.info(
        f"--- Starting Flask-SocketIO development server (FLASK_ENV: {flask_env}, SocketIO Debug: {run_in_debug_mode}, Reloader: {use_reloader_flag}) ---"
    )
